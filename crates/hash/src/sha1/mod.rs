...
mod tests {
	...
	fn test_vector_4_incremental() {
		...
		hasher.update(
			b"abcdefghbcdefghicdefghijdefghijkefghijklfghijklmghij",
		);
		hasher.update(b"klmnhijklmnoijklmnopjklmnopqklmnopqrlmnopqrsmnopqrstnopqrstu");
		let result = hasher.finalize();

		let expected = [
			0xa4, 0x9b, 0x24, 0x46, 0xa0, 0x2c, 0x64, 0x5b, 0xf4,
			0x19, 0xf9, 0x95, 0xb6, 0x70, 0x91, 0x25, 0x3a, 0x04,
			0xa2, 0x59,
		];
		assert_eq!(result, Hash160(expected));
	}
	...
	fn test_sha1_1024_bytes() {
		...
		let expected = [
			...
			0x51, 0x0e, 0x1a, 0x70, 0xae, 0x48, 0xc7, 0x0f, 0x9b,
			0x93, 0x84,
		];
		assert_eq!(result, Hash160(expected));
	}
	...
	fn test_sha1_various_sizes() {
		...
		let sizes = [
			0, 1, 2, 3, 4, 7, 8, 9, 15, 16, 17, 31, 32, 33, 63, 64,
			65, 127, 128, 129, 255, 256, 257, 511, 512, 513, 1023,
