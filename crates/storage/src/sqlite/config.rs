...
mod tests {
    ...
    fn test_default_config() {
        ...
        assert_eq!(config.path, PathBuf::from("reify.db"));
        assert_eq!(config.journal_mode, JournalMode::Wal);
        assert_eq!(config.synchronous_mode, SynchronousMode::Normal);
        assert_eq!(config.temp_store, TempStore::Memory);
    ...
    fn test_safe_config() {
        temp_dir(|db_path| {
            let db_file = db_path.join("safe.db");
    ...
    fn test_fast_config() {
        temp_dir(|db_path| {
            let db_file = db_path.join("fast.db");
    ...
    fn test_config_chaining() {
        temp_dir(|db_path| {
            let db_file = db_path.join("chain.db");
    ...
    fn test_path_handling() {
        temp_dir(|db_path| {
            // Test with file path
            let file_path = db_path.join("test.db");
