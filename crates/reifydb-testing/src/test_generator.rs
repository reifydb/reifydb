...
//! directories of test scripts, making each test individually runnable and
//! debuggable.
...
impl TestGenerator {
	...
	/// * `output_subdir` - Subdirectory under tests/ for generated files
	///   (e.g., "generated")
	pub fn new(
		manifest_dir: &str,
		output_subdir: &str,
		...
		// If the target directory doesn't exist or is being rebuilt
		// (cargo clean was run), clean up generated test files
		...
		if !target_dir.exists() && output_dir.exists() {
			...
			if let Err(e) = fs::remove_dir_all(&output_dir) {
				eprintln!(
					"Warning: Failed to clean generated tests: {}",
					e
				);
			}
		}
	...
	pub fn add(&mut self, config: TestConfig) -> std::io::Result<()> {
		let full_path =
			Path::new(&config.manifest_dir).join(&config.test_dir);

		if !full_path.exists() {
			eprintln!(
				"Warning: Test directory does not exist: {}",
				full_path.display()
			);
			return Ok(());
		}
		...
		if test_files.is_empty() {
			eprintln!(
				"Warning: No test files found in: {}",
				full_path.display()
			);
			return Ok(());
		}
		...
		for file_path in test_files.iter() {
			...
			let test_file_path = if let Some(parent) =
				relative_from_test_dir.parent()
			{
				if !parent.as_os_str().is_empty() {
					let parent_dir =
						module_dir.join(parent);
					fs::create_dir_all(&parent_dir)?;
					parent_dir.join(format!(
						"{}.rs",
						relative_from_test_dir
							.file_name()
							.unwrap()
							.to_string_lossy()
					))
			...
			// Test function name: replace path separators with
			// underscores and prefix with test_
			let raw_name = relative_from_test_dir
				.to_string_lossy()
				.replace('/', "_")
				.replace('\\', "_");
			let test_name = format!("test_{}", raw_name);
	...
	pub fn generate_multi_path_tests(
		&mut self,
		config: MultiFileTestConfig,
	) -> std::io::Result<()> {
		let full_path =
			Path::new(&config.manifest_dir).join(&config.test_dir);
		...
		// Find file groups
		let test_groups =
			self.find_file_groups(&full_path, &config.file_groups)?;

		if test_groups.is_empty() {
			eprintln!(
				"Warning: No matching file groups found in: {}",
				full_path.display()
			);
			return Ok(());
		}
		...
		for group in test_groups.iter() {
			...
			let relative_paths: Vec<String> = group
				.iter()
				.map(|p| {
					p.strip_prefix(&config.manifest_dir)
						.unwrap_or(p)
						.to_string_lossy()
						.replace('\\', "/")
				})
...
"#,
	...
	fn collect_test_files(
		...
		for entry in fs::read_dir(dir)? {
			...
			if path.is_dir() {
				...
				// Only include files WITHOUT extensions (test
	...
	fn find_file_groups(
		...
		self.collect_files_with_extension(
			dir,
			first_ext,
			&mut base_files,
		)?;
	...
	fn format_single_test_file(
		...
		// Count slashes after "tests/scripts/" to determine nesting
		// level
		let after_scripts = relative_path
			.find("scripts/")
			.map(|i| &relative_path[i + 8..])
			.unwrap_or(relative_path);
		let depth = after_scripts.matches('/').count() + 2; // +2 for module dir and generated dir
		let super_path = "super::".repeat(depth);

		format!(r#"//! Auto-generated test for: {relative_path}
//! File: {file_name}
//! 
...
"#)
	...
	fn format_multi_test_file(
		...
		format!(r#"//! Auto-generated test for file group
//! Files: {paths_list}
//! 
//! This file is auto-generated by build.rs - DO NOT EDIT
//! 
...
"#)
	...
	fn generate_module_file(
		...
		// Recursively find all .rs files in the module directory and
...
"#,
		...
		self.generate_mod_declarations(
			module_dir,
			module_dir,
			&mut mod_content,
		)?;
		...
		for path in &entries {
					...
					.map_or(false, |ext| ext == "rs")
			{
				let file_name = path
					...
					.to_string_lossy();
				if file_name != "mod" {
					// Ensure module name is valid - prefix
					// with 'test_' if it starts with a
					// number
					let module_name = if file_name
						.chars()
						.next()
						.map_or(false, |c| {
							c.is_numeric()
...
///
/// This automatically derives all necessary imports and includes all generated
/// test modules.
pub fn generate_combined_test_file(
	...
	// Use the manifest_dir from the first generator (they should all be the
	// same)
	...
	// If target directory doesn't exist (cargo clean was run), remove the
	// generated test file
	let target_dir = Path::new(manifest_dir).join("target");
	if !target_dir.exists() && output_path.exists() {
		if let Err(e) = fs::remove_file(&output_path) {
			eprintln!(
				"Warning: Failed to clean generated test file: {}",
				e
			);
		}
	}
...
"#);
	...
	for generator in generators {
		for test_fn in &generator.test_functions {
			...
			if let Some(module_name) =
				TestGenerator::derive_module_name(test_fn)
			{
				test_fn_to_module
					.insert(test_fn.clone(), module_name);
			}
		}
	}
	...
	if !all_test_functions.is_empty() {
		...
		let mut sorted_functions: Vec<_> =
			all_test_functions.iter().collect();
		sorted_functions.sort();

		for test_fn in sorted_functions {
			if let Some(module_name) =
				test_fn_to_module.get(test_fn)
			{
				content.push_str(&format!(
					"pub use {}::{};\n",
					module_name, test_fn
	...
	if !source_modules.is_empty() {
		...
		// Sort for consistent output
		let mut sorted_modules: Vec<_> =
			source_modules.iter().collect();
		sorted_modules.sort();
