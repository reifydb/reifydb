...
fn an_extra_type_name_argument_reports_the_argument_count() {
	// A type name past the type slots reads as a column, so an argument-first check reports a missing column
	// instead of the wrong count.
	let t = engine();
...
fn an_extra_argument_naming_a_missing_column_reports_the_argument_count() {
	// The count is known before any row is read, so it must win over an error raised while evaluating the extra
	// argument.
	let t = engine();
