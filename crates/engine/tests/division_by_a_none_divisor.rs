...
#[test]
fn a_none_divisor_yields_none_for_its_row_in_every_saturation_mode() {
	// A none slot holds a zero placeholder, so dividing by it must give none, never a division by zero error.
	...
	t.command("INSERT test::e [{ id: 1, x: 10, y: 2 }, { id: 2, x: 10, y: none }]");
	t.command(
		"INSERT test::n [{ id: 1, x: 10, y: 2, q: 99 }, { id: 2, x: 10, y: none, q: 99 }, { id: 3, x: 10, y: 0, q: 99 }]",
	);
	...
	assert_eq!(
		...
		(r#"ok ["5", "none"]"#, r#"ok ["5", "none", "none"]"#),
		"the error mode row is the bug; the none mode zero row proves that mode is live"
	);
}
