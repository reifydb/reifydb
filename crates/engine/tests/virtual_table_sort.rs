...
fn test_sort_system_namespaces() {
	...
	let frames = engine.query_as(&identity, "FROM system.namespaces SORT name", Default::default()).unwrap();
...
fn test_sort_system_namespaces_asc() {
	...
	let frames = engine.query_as(&identity, "FROM system.namespaces SORT name ASC", Default::default()).unwrap();
...
fn test_sort_system_tables() {
	...
	let frames = engine.query_as(&identity, "FROM system.tables SORT name ASC", Default::default()).unwrap();
...
fn test_sort_system_tables_with_pipe_syntax() {
	...
	let frames = engine.query_as(&identity, "from system.tables | sort name", Default::default()).unwrap();
...
sort total_bytes asc";
	...
	let frames = engine.query_as(&identity, multiline_query, Default::default()).unwrap();
