...
pub struct NumericPool<T> {
    /// Small buffers (0-1024 elements)
    small: Mutex<Vec<Vec<T>>>,
    /// Medium buffers (1024-16384 elements)  
    medium: Mutex<Vec<Vec<T>>>,
    /// Large buffers (16384+ elements)
    ...
    T: Default + Clone + Send + Sync + 'static
{
    fn acquire(&self, capacity: usize) -> PooledBuffer<T> {
        ...
        if let Some(mut buffer) = self.get_from_bucket(capacity) {
            ...
            PooledBuffer::new(buffer, std::sync::Arc::new(NumericPoolWrapper::new(self)))
        } else {
...
impl<T> NumericPoolWrapper<T> {
    fn new(pool: &NumericPool<T>) -> Self {
        Self {
            pool_ptr: pool as *const NumericPool<T>,
            _phantom: PhantomData,
        }
    }
