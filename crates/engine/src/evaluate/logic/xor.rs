...
impl StandardEvaluator {
	pub(crate) fn xor(&self, ctx: &EvaluationContext, expr: &XorExpression) -> crate::Result<Column> {
		...
		match (&left.data(), &right.data()) {
			...
			(l, r) => {
				if l.is_number() || r.is_number() {
					return_error!(xor_can_not_applied_to_number(expr.full_fragment_owned()));
				} else if l.is_text() || r.is_text() {
					return_error!(xor_can_not_applied_to_text(expr.full_fragment_owned()));
				} else if l.is_temporal() || r.is_temporal() {
					return_error!(xor_can_not_applied_to_temporal(expr.full_fragment_owned()));
				} else {
					return_error!(xor_can_not_applied_to_uuid(expr.full_fragment_owned()));
				}
			}
		}
	}
