...
impl TemporalParser {
    ...
    pub fn parse_temporal(span: impl Span, row_count: usize) -> crate::Result<EngineColumnData> {
        ...
        if fragment.starts_with('P') || fragment.starts_with('p') {
            ...
            Ok(EngineColumnData::interval(vec![interval; row_count]))
            ...
            Ok(EngineColumnData::datetime(vec![datetime; row_count]))
            ...
            Ok(EngineColumnData::date(vec![date; row_count]))
        } else if fragment.contains(':') {
            ...
            Ok(EngineColumnData::time(vec![time; row_count]))
        } else {
