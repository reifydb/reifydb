...
use reifydb_core::diagnostic::{cast, number};
...
impl NumberParser {
    ...
    pub(crate) fn from_number(
        ...
        match target {
            ...
            _ => Err(Error(cast::unsupported_cast(
                span.to_owned(),
    ...
    fn parse_int1(span: impl Span, ty: Type, row_count: usize) -> evaluate::Result<ColumnValues> {
        if let Ok(v) = parse_int::<i8>(span.clone()) {
            ...
            match parse_int::<i8>(span.clone()) {
                Ok(_) => unreachable!(),
                Err(err) => Err(Error(cast::invalid_number(span.to_owned(), ty, err.diagnostic()))),
            }
        }
    }
