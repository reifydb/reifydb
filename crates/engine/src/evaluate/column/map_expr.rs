...
use reifydb_rql::expression::{Expression, MapExpression};
...
impl StandardColumnEvaluator {
	pub(super) fn map_expr(&self, ctx: &ColumnEvaluationContext, expr: &MapExpression) -> crate::Result<Column> {
		if expr.expressions.len() == 1 {
			return self.evaluate(ctx, &expr.expressions[0]);
		}
		let columns = self.map_expr_multi(ctx, expr)?;
		Ok(columns.into_iter().next().unwrap())
	}

	pub(super) fn map_expr_multi(
		&self,
