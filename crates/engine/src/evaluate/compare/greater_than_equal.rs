    ...
    <L as Promote<R>>::Output: PartialOrd,
{
    ...
    for i in 0..l.len() {
        if lv[i] && rv[i] {
            values.push(is_greater_than_equal(l[i], r[i]));
            valid.push(true);
        } else {
            values.push(false);
            valid.push(false);
