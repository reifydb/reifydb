...
use reifydb_core::frame::column::container::NumberContainer;
...
fn bool_to_number(
    ...
    match values {
        ColumnValues::Bool(container) => {
            ...
            let mut out = ColumnValues::with_capacity(target, container.len());
            for idx in 0..container.len() {
                if container.is_defined(idx) {
                    let val = container.values().get(idx);
...
fn float_to_integer(
    ...
    match values {
        ColumnValues::Float4(container) => match target {
        ...
        ColumnValues::Float8(container) => match target {
        ...
        ColumnValues::Utf8(container) => {
            ...
            for idx in 0..container.len() {
                if container.is_defined(idx) {
                    ...
                    match target {
                        Type::Int1 => {
                            parse_and_push!(parse_int, i8, Type::Int1, out, temp_span, base_span)
                        }
                        Type::Int2 => {
                            parse_and_push!(parse_int, i16, Type::Int2, out, temp_span, base_span)
                        }
                        Type::Int4 => {
                            parse_and_push!(parse_int, i32, Type::Int4, out, temp_span, base_span)
                        }
                        Type::Int8 => {
                            parse_and_push!(parse_int, i64, Type::Int8, out, temp_span, base_span)
                        }
                        Type::Int16 => {
                            parse_and_push!(parse_int, i128, Type::Int16, out, temp_span, base_span)
                        }
                        Type::Uint1 => {
                            parse_and_push!(parse_uint, u8, Type::Uint1, out, temp_span, base_span)
                        }
                        Type::Uint2 => {
                            parse_and_push!(parse_uint, u16, Type::Uint2, out, temp_span, base_span)
                        }
                        Type::Uint4 => {
                            parse_and_push!(parse_uint, u32, Type::Uint4, out, temp_span, base_span)
                        }
                        Type::Uint8 => {
                            parse_and_push!(parse_uint, u64, Type::Uint8, out, temp_span, base_span)
                        }
...
fn text_to_float(
    ...
    if let ColumnValues::Utf8(container) = column_values {
...
macro_rules! float_to_int_vec {
    ($fn_name:ident, $float_ty:ty, $int_ty:ty, $target_type:expr, $min_val:expr, $max_val:expr) => {
        fn $fn_name(container: &NumberContainer<$float_ty>) -> crate::Result<ColumnValues>
        where
...
fn number_to_number(
    ...
    macro_rules! cast {
            ...
            if let ColumnValues::$src_variant(container) = values {
                    match target {
                        $(
...
where
    ...
    To: GetType,
{
    let mut out = ColumnValues::with_capacity(target_kind, container.len());
    for idx in 0..container.len() {
        if container.is_defined(idx) {
            let val = container[idx];
...
mod tests {
    mod promote {
        ...
        fn test_ok() {
            ...
            let container = NumberContainer::new(values.to_vec(), bitvec);
