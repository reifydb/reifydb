...
impl StandardEvaluator {
	pub(crate) fn div(
		...
		match (&left.data(), &right.data()) {
			...
			// VarInt with other types
			(ColumnData::VarInt(l), ColumnData::VarInt(r)) => {
				div_numeric_clone(ctx, l, r, target, || {
					div.full_fragment_owned()
				})
			}
			...
			// Standard types with VarInt, VarUint, Decimal
	...
	for i in 0..l.len() {
		match (l.get(i), r.get(i)) {
			(Some(l_val), Some(r_val)) => {
				...
				if let Some(value) =
					ctx.div(&l_clone, &r_clone, fragment)?
				{
					data.push(value);
				} else {
					data.push_undefined()
