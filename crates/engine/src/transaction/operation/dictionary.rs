...
use reifydb_core::{
	interface::{CommandTransaction, DictionaryDef, EncodableKey, QueryTransaction},
...
pub(crate) trait DictionaryOperations {
	...
	async fn insert_into_dictionary(
		&mut self,
...
impl DictionaryOperations for StandardCommandTransaction {
	async fn insert_into_dictionary(
		...
		if let Some(existing) = self.get(&entry_key).await? {
		...
		let next_id = match self.get(&seq_key).await? {
		...
		self.set(&entry_key, EncodedValues(CowVec::new(entry_value))).await?;
		...
		self.set(&index_key, EncodedValues(CowVec::new(value_bytes))).await?;

		// 7. Update sequence
		self.set(&seq_key, EncodedValues(CowVec::new(next_id.to_be_bytes().to_vec()))).await?;
	...
	async fn find_in_dictionary(
		...
		let entry_key = DictionaryEntryKey::encoded(dictionary.id, hash);
		match self.get(&entry_key).await? {
			Some(v) => {
...
impl DictionaryOperations for crate::StandardTransaction<'_> {
	async fn insert_into_dictionary(
		...
		match self {
			crate::StandardTransaction::Command(cmd) => cmd.insert_into_dictionary(dictionary, value).await,
...
mod tests {
	...
	#[tokio::test]
	async fn test_insert_into_dictionary() {
		let mut txn = create_test_command_transaction().await;
		let dict = test_dictionary();
		let value = Value::Utf8("hello".to_string());

		let id = txn.insert_into_dictionary(&dict, &value).await.unwrap();
		assert_eq!(id, DictionaryEntryId::U8(1)); // First entry gets ID 1
	}

	#[tokio::test]
	async fn test_insert_duplicate_value() {
		let mut txn = create_test_command_transaction().await;
		let dict = test_dictionary();
		let value = Value::Utf8("hello".to_string());

		let id1 = txn.insert_into_dictionary(&dict, &value).await.unwrap();
		let id2 = txn.insert_into_dictionary(&dict, &value).await.unwrap();
	...
	async fn test_insert_multiple_values() {
		...
		let id1 = txn.insert_into_dictionary(&dict, &Value::Utf8("hello".to_string())).await.unwrap();
		let id2 = txn.insert_into_dictionary(&dict, &Value::Utf8("world".to_string())).await.unwrap();
		let id3 = txn.insert_into_dictionary(&dict, &Value::Utf8("foo".to_string())).await.unwrap();
	...
	#[tokio::test]
	async fn test_get_from_dictionary() {
		let mut txn = create_test_command_transaction().await;
		let dict = test_dictionary();
		let value = Value::Utf8("hello".to_string());

		let id = txn.insert_into_dictionary(&dict, &value).await.unwrap();
		let retrieved = txn.get_from_dictionary(&dict, id).await.unwrap();
	...
	async fn test_get_nonexistent_id() {
		...
		let retrieved = txn.get_from_dictionary(&dict, DictionaryEntryId::U8(999)).await.unwrap();
		assert_eq!(retrieved, None);
	}

	#[tokio::test]
	async fn test_find_in_dictionary() {
		let mut txn = create_test_command_transaction().await;
		let dict = test_dictionary();
		let value = Value::Utf8("hello".to_string());
		...
		let found = txn.find_in_dictionary(&dict, &value).await.unwrap();
		assert_eq!(found, Some(id));
	}

	#[tokio::test]
	async fn test_find_nonexistent_value() {
		let mut txn = create_test_command_transaction().await;
		let dict = test_dictionary();
	...
	#[tokio::test]
	async fn test_dictionary_with_uint1_id() {
		let mut txn = create_test_command_transaction().await;
		...
		let id = txn.insert_into_dictionary(&dict, &Value::Utf8("test".to_string())).await.unwrap();
		assert_eq!(id, DictionaryEntryId::U1(1));
		assert_eq!(id.id_type(), Type::Uint1);
	}

	#[tokio::test]
	async fn test_dictionary_with_uint2_id() {
		let mut txn = create_test_command_transaction().await;
		let dict = DictionaryDef {
			id: DictionaryId(3),
			namespace: NamespaceId(1),
	...
	#[tokio::test]
	async fn test_dictionary_with_uint4_id() {
		let mut txn = create_test_command_transaction().await;
		let dict = DictionaryDef {
			id: DictionaryId(4),
			namespace: NamespaceId(1),
