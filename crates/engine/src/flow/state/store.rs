...
impl StateStore {
    pub fn new() -> Self {
        Self { rows: HashMap::new(), indices: HashMap::new(), next_row_id: 0 }
    }
    ...
    pub fn update(&mut self, row_id: usize, new_row: Row) -> crate::Result<Option<Row>> {
    ...
    pub fn lookup_by_index(&self, index_name: &str, key: &IndexKey) -> Vec<&Row> {
        if let Some(index) = self.indices.get(index_name) {
            if let Some(row_ids) = index.lookup(key) {
                return row_ids.iter().filter_map(|&row_id| self.rows.get(&row_id)).collect();
            }
        }
        Vec::new()
    }
