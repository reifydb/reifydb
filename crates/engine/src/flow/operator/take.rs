...
fn encode_take_row(shape: &RowShape, columns: &Columns, row_idx: usize) -> EncodedBytes {
	...
	encoded.freeze().into_bytes()
}

fn decode_take_row(shape: &RowShape, row_number: RowNumber, encoded: &EncodedBytes) -> Columns {
...
impl TakeOperator {
	...
	fn load_take_state(&self, txn: &mut FlowTransaction) -> Result<TakeState> {
		...
		if state_row.is_empty() || !self.shape.is_defined(state_row.bytes(), 0) {
			return Ok(TakeState::default());
		}

		let blob = self.shape.get_blob(state_row.bytes(), 0);
		if blob.is_empty() {
			return Ok(TakeState::default());
		}
	...
	fn acquire_take_state(&self, txn: &mut FlowTransaction) -> Result<(TakeState, PersistFn)> {
		...
		txn.take_operator_state::<TakeState, _>(node_id, |txn| {
			...
			let persist: PersistFn = Box::new(move |txn, value| {
				...
				let mut row = utils::load_or_create_row(node_id, txn, &key, &shape)?.thaw();
				shape.set_blob(&mut row, 0, &blob);
				utils::save_row(node_id, txn, &key, row.freeze())?;
				Ok(())
			});
			Ok((s, persist))
		})
