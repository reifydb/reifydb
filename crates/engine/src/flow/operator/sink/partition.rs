...
static REGISTRY_SHAPE: LazyLock<RowShape> =
	LazyLock::new(|| RowShape::new(RowFamily::Operator, vec![RowShapeField::unconstrained("values", ValueType::Blob)]));
...
pub(crate) fn ensure_partition_unchanged(shape: ObjectId, pre: Partition, post: Partition) -> Result<()> {
	if pre != post {
...
pub(crate) fn resolve_partition_flow(
	...
	match txn.get(&key)? {
		Some(row) => {
			if REGISTRY_SHAPE.get_value(row.bytes(), 0) != candidate {
				return Err(PartitionError::PartitionHashCollision {
					object: shape,
					hash: partition.0,
				}
		...
		None => {
			let mut row = REGISTRY_SHAPE.allocate_operator();
