...
impl SinkRingBufferViewOperator {
	...
	fn read_metadata(&self, txn: &mut FlowTransaction) -> Result<RingBufferMetadata> {
		...
		match txn.get(&key)? {
			Some(row) => decode_ringbuffer_metadata(EncodedPodRow::view(row.bytes())),
			None => Ok(RingBufferMetadata::new()),
		}
	}
	...
	fn set_forward(&self, txn: &mut FlowTransaction, source_rn: RowNumber, storage_rn: RowNumber) -> Result<()> {
		let key = self.forward_key(source_rn);
		let mut row = self.state_shape.allocate_operator();
	...
	fn decode_row_number(&self, row: &EncodedOperatorRow, state: &'static str) -> Result<RowNumber> {
		let blob = self.state_shape.get_blob(row.bytes(), 0);
...
impl SinkRingBufferViewOperator {
	...
	fn build_evicted_diff(
		...
		let storage_columns: Vec<ColumnWithName> = view
			...
			.collect();
		let mut evicted = Columns::with_system(
			storage_columns,
			SystemColumns::new(Vec::new(), Vec::new(), Vec::new(), Vec::new(), Vec::new()),
		);
