...
impl DistinctOperator {
	...
	pub(super) fn with_stable_rn(cols: Columns, stable_rn: RowNumber) -> Columns {
		Columns::with_system(
			...
			SystemColumns::new(vec![stable_rn], Vec::new(), cols.created_at().to_vec(), cols.updated_at().to_vec(), cols.time().to_vec()),
		)
	}
	...
	pub(super) fn process_insert(
		...
		if !new_entries.is_empty() {
			...
			source_cols
					...
					.collect(),
			SystemColumns::new(stable_rns, Vec::new(), source_cols.created_at().to_vec(), source_cols.updated_at().to_vec(), source_cols.time().to_vec()),
		);
