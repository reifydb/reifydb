...
use crate::flow::{Operator, operator::OperatorCell, transaction::FlowTransaction};
...
impl ExtendOperator {
	...
	fn extend(&self, columns: &Columns) -> Result<Columns> {
		...
		Ok(Columns::with_system(
			result_columns,
			SystemColumns::new(row_numbers, Vec::new(), columns.created_at().to_vec(), columns.updated_at().to_vec(), columns.time().to_vec()),
		))
	}
}
