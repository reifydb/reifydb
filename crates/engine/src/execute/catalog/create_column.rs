...
impl<VS: VersionedStorage, US: UnversionedStorage> Executor<VS, US> {
    pub(crate) fn create_column(
        ...
        if let Some(column) = self.get_column_by_name(tx, table, &column_to_create.column)? {
            return Err(Error::execution(Diagnostic::column_already_exists(
                None,
...
mod test {
    ...
    fn test_create_column() {
        ...
        let column_1 = executor.get_column(&mut tx, ColumnId(1)).unwrap().unwrap();
        assert_eq!(column_1.id, 1);
        assert_eq!(column_1.name, "col_1");
        assert_eq!(column_1.value, ValueKind::Bool);

        let column_2 = executor.get_column(&mut tx, ColumnId(2)).unwrap().unwrap();
