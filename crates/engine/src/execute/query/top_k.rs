...
impl<'a> TopKNode<'a> {
	pub(crate) fn new(input: Box<ExecutionPlan<'a>>, by: Vec<SortKey>, limit: usize) -> Self {
		Self {
			input,
			by,
			limit,
...
impl<'a> QueryNode<'a> for TopKNode<'a> {
...
impl<'a> TopKNode<'a> {
	...
	fn sort_all(&self, columns: &mut Columns<'a>) -> crate::Result<Option<Batch<'a>>> {
		let key_refs: Vec<_> =
			self.by.iter()
				.map(|key| {
					let col = columns
