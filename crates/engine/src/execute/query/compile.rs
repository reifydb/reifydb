...
) -> Pin<Box<dyn Future<Output = ExecutionPlan> + Send + 'a>> {
	Box::pin(async move {
		match plan {
			...
			PhysicalPlan::IndexScan(node) => {
				...
				ExecutionPlan::IndexScan(
					IndexScanNode::new(table, IndexId::primary(pk.id), context).unwrap(),
				)
			}

			PhysicalPlan::TableScan(node) => ExecutionPlan::TableScan(
				TableScanNode::new(node.source.clone(), context, rx).await.unwrap(),
			),
			...
			PhysicalPlan::DictionaryScan(node) => ExecutionPlan::DictionaryScan(
				DictionaryScanNode::new(node.source.clone(), context).unwrap(),
			),
					...
					context.executor.virtual_table_registry.find_by_name(namespace.id, &table.name)
				{
					...
					match table.name.as_str() {
						...
						"operator_retention_policies" => {
							Box::new(OperatorRetentionPolicies::new())
						}
						...
						"table_storage_stats" => Box::new(TableStorageStats::new(
							context.executor.stats_tracker.clone(),
						)),
						"view_storage_stats" => Box::new(ViewStorageStats::new(
							context.executor.stats_tracker.clone(),
						)),
						"flow_storage_stats" => Box::new(FlowStorageStats::new(
							context.executor.stats_tracker.clone(),
						)),
						"flow_node_storage_stats" => Box::new(FlowNodeStorageStats::new(
							context.executor.stats_tracker.clone(),
						)),
						"index_storage_stats" => Box::new(IndexStorageStats::new(
							context.executor.stats_tracker.clone(),
						)),
			...
			PhysicalPlan::Variable(var_node) => {
				ExecutionPlan::Variable(VariableNode::new(var_node.variable_expr))
			}
