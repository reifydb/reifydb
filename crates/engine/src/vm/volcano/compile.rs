...
use reifydb_catalog::{
	...
	vtable::{
		...
		system::{
			...
			handlers::Handlers, identities::Identities, index_storage_stats::IndexStorageStats,
			migrations::Migrations, namespaces::Namespaces,
			...
			tables_virtual::TablesVirtual, tag_variants::TagVariants, tags::Tags, types::Types,
			user_roles::UserRoles, versions::Versions, view_storage_stats::ViewStorageStats, views::Views,
		...
		RqlQueryPlan::TableVirtualScan(node) => {
				...
				context.services.virtual_table_registry.find_by_name(namespace.id(), &table.name)
			{
				...
				match table.name.as_str() {
					...
					"identities" => VTables::Identities(Identities::new()),
