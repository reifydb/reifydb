...
pub(crate) fn value_to_u64(value: Value, key: &SeriesKey) -> Option<u64> {
	match value {
...
pub(crate) fn value_from_u64(v: u64, ty: Option<&Type>, key: &SeriesKey) -> Value {
	match ty {
...
pub(crate) fn column_data_from_u64_keys(keys: Vec<u64>, series_def: &SeriesDef, key: &SeriesKey) -> ColumnData {
	...
	match &key_type {
		Some(ty) => {
			...
			for k in keys {
				data.push_value(value_from_u64(k, key_type.as_ref(), key));
			}
			data
		}
