...
use transaction::{TransactionEngine, TransactionEngineMut};

pub struct Engine<'a, S: StorageEngineMut, T: TransactionEngine<'a, S>> {
    transaction: T,
    _marker: std::marker::PhantomData<(&'a (), S)>,
}

impl<'a, S: StorageEngineMut, T: TransactionEngine<'a, S>> Engine<'a, S, T> {
    pub fn new(transaction: T) -> Self {
        Self { transaction, _marker: Default::default() }
    }
}

impl<'a, S: StorageEngineMut, T: TransactionEngineMut<'a, S>> Engine<'a, S, T> {
