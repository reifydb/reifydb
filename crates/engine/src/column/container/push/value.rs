    ...
    pub fn push_value(&mut self, value: Value) {
        match value {
            Value::Bool(v) => match self {
                ColumnValues::Bool(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::bool(vec![]);
            ...
            Value::Float4(v) => match self {
                ColumnValues::Float4(_) => self.push(v.value()),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::float4(vec![]);
                    if let ColumnValues::Float4(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
            ...
            Value::Float8(v) => match self {
                ColumnValues::Float8(_) => self.push(v.value()),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::float8(vec![]);
                    if let ColumnValues::Float8(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v.value());
            ...
            Value::Int1(v) => match self {
                ColumnValues::Int1(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::int1(vec![]);
            ...
            Value::Int2(v) => match self {
                ColumnValues::Int2(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::int2(vec![]);
            ...
            Value::Int4(v) => match self {
                ColumnValues::Int4(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::int4(vec![]);
            ...
            Value::Int8(v) => match self {
                ColumnValues::Int8(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::int8(vec![]);
            ...
            Value::Int16(v) => match self {
                ColumnValues::Int16(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::int16(vec![]);
            ...
            Value::Utf8(v) => match self {
                ColumnValues::Utf8(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::utf8(Vec::<String>::new());
            ...
            Value::Uint1(v) => match self {
                ColumnValues::Uint1(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uint1(vec![]);
            ...
            Value::Uint2(v) => match self {
                ColumnValues::Uint2(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uint2(vec![]);
            ...
            Value::Uint4(v) => match self {
                ColumnValues::Uint4(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uint4(vec![]);
            ...
            Value::Uint8(v) => match self {
                ColumnValues::Uint8(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uint8(vec![]);
            ...
            Value::Uint16(v) => match self {
                ColumnValues::Uint16(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uint16(vec![]);
            ...
            Value::Date(v) => match self {
                ColumnValues::Date(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::date(vec![]);
                    if let ColumnValues::Date(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
            ...
            Value::DateTime(v) => match self {
                ColumnValues::DateTime(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::datetime(vec![]);
                    if let ColumnValues::DateTime(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
            ...
            Value::Time(v) => match self {
                ColumnValues::Time(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::time(vec![]);
                    if let ColumnValues::Time(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
            ...
            Value::Interval(v) => match self {
                ColumnValues::Interval(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::interval(vec![]);
                    if let ColumnValues::Interval(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
            ...
            Value::Uuid4(v) => match self {
                ColumnValues::Uuid4(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uuid4(vec![]);
            ...
            Value::Uuid7(v) => match self {
                ColumnValues::Uuid7(_) => self.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::uuid7(vec![]);
            ...
            Value::RowId(row_id) => match self {
                ColumnValues::RowId(container) => container.push(row_id),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::row_id(vec![]);
                    if let ColumnValues::RowId(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
            ...
            Value::Blob(v) => match self {
                ColumnValues::Blob(container) => container.push(v),
                ColumnValues::Undefined(container) => {
                    let mut new_container = ColumnValues::blob(vec![]);
                    if let ColumnValues::Blob(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
...
mod tests {
    ...
    fn test_date() {
        ...
        let ColumnValues::Date(container) = col else {
            panic!("Expected Date");
        };
    ...
    fn test_datetime() {
        ...
        let ColumnValues::DateTime(container) = col else {
    ...
    fn test_time() {
        ...
        let ColumnValues::Time(container) = col else {
            panic!("Expected Time");
        };
    ...
    fn test_interval() {
        ...
        let ColumnValues::Interval(container) = col else {
    ...
    fn test_row_id() {
        ...
        let ColumnValues::RowId(container) = col else {
    ...
    fn test_uuid4() {
        ...
        let ColumnValues::Uuid4(container) = col else {
    ...
    fn test_uuid7() {
        ...
        let ColumnValues::Uuid7(container) = col else {
