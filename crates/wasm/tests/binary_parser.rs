...
use reifydb_wasm::parse::{
    WasmData, WasmElement, WasmElementMode, WasmExport, WasmExportDescriptor, WasmFunc,
    WasmFunctionBody, WasmImport, WasmImportDescriptor, WasmInstruction, WasmMemory,
    WasmMemoryArgument, WasmParser, WasmResizableLimit, WasmResultType, WasmTable, WasmValueType,
};
...
fn parse_store() {
    ...
    assert_eq!(
        ...
        [WasmMemory {
            limits: WasmResizableLimit { min: 1, max: None }
        }]
    );
    assert_eq!(
        result.exports.as_ref(),
