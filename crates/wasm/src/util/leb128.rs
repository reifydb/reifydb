...
mod tests {
    ...
    fn i32_ok() {
        for (given, expected, expected_consumption) in [
            ...
            let (result, consumed) =
                i32::read_leb128(&given).expect(format!(" {:#04X?}", given).as_ref());
            assert_eq!(
                result, expected,
                "expected {} but got {} for {:#04X?}",
                expected, result, given
    ...
    fn u64_invalid() {
        for (given, expected) in [
            ...
            (vec![0x80], IncompleteEncoding), // missing bytes
            (
                vec![0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x80, 0x02],
                InvalidEncoding,
            ), // Too many bytes for a valid u64
            (vec![0x80, 0x80], IncompleteEncoding), // Missing continuation for multi-byte
        ] {
    ...
    fn i64_ok() {
        for (given, expected, expected_consumption) in [
            ...
            let (result, consumed) =
                i64::read_leb128(&given).expect(format!(" {:#04X?}", given).as_ref());
            assert_eq!(
                result, expected,
                "expected {} but got {} for {:#04X?}",
                expected, result, given
    ...
    fn i64_invalid() {
        for (given, expected) in [
            ...
            (
                ...
                InvalidEncoding,
            ), // Too many bytes for a valid i64
        ] {
