...
use crate::parse::WasmCustom;
use crate::parse::WasmData;
use crate::parse::WasmElement;
use crate::parse::WasmElementMode;
use crate::parse::WasmExport;
use crate::parse::WasmExportDescriptor;
use crate::parse::WasmFunc;
use crate::parse::WasmFunctionBody;
use crate::parse::WasmGlobal;
use crate::parse::WasmGlobalInit;
use crate::parse::WasmImport;
use crate::parse::WasmImportDescriptor;
use crate::parse::WasmInstruction;
use crate::parse::WasmMemory;
use crate::parse::WasmMemoryArgument;
use crate::parse::WasmModule;
use crate::parse::WasmParseError;
use crate::parse::WasmParseError::{InvalidMagicNumber, InvalidSectionCode, UnsupportedVersion};
use crate::parse::WasmResizableLimit;
use crate::parse::WasmResultType;
use crate::parse::WasmTable;
use crate::parse::WasmValueType;
use crate::parse::WasmParseError::InvalidValueType;
use crate::parse::WasmParseError::InvalidExportDescriptor;
...
fn parse_import_section(size: u32, reader: &ByteReader) -> Result<Box<[WasmImport]>> {
    ...
    for _ in 0..count {
        ...
        let desc = match import_kind {
            ...
            0x03 => Ok(WasmImportDescriptor::Global(parse_global(reader)?)),
            _ => Err(WasmParseError::InvalidImportDescriptor(import_kind)),
        }?;

        result.push(WasmImport { module, name, desc });
    }
    debug_assert_eq!(reader.pos(), expected_reader_pos);
    Ok(result.into())
}
...
fn parse_element(reader: &ByteReader) -> Result<WasmElement> {
    ...
    let mode = match flags {
        0x00 => WasmElementMode::Active {
            table: 0u32,
            offset: parse_instructions(reader)?,
        },
        0x01 => WasmElementMode::Passive,
        0x02 => WasmElementMode::Active {
            table: reader.read_leb128_u32()?,
            offset: parse_instructions(reader)?,
        },
        0x03 => WasmElementMode::Declarative,
        m => return Err(WasmParseError::InvalidElementMode(m)),
    };
...
fn parse_opcode(reader: &ByteReader) -> Result<Opcode> {
    ...
    let extension = reader.peek_u8().unwrap_or(0);
    let op = Opcode::from_u8(main, extension)?;

    if main == 0xFC || main == 0xFD {
        let _ = reader.read_u8()?;
    }
...
fn parse_instruction_of(opcode: Opcode, reader: &ByteReader) -> Result<WasmInstruction> {
    match opcode {
        ...
        Opcode::CallIndirect => {
            ...
            Ok(WasmInstruction::CallIndirect(
                function_type_index,
                table_index,
            ))
        }
...
fn parse_block_or_loop(op: Opcode, reader: &ByteReader) -> Result<WasmInstruction> {
    ...
    if op == Opcode::Block {
        Ok(WasmInstruction::Block { result_type, body })
    } else {
        Ok(WasmInstruction::Loop { result_type, body })
    }
}
...
fn parse_result_type(reader: &ByteReader) -> Result<WasmResultType> {
    ...
    Ok(match result_type {
        ...
        0x7F => WasmResultType::FromValue(
            WasmValueType::try_from(0x7F).map_err(|v| InvalidValueType(v))?,
        ),
        0x7E => WasmResultType::FromValue(
            WasmValueType::try_from(0x7E).map_err(|v| InvalidValueType(v))?,
        ),
        0x7D => WasmResultType::FromValue(
            WasmValueType::try_from(0x7D).map_err(|v| InvalidValueType(v))?,
        ),
        0x7C => WasmResultType::FromValue(
            WasmValueType::try_from(0x7C).map_err(|v| InvalidValueType(v))?,
        ),
        0x70 => WasmResultType::FromValue(
            WasmValueType::try_from(0x70).map_err(|v| InvalidValueType(v))?,
        ),
        0x6F => WasmResultType::FromValue(
            WasmValueType::try_from(0x6F).map_err(|v| InvalidValueType(v))?,
        ),
...
#[cfg(test)]
mod tests {
    use crate::parse::WasmParseError::{InvalidMagicNumber, UnexpectedEndOfFile, UnsupportedVersion};
    use crate::parse::binary::WasmParser;
    use crate::util::byte_reader::ByteReader;
