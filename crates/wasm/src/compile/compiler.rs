...
use crate::module::{
    Export, ExportData, Function, FunctionExternal, FunctionIndex, FunctionType, Global,
    GlobalIndex, Instruction, Memory, MemoryArgument, MemoryIndex, Module, ModuleId, Table,
    TableLimit, Value, ValueType, PAGE_SIZE,
};
use crate::parse::{
    WasmElementMode, WasmExportDescriptor, WasmFunc, WasmGlobalInit, WasmImportDescriptor,
    WasmInstruction, WasmMemoryArgument, WasmModule, WasmResultType, WasmValueType,
};
...
impl Compiler {
    ...
    pub fn compile(
                ...
                match &import.desc {
                    WasmImportDescriptor::Function(type_idx) => {
                        ...
                        let func = Function::External(FunctionExternal {
                            module: std::str::from_utf8(&*module_name)
                                .unwrap()
                                .to_string(),
                    ...
                    WasmImportDescriptor::Table(_table) => {
                        // FIXME
                    }
                    WasmImportDescriptor::Memory(_mem) => {
                        // FIXME
                    }
        ...
        for data in &wasm.data {
            let mem = memories.get_mut(data.index as usize).unwrap();
        ...
        for element in &wasm.elements {
            match &element.mode {
                ...
                WasmElementMode::Passive => todo!(),
                WasmElementMode::Declarative => todo!(),
            }
        }
        ...
        for global in &wasm.globals {
            let value = match &global.init {
                ...
                WasmGlobalInit::Global(_idx) => todo!(),
                WasmGlobalInit::NullRef(value_type) => {
                    Value::RefNull(convert_value_type(value_type))
                }
...
fn convert_instructions(
    ...
    instructions
        .iter()
        .for_each(|i| result.push(convert_instruction(i, module)));
...
pub fn convert_instruction(wasm_instruction: &WasmInstruction, module: &WasmModule) -> Instruction {
    match wasm_instruction {
        ...
        WasmInstruction::Block { result_type, body } => Instruction::Block {
            result_types: convert_result_types(module, result_type),
            body: body
                .iter()
                .map(|instr| convert_instruction(instr, module))
                .collect(),
        },
        WasmInstruction::Loop { result_type, body } => Instruction::Loop {
            result_types: convert_result_types(module, result_type),
            body: body
                .iter()
                .map(|instr| convert_instruction(instr, module))
        ...
        WasmInstruction::BrTable { cases, default } => Instruction::BrTable {
            cases: cases
                .iter()
                .map(|c| c.clone() as usize)
                .collect::<Vec<_>>()
                .into(),
