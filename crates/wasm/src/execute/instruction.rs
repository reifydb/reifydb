...
use crate::module::{
    BranchingDepth, Instruction, Trap, TrapDivisionByZero, TrapNotImplemented, TrapOverflow,
    Value, ValueType,
};

use crate::execute::exec::Exec;
...
impl ExecInstruction for Instruction {
    fn execute(&self, exec: &mut Exec) -> ExecResult {
        match self {
            ...
            Instruction::Block { result_types, body } => {
            ...
            Instruction::BrTable { cases, default } => {
            ...
            Instruction::Loop { result_types, body } => {
                let stack_pointer = exec.stack.pointer();
                loop {
                    match body.execute(exec)? {
                        ...
                        ExecStatus::Return => return Ok(ExecStatus::Return),
                        ExecStatus::Break(0) => {
                            exec.stack.revert(stack_pointer.clone(), result_types)?
                        }
                        ExecStatus::Break(depth) => return Ok(ExecStatus::Break(depth - 1)),
                    }
                }
            ...
            Instruction::If {
                ...
                let stack_pointer = exec.stack.pointer();
                let body = if cond != 0 { then } else { otherwise };
            ...
            Instruction::CallIndirect(type_idx, table_idx) => {
                exec.call_indirect(*type_idx, *table_idx)?
            }
            ...
            Instruction::MemoryGrow(_) => {
                ...
                let prev_pages = exec.memory_grow(pages as u32)?;
                exec.stack.push(prev_pages)?;
            }
...
impl IntegerExtension for u32 {
    fn div_checked(self, other: Self) -> Result<Self, Trap> {
        self.checked_div(other)
            .ok_or_else(|| Trap::DivisionByZero(TrapDivisionByZero::Integer))
    }
...
macro_rules! float_to_int {
    ($name:ident, $float:ty, $int:ty) => {
        pub fn $name(f: $float) -> Result<$int, Trap> {
            ...
            //       (In this case, of course, MIN - 1.0 != MIN.)
