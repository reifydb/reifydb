...
impl StackAccess for ExternalIndex {
	...
	fn peek(stack: &Stack) -> Result<Self> {
		...
		Ok(ExternalIndex(u32::from_le_bytes(stack.peek_bytes(size_of::<u32>())?)))
	}
}
...
impl StackAccess for Value {
	fn push(stack: &mut Stack, v: Self) -> Result<()> {
		match v {
			...
			Value::RefFunc(idx) => stack.push_bytes(&(idx as u32).to_le_bytes(), ValueType::RefFunc),
			Value::RefExtern(idx) => StackAccess::push(stack, idx),
			Value::RefNull(ref ty) => {
				// Push a sentinel value (0xFFFFFFFF) to mark null refs.
				// We use the underlying ref type to distinguish funcref null from externref null.
				let vt = ty.clone();
				stack.push_bytes(&0xFFFF_FFFFu32.to_le_bytes(), vt)
			}
		}
	}
	...
	fn pop(stack: &mut Stack) -> Result<Self> {
		match stack.peek_type()? {
			...
			ValueType::RefExtern => {
				let raw = u32::from_le_bytes(stack.peek_bytes(size_of::<u32>())?);
				stack.pop_bytes(size_of::<u32>())?;
				if raw == 0xFFFF_FFFF {
					Ok(Value::RefNull(ValueType::RefExtern))
				} else {
					Ok(Value::RefExtern(ExternalIndex(raw)))
				}
			}
			ValueType::RefFunc => {
				let raw = u32::from_le_bytes(stack.peek_bytes(size_of::<u32>())?);
				stack.pop_bytes(size_of::<u32>())?;
				if raw == 0xFFFF_FFFF {
					Ok(Value::RefNull(ValueType::RefFunc))
				} else {
					Ok(Value::RefFunc(raw as usize))
				}
			}
		}
	}
...
impl Stack {
	...
	where
		...
		T::Error: fmt::Debug,
	{
		Ok(self.bytes[self.bytes.len() - s..].try_into().map_err(|_| Trap::Underflow(TrapUnderflow::Stack))?)
	}
	...
	pub fn revert(&mut self, sp: StackPointer, result_types: impl AsRef<[ValueType]>) -> Result<()> {
...
mod tests {
	...
	fn type_mismatch_on_pop() {
		...
		assert_eq!(result, Err(Trap::Type(TrapType::MismatchValueType(ValueType::I64, ValueType::I32))));
	}
	...
	fn revert_with_multiple_result_types() {
		...
		ti.revert(ground.clone(), [ValueType::I32, ValueType::I32]).unwrap();
		assert_eq!(
			ti.pointer(),
			StackPointer {
				bp: 8,
	...
	fn revert_with_type_mismatch() {
		...
		assert!(result.is_err());
		assert_eq!(result.err().unwrap().to_string(), "expected type F64, got I32".to_string());
	}
}
