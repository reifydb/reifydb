...
use crate::ffi::wrapper::{OperatorWrapper, create_vtable};
use reifydb_operator_api::{FFIOperatorDescriptor, CURRENT_API_VERSION};
...
pub fn create_operator_instance<O: Operator>(config: *const u8, config_len: usize) -> *mut c_void {
    // Parse config
    ...
    // Create operator
    let operator = match create_and_initialize::<O>(config_slice) {
        Ok(op) => op,
...
fn create_and_initialize<O: Operator>(config: &[u8]) -> Result<O, crate::error::Error> {
...
fn create_temp_instance<O: Operator>() -> O {
    ...
    panic!(
        "Operator must implement Default trait or provide a no-arg constructor. \
