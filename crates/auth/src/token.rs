...
impl AuthenticationProvider for TokenProvider {
	...
	fn create(&self, _config: &HashMap<String, String>) -> reifydb_type::Result<HashMap<String, String>> {
	...
	fn validate(&self, stored: &HashMap<String, String>, credential: &str) -> reifydb_type::Result<bool> {
		let token =
			stored.get("token").ok_or_else(|| reifydb_type::error::Error::from(AuthError::MissingToken))?;
