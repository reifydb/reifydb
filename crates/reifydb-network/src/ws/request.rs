...
use reifydb_core::{
    ...
    value::{
        ...
        temporal::{parse_date, parse_datetime, parse_interval, parse_time},
    },
};
...
fn parse_typed_value(type_str: &str, value_val: &serde_json::Value) -> Result<Value, String> {
    ...
    let str_val = value_val.as_str()
        .ok_or_else(|| format!("expected string value for type {}", type_str))?;
    ...
    let parsed_value = match value_type {
        ...
        Type::Float8 => {
            parse_float::<f64>(span)
                .ok()
                .and_then(|f| OrderedF64::try_from(f).ok())
                .map(Value::Float8)
                .unwrap_or(Value::Undefined)
        }
        Type::Int1 => parse_int::<i8>(span).map(Value::Int1).unwrap_or(Value::Undefined),
        Type::Int2 => parse_int::<i16>(span).map(Value::Int2).unwrap_or(Value::Undefined),
        Type::Int4 => parse_int::<i32>(span).map(Value::Int4).unwrap_or(Value::Undefined),
        Type::Int8 => parse_int::<i64>(span).map(Value::Int8).unwrap_or(Value::Undefined),
        Type::Int16 => parse_int::<i128>(span).map(Value::Int16).unwrap_or(Value::Undefined),
        Type::Utf8 => Value::Utf8(str_val.to_string()),
        Type::Uint1 => parse_uint::<u8>(span).map(Value::Uint1).unwrap_or(Value::Undefined),
        Type::Uint2 => parse_uint::<u16>(span).map(Value::Uint2).unwrap_or(Value::Undefined),
        Type::Uint4 => parse_uint::<u32>(span).map(Value::Uint4).unwrap_or(Value::Undefined),
        Type::Uint8 => parse_uint::<u64>(span).map(Value::Uint8).unwrap_or(Value::Undefined),
        Type::Uint16 => parse_uint::<u128>(span).map(Value::Uint16).unwrap_or(Value::Undefined),
        Type::Date => {
            parse_date(span)
                .map(Value::Date)
                .unwrap_or(Value::Undefined)
        ...
        impl<'de> Visitor<'de> for WsParamsVisitor {
                ...
                A: de::SeqAccess<'de>,
            {
                ...
                while let Some(value) = seq.next_element::<serde_json::Value>()? {
                    // Check if it's in the {"type": "Bool", "value": "true"} format
                    ...
                    // Otherwise try to deserialize as a normal Value
...
mod tests {
    ...
    fn roundtrip(params: WsParams) {
        ...
        match (&params, &deserialized) {
            (WsParams::Positional(v1), WsParams::Positional(v2)) => {
                assert_eq!(v1.len(), v2.len());
                for (val1, val2) in v1.iter().zip(v2.iter()) {
                    match (val1, val2) {
                        (Value::Uuid4(_), Value::Uuid4(_)) => (), // UUIDs will be different
                        (Value::Uuid7(_), Value::Uuid7(_)) => (), // UUIDs will be different
                        (Value::DateTime(_), Value::DateTime(_)) => (), // DateTimes may be different
            ...
            (WsParams::Named(m1), WsParams::Named(m2)) => {
                ...
                for (k, v1) in m1.iter() {
                    let v2 = m2.get(k).expect("Key missing in deserialized map");
                    match (v1, v2) {
                        (Value::Uuid4(_), Value::Uuid4(_)) => (), // UUIDs will be different
                        (Value::Uuid7(_), Value::Uuid7(_)) => (), // UUIDs will be different
                        (Value::DateTime(_), Value::DateTime(_)) => (), // DateTimes may be different
    ...
    fn test_int1() {
        let params =
            WsParams::Positional(vec![Value::Int1(i8::MIN), Value::Int1(i8::MAX), Value::Int1(0)]);
        roundtrip(params);
    }
    ...
    #[test]
    fn test_datetime() {
        let params = WsParams::Positional(vec![Value::DateTime(DateTime::now())]);
        roundtrip(params);
    }
    ...
    #[test]
    fn test_uuid4() {
        let params = WsParams::Positional(vec![Value::Uuid4(Uuid4::generate())]);
        roundtrip(params);
    }

    #[test]
    fn test_uuid7() {
        let params = WsParams::Positional(vec![Value::Uuid7(Uuid7::generate())]);
        roundtrip(params);
    }
    ...
    fn test_full_request_deserialization() {
        ...
        match request.payload {
            RequestPayload::Command(cmd) => {
                assert_eq!(cmd.statements, vec!["MAP $1 as result"]);
                match cmd.params {
                    Some(WsParams::Positional(values)) => {
                        assert_eq!(values.len(), 1);
                        assert_eq!(values[0], Value::Bool(true));
    ...
    fn test_client_format_named_params() {
        ...
        match params {
            WsParams::Named(map) => {
                ...
                assert_eq!(map.get("active"), Some(&Value::Bool(true)));
                match map.get("score") {
                    Some(Value::Float8(f)) => assert!((**f - 98.5).abs() < 0.01),
                    _ => panic!("Expected Float8 for score"),
                }
            }
            _ => panic!("Expected named parameters"),
        }
    ...
    fn test_mixed_format_params() {
        // Test that we can handle both old and new formats in the same array
        let json = r#"[
        ...
        match params {
            WsParams::Positional(values) => {
                ...
                assert_eq!(values[2], Value::Utf8("hello".to_string())); // New format
