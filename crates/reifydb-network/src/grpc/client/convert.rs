...
use reifydb_core::{
	BitVec, Date, DateTime, FrameColumnData, Interval, OwnedFragment, RowId,
	StatementColumn, StatementLine, Time, Type,
	interface::Params,
	result::{
		Frame, FrameColumn,
...
pub(crate) fn convert_diagnostic(grpc: grpc::Diagnostic) -> Diagnostic {
	Diagnostic {
		...
		fragment: grpc.span.map(|s| OwnedFragment::Statement {
