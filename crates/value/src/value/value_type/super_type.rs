...
impl ValueType {
	pub fn super_type_of<I>(iter: I) -> ValueType
	where
		I: IntoIterator<Item = ValueType>,
	{
		let mut iter = iter.into_iter();
		match iter.next() {
			Some(first) => iter.fold(first, ValueType::promote),
			None => ValueType::Any,
		}
	}
}
...
mod tests {
	...
	fn empty_input_returns_any() {
		assert_eq!(ValueType::super_type_of(Vec::<ValueType>::new()), Any);
	}
	...
	fn single_element_passes_through_for_all_representative_types() {
		...
		for ty in cases {
			let input = [ty.clone()];
			let got = ValueType::super_type_of(input.iter().cloned());
	...
	#[test]
	fn pair_grid() {
		let cases: &[(ValueType, ValueType, ValueType)] = &[
			(Int1, Int1, Int2),
			(Int2, Int2, Int4),
			(Int4, Int4, Int8),
			(Int8, Int8, Int16),
		...
		for (l, r, expected) in cases {
			let got = ValueType::super_type_of([l.clone(), r.clone()]);
	...
	#[test]
	fn triple_fold() {
		let cases: &[(ValueType, ValueType, ValueType, ValueType)] = &[
			(Int1, Int1, Int1, Int4),
			(Int1, Int2, Int4, Int8),
		...
		for (a, b, c, expected) in cases {
			let got = ValueType::super_type_of([a.clone(), b.clone(), c.clone()]);
			assert_eq!(
				got, *expected,
				"triple [{:?}, {:?}, {:?}] expected {:?} got {:?}",
				a, b, c, expected, got
	...
	fn accepts_vec_input() {
		let input: Vec<ValueType> = vec![Int1, Int1];
		assert_eq!(ValueType::super_type_of(input), Int2);
	}

	#[test]
	fn accepts_slice_iter_cloned_input() {
		let input: &[ValueType] = &[Int1, Int1];
		assert_eq!(ValueType::super_type_of(input.iter().cloned()), Int2);
	}

	#[test]
	fn signed_progressive_widening() {
		let cases: &[(Vec<ValueType>, ValueType)] = &[
	...
	fn utf8_dominates_boolean() {
		assert_eq!(ValueType::super_type_of([Boolean, Utf8, Int4]), Utf8);
		assert_eq!(ValueType::super_type_of([Int4, Boolean, Utf8]), Utf8);
		assert_eq!(ValueType::super_type_of([Utf8, Boolean, Boolean]), Utf8);
	}

	#[test]
	fn option_on_left_short_circuits() {
		assert_eq!(ValueType::super_type_of([opt(Int4), Int1]), opt(Int4));
		assert_eq!(ValueType::super_type_of([opt(Int4), Int16]), opt(Int4));
		assert_eq!(ValueType::super_type_of([opt(Utf8), Boolean]), opt(Utf8));
		assert_eq!(ValueType::super_type_of([opt(opt(Int4)), Int1]), opt(opt(Int4)));
		assert_eq!(ValueType::super_type_of([opt(Int4), Int1, Int2]), opt(Int4));
	}

	#[test]
	fn option_on_right_short_circuits_to_left() {
		assert_eq!(ValueType::super_type_of([Int1, opt(Int4)]), Int1);
		assert_eq!(ValueType::super_type_of([Int16, opt(Int4)]), Int16);
		assert_eq!(ValueType::super_type_of([Boolean, opt(Utf8)]), Boolean);
		assert_eq!(ValueType::super_type_of([Utf8, opt(Int4)]), Utf8);
	}
	...
	fn non_numeric_pass_through() {
		...
		for ty in cases {
			...
			let got3 = ValueType::super_type_of([ty.clone(), ty.clone(), ty.clone()]);
