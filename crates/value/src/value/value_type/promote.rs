...
impl ValueType {
	pub fn promote(left: ValueType, right: ValueType) -> ValueType {
		...
		let is_signed = |k: &ValueType| signed_order.contains(k);
		let is_unsigned = |k: &ValueType| unsigned_order.contains(k);

		let rank = |k: &ValueType| match k {
			Int1 | Uint1 => 0,
			Int2 | Uint2 => 1,
			Int4 | Uint4 => 2,
			Int8 | Uint8 => 3,
...
pub mod tests {
	...
	fn test_promote_string() {
		...
		for ty in kinds {
			assert_eq!(ValueType::promote(Utf8, ty), Utf8);
		}
	}
	...
	fn test_promote_any_is_absorbing() {
		...
		for ty in kinds {
			assert_eq!(ValueType::promote(Any, ty.clone()), Any, "Any on left with {:?}", ty);
			assert_eq!(ValueType::promote(ty.clone(), Any), Any, "Any on right with {:?}", ty);
		}
		assert_eq!(ValueType::promote(Any, Option(Box::new(Int4))), Any);
		assert_eq!(ValueType::promote(Option(Box::new(Int4)), Any), Any);
	}
