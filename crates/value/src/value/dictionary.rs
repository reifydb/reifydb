...
impl DictionaryEntryId {
	pub fn from_u128(value: u128, id_type: ValueType) -> Result<Self> {
		match id_type {
			ValueType::Uint1 => {
			...
			ValueType::Uint16 => Ok(Self::U16(value)),
	...
	pub fn id_type(&self) -> ValueType {
		match self {
			Self::U1(_) => ValueType::Uint1,
			Self::U2(_) => ValueType::Uint2,
			Self::U4(_) => ValueType::Uint4,
			Self::U8(_) => ValueType::Uint8,
			Self::U16(_) => ValueType::Uint16,
		}
	}
...
pub mod tests {
	...
	fn test_from_u128_u1() {
		let id = DictionaryEntryId::from_u128(42, ValueType::Uint1).unwrap();
	...
	fn test_from_u128_u1_overflow() {
		let err = DictionaryEntryId::from_u128(256, ValueType::Uint1).unwrap_err();
		assert!(err.to_string().contains("DICT_001"));
	}

	#[test]
	fn test_from_u128_u2() {
		let id = DictionaryEntryId::from_u128(1000, ValueType::Uint2).unwrap();
	...
	fn test_from_u128_u4() {
		let id = DictionaryEntryId::from_u128(100_000, ValueType::Uint4).unwrap();
	...
	fn test_from_u128_u8() {
		let id = DictionaryEntryId::from_u128(10_000_000_000, ValueType::Uint8).unwrap();
	...
	fn test_from_u128_u16() {
		...
		let id = DictionaryEntryId::from_u128(large_value, ValueType::Uint16).unwrap();
