		...
		Err(err) => match err.kind() {
			...
			IntErrorKind::InvalidDigit => {
				if let Ok(f) = value.parse::<f64>() {
					...
					let type_enum = T::type_enum();
					let in_range = match type_enum {
						ValueType::Uint1 => truncated >= 0.0 && truncated <= u8::MAX as f64,
						ValueType::Uint2 => truncated >= 0.0 && truncated <= u16::MAX as f64,
						ValueType::Uint4 => truncated >= 0.0 && truncated <= u32::MAX as f64,
						ValueType::Uint8 => truncated >= 0.0 && truncated <= u64::MAX as f64,
						ValueType::Uint16 => truncated >= 0.0 && truncated <= u128::MAX as f64,
						_ => false,
					};
					if in_range {
						Ok(cast_float_to_int::<T>(truncated))
