...
use crate::{
	tier::{bound::parts, persistent::OperatorPersistentTier, resident::state_hash},
...
impl OperatorStateKeySource {
	pub fn new(persistent: OperatorPersistentTier) -> Self {
		Self {
			persistent,
			pending: Vec::new(),
			cursor: None,
	...
	fn occupied(&self) -> Vec<OperatorStateCensus> {
		self.persistent.census().into_iter().filter(|entry| entry.keys > 0).collect()
	}
}
