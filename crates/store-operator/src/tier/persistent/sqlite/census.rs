...
use crate::{
	tier::{
		persistent::sqlite::{
			...
			sql::{
				CENSUS_APPLY_SQL, CENSUS_ZERO_OPERATOR_SQL, JOIN_EXPIRY_CENSUS_SQL,
				JOIN_EXPIRY_COUNT_SQL, JOIN_EXPIRY_TOTAL_COUNT_SQL, STATE_BYTES_SQL, STATE_CENSUS_SQL,
...
pub(super) fn flush_delta(batch: &FlushBatch) -> CensusDelta {
	...
	for ((operator, key), entry) in &batch.state {
		delta.record(operator, key, entry.durable_pre, entry.post.as_ref().map(|row| row.bytes().len() as u64));
	}
	delta
}
