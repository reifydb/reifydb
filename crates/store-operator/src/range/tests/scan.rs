...
use reifydb_core::{
	...
	key::{
		...
		typed::{BoundedKey, Edge, direction::Asc, range::KeyRange},
	},
};
use reifydb_store::{
	coverage::{
...
fn whole() -> Interval<Suffix> {
	Interval::new(Edge::Key(Suffix::low()), Edge::Top)
}

fn spanning(from: Suffix, to: Suffix) -> Interval<Suffix> {
	Interval::new(Edge::Key(from), Edge::Key(to))
}
...
fn a_plan_tiles_the_range_with_a_head_gap_a_tail_gap_and_no_merge_across_a_real_gap() {
	...
	assert_eq!(
		...
		[
			Segment::Gap {
				interval: Interval::new(Edge::Key(Suffix::low()), Edge::Key(at(3))),
				exempt: false,
			},
			...
			Segment::Gap {
				interval: Interval::new(Edge::Key(at(8)), Edge::Top),
				exempt: false,
			},
		],
...
fn a_materialize_lands_every_row_of_the_span_it_was_given() {
	// a partial materialize would leave uncached keys inside the span reading back as proven absences
	let tiers = roomy();
	let head = at(1);
	let tail = at(9_000_000);
	let span = Interval::new(Edge::Key(head), Edge::Top);
...
fn a_materialize_that_proves_an_empty_span_claims_it_without_paying_for_a_partition() {
	// charging a partition for a proven-empty span would let empty spans alone exhaust the eviction budget
	let tiers = roomy();
