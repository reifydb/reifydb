...
use crate::{
	persistent::sqlite::SqlitePersistent as OperatorStore,
	types::{FlushBatch, StagedWrite},
};
...
fn state_batch(writes: &[(OperatorId, GroupId, u64, Option<EncodedPodRow>)]) -> FlushBatch {
	...
	for (operator, group, row_num, post) in writes {
		let key = GroupStateKey::new(*group, KeyspaceId::JOIN_LEFT, Asc(RowNumber(*row_num)).to_suffix_bytes());
		let write = match post {
			Some(row) => StagedWrite::Set(row.clone()),
