...
fn rolling_sum_accumulates_correctly_across_separate_commits() {
	...
	admin(&db, "CREATE TABLE app::t { g: int4, v: float8, ts: datetime }");
	admin(
		&db,
		r#"CREATE DEFERRED VIEW app::r { g: int4, total: float8 } AS {
			FROM app::t
	...
	let insert = |g: i32, v: f64, ts: &str| {
		db.command_as_root(&format!("INSERT app::t [{{ g: {g}, v: {v}, ts: \"{ts}\" }}]"), Params::None)
			.expect("insert");
	};
	let await_total = |g: i32, total: f64| {
		let rql = format!("FROM app::r | filter {{ g == {g} and total == {total} }}");
		let got = await_row_count(&db, &rql, 1, StdDuration::from_secs(5));
		assert_eq!(
			got,
			1,
			"group {g} must roll up to {total} from state carried across commits; view now: {:?}",
			db.query_as_root("FROM app::r", Params::None)
		);
	};
	...
	let rows = row_count(&db, "FROM app::r");
