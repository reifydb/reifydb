...
use reifydb_test_harness::operator::transaction::{FlowTxn, OPERATOR_ID, engine, key, make_bytes};
...
fn assert_zero_prior_anchor_is_not_pinned(txn: &mut FlowTransaction) {
	...
	txn.state_set(OPERATOR_ID, &k, make_bytes("v0", 0, 0)).unwrap();
	txn.state_set(OPERATOR_ID, &k, make_bytes("v1", 7_000, 7_000)).unwrap();
