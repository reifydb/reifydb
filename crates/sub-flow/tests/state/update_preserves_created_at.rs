...
use reifydb_test_harness::operator::transaction::{FlowTxn, OPERATOR_ID, engine, key, make_row, payload};
...
fn assert_update_uses_caller_anchors(txn: &mut FlowTransaction) {
	...
	txn.state_set(OPERATOR_ID, &k, make_row("v1", 1_000, 1_000)).unwrap();
	txn.state_set(OPERATOR_ID, &k, make_row("v2", 5_000, 5_000)).unwrap();
