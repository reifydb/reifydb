...
// The instant to fire a seal at, which is what bounds the data phase. Nothing an operator has not
// sealed is reclaimable, so a sweep against an unsealed operator correctly does nothing at all - the
// watermark it is swept at cannot substitute.
//
// A seal fired at T only proves windows anchored at or before `T - span - 1` are closed, so the
// ledger has to clear a whole grid bucket past the group's own before its data comes due: every
// arrival in this suite lands in bucket zero, so the anchor must reach one full grid width.
...
// The identity phase is the one cutoff still measured from the sweep watermark, because it belongs
// to the sink rather than to the operator: a mapping has to outlive the published row naming it, and
// that row lives exactly the sink's row ttl. With the ttl set equal to the span, this clears a
// group in bucket zero.
...
fn the_identity_phase_reaches_a_group_only_on_a_sweep_after_the_data_phase() {
	...
	//
	// The ordering is STRUCTURAL, not arithmetic. It used to be bought by cutting identity one grid
	// width behind data, which meant a mis-tuned slack could silently collapse the split. It is now
	// enforced by `reclaim_identity` only offering groups the data phase has already finished and
	// marked, so identity cannot reach a group on the same sweep that reclaims its data whatever the
	// two cutoffs say. Both halves stay load-bearing: a sweep that took identity together with data
	// would collapse the split, and one that never took identity would leave every mapping in the
...
fn a_sweep_before_the_seal_clears_the_bucket_is_a_no_op_in_every_observable_way() {
	// The control that gives the four tests above their meaning. Same corpus, same calls, a seal one
