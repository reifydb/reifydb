...
fn sliding_window(db: &TestDb, size: &str, slide: &str, lateness: &str) {
	...
	db.admin(&format!(r#"CREATE DEFERRED VIEW app::w {{ g: int4, total: int8 }} AS {{
				FROM app::t
					| window sliding {{ total: math::sum(v) }}
						with {{ duration: {size}, slide: {slide}, lateness: {lateness} }}
						by {{ g }}
			}}"#));
}
...
fn a_row_lands_in_every_window_that_covers_it() {
	...
	// 60s cover t=70s while the one starting at 0s ends before it. The 120s lateness is load-bearing -
	// every wrongly-covering window is an older one, and at zero lateness it has already sealed.
	let db = setup();
...
#[test]
fn a_sliding_window_seals_size_plus_lateness_after_its_start() {
	// Each window's seal horizon is its own start plus size plus lateness, so overlapping windows
