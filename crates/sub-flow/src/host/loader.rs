    ...
    pub fn load_operator(
        ...
        let descriptor = unsafe {
            let get_descriptor: Symbol<extern "C" fn() -> *const FFIOperatorDescriptor> =
                library
                    .get(b"ffi_operator_get_descriptor\0")
                    .map_err(|e| {
                        FFIError::Other(format!(
                            "Failed to find ffi_operator_get_descriptor: {}",
                            e
                        ))
                    })?;
        ...
        let create_fn: FFIOperatorCreateFn = unsafe {
            let create_symbol: Symbol<FFIOperatorCreateFn> =
                library.get(b"ffi_operator_create\0").map_err(|e| {
                    FFIError::Other(format!("Failed to find ffi_operator_create: {}", e))
                })?;
        ...
        if instance.is_null() {
            return Err(FFIError::Other(
                "Failed to create operator instance".to_string(),
            ));
        }
...
mod tests {
    ...
    fn test_loader_creation() {
        let loader = FFIOperatorLoader::new();
