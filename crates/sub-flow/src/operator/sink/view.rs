...
mod tests {
	...
	fn update_preserves_created_at_from_the_operator_cache_and_falls_back_after_rebuild() {
		...
		assert_eq!(stored.created_at(), DateTime::from_nanos(1_000), "created_at must survive the cached update");
		...
		assert_eq!(stored.created_at(), DateTime::from_nanos(1_000), "created_at must survive the fallback path too");
