...
type OperatorFactoryFn = Box<dyn Fn(FlowNodeId, &[Expression]) -> reifydb_type::Result<BoxedOperator> + Send + Sync>;
...
impl TransformOperatorRegistry {
	...
	where
		F: Fn(FlowNodeId, &[Expression]) -> reifydb_type::Result<BoxedOperator> + Send + Sync + 'static,
	{
