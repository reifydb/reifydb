...
pub fn random_sub_range(rng: &mut StdRng, keyspace: u64) -> (EncodedKeyRange, RangeFilter) {
	...
	let storage_lo = RowKey::storage_start(STORAGE).to_vec();
	let storage_hi = RowKey::storage_end(STORAGE).to_vec();
	let start = match rng.random_range(0u32..3) {
		0 => Bound::Included(lo),
		1 => Bound::Excluded(lo),
		_ => Bound::Included(storage_lo),
	};
	let end = match rng.random_range(0u32..3) {
		0 => Bound::Included(hi),
		1 => Bound::Excluded(hi),
		_ => Bound::Included(storage_hi),
	};
