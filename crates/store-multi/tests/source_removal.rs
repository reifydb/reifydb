...
fn source_removal_hides_the_key_at_or_above_its_version_only() {
	...
	let keys = range_keys(&store, RowKey::full_scan(shape), 5);
...
fn partitioned_source_removal_hides_only_the_removed_partition_row() {
	...
	assert!(
		range_keys(&store, PartitionedRowKey::partition_range(shape, us), 9).is_empty(),
		"the removed partition row must not surface above the tombstone"
	);
	assert!(
		range_keys(&store, PartitionedRowKey::partition_range(shape, eu), 9).contains(&k_eu),
		"the sibling partition must be unaffected"
	);
	assert!(
		range_keys(&store, PartitionedRowKey::partition_range(shape, us), 5).contains(&k_us),
...
fn memory_only_source_removal_keeps_every_version_below_the_tombstone() {
	...
	assert!(range_keys(&store, RowKey::full_scan(shape), 2).contains(&k));
}
