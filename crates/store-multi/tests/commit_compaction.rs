...
test_each_path! { in "crates/store-multi/tests/scripts/buffer/compaction" as buffer_drop_memory => test_memory }
		...
		match command.name.as_str() {
			...
			"compact" => {
				...
				if !all_versions.is_empty() {
					...
					self.storage.compact(HashMap::from([(table, versions_to_compact)]))?;
				}
				writeln!(output, "ok")?;
			}
