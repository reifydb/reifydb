...
use reifydb_core::{common::CommitVersion, runtime::compute::ComputePool};
...
impl MemoryPrimitiveStorage {
	...
	fn process_table(&self, table: EntryKind, version: CommitVersion, entries: Vec<(CowVec<u8>, Option<CowVec<u8>>)>) {
		...
		for (key, value) in entries {
			...
			if let Some((old_version, old_value)) = current.get(&key) {
				if *old_version < version {
					...
					historical.entry(key.clone()).or_default().insert(Reverse(old_version), old_value);
...
impl TierStorage for MemoryPrimitiveStorage {
	...
	fn clear_table(&self, table: EntryKind) -> Result<()> {
		...
		if let Some(entry) = self.inner.entries.data.get(&table_key) {
			*entry.value().current.write() = CurrentMap::new();
			*entry.value().historical.write() = HistoricalMap::new();
		}
		Ok(())
	}
		...
		for (table, entries) in batches {
			...
			for (key, version) in entries {
				...
				if let Some((cur_version, _)) = current.get(&key) {
					if *cur_version == version {
						// Dropping current version - remove from current and all historical versions
						current.remove(&key);
...
pub mod tests {
	...
	fn test_tombstones() {
		...
		storage.set(CommitVersion(1), HashMap::from([(
			EntryKind::Multi,
			vec![(key.clone(), Some(CowVec::new(b"value".to_vec())))],
		)]))
		.unwrap();
