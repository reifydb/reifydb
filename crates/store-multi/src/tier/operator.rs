...
pub fn scan_operator_expired(
	...
	for entry in &batch.entries {
		if entry.version <= cutoff_version {
			expired.push(ExpiredOperatorState {
				node_id,
				key: entry.key.clone(),
				version: entry.version,
				scanned_bytes: entry.value.as_ref().map_or(0, |value| value.len() as u64),
			});
		}
	}
...
pub fn compact_expired_operator_keys(
	storage: &MultiCommitBufferTier,
	...
	let mut compaction_batches: HashMap<EntryKind, Vec<(EncodedKey, CommitVersion)>> = HashMap::new();

	for row in expired {
		let table = EntryKind::Operator(row.node_id);
		let node_bytes = stats.bytes_reclaimed.entry(row.node_id).or_insert(0);
		let compaction_batch = compaction_batches.entry(table).or_default();
		...
		for (version, value) in &versions {
			...
			compaction_batch.push((row.key.clone(), *version));
			stats.versions_dropped += 1;
		}
	}

	if !compaction_batches.is_empty() {
		storage.compact(compaction_batches)?;
	}
...
mod tests {
	...
	fn ttl_drop_never_reclaims_a_version_written_after_the_scan() {
		...
		compact_expired_operator_keys(&storage, &expired, &mut stats).unwrap();
	...
	/// A silently-removed operator key leaves a tombstone as its current version. The scan reads at
	/// `AsOf { read: u64::MAX }`, so that tombstone is what it sees - and while it guarded on
	/// `entry.value` being present, such a key was invisible to operator TTL forever: neither the
	/// tombstone nor any value version beneath it could ever be reclaimed. Under the retired Drop
	/// model this could not arise, because a removal erased the key outright rather than tombstoning
	/// it, so the guard was harmless until the deletion unification made tombstones the norm.
	#[test]
	fn ttl_reclaims_a_key_whose_current_version_is_a_tombstone() {
		let storage = MultiCommitBufferTier::memory();
		let node = FlowNodeId(11);
		let table = EntryKind::Operator(node);
		let key = FlowNodeStateKey::encoded(node, vec![3u8]);

		storage.set(CommitVersion(1), HashMap::from([(table, vec![(key.clone(), Some(row(b"v1")))])])).unwrap();
		storage.set(CommitVersion(2), HashMap::from([(table, vec![(key.clone(), Some(row(b"v2")))])])).unwrap();
		// The silent removal: a tombstone at v3 becomes the key's current version.
		storage.set(CommitVersion(3), HashMap::from([(table, vec![(key.clone(), None)])])).unwrap();

		let mut cursor = RangeCursor::default();
		let (expired, _) = scan_operator_expired(&storage, node, CommitVersion(5), 4096, &mut cursor).unwrap();
		assert_eq!(
			expired.len(),
			1,
			"a tombstone at or below the cutoff is the key's current version and must be selected; \
			 skipping it strands the tombstone and every value version beneath it in the buffer forever"
		);
		...
		assert!(
			storage.get_all_versions(table, key.as_ref()).unwrap().is_empty(),
			"reclaiming an expired tombstone must take the whole key with it, values included"
		);
		assert_eq!(
			stats.versions_dropped, 3,
			"all three versions sat at or below the tombstone and must be accounted for"
		);
	}
