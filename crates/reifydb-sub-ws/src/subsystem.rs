		...
		if let Some(server) = self.server.take() {
			...
			let handle = if let Some(rt) = &runtime {
				rt.spawn(async move {
					...
					println!(
						"[WsSubsystem] Starting WebSocket server"
					);
					...
					println!(
						"[WsSubsystem] WebSocket server stopped"
					);
				})
			} else {
				tokio::spawn(async move {
	...
	fn shutdown(&mut self) -> Result<()> {
		...
		if let Some(server) = &self.server {
				...
				tokio::runtime::Handle::try_current()
			{
				...
				// No runtime available, just log
				println!(
					"[WsSubsystem] Unable to gracefully close WebSocket server"
				);
			};
		}
