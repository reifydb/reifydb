...
impl ConsoleBackend {
	...
	fn format_timeline_records(&self, records: &[Record]) -> Vec<String> {
		...
		for record in records {
			...
			// Check if this is a new module or we need to flush the
			// current group
	...
	fn format_timeline_group(
		...
		let time_str = if let Some(last_ms) = *last_timestamp_ms {
			...
			if timestamp_ms != last_ms {
				...
				// Same timestamp - just show the connector
				// aligned (12 chars for timestamp)
				"             ├─".to_string()
			}
		} else {
		...
		*last_timestamp_ms = Some(timestamp_ms);
		...
		// Start the group header - don't colorize timestamp or
		...
		match self.format_style {
			FormatStyle::Timeline => {
				...
				for formatted in formatted_groups {
					...
					// should go to stderr
					// For simplicity, we'll send all to
					// stdout for now
