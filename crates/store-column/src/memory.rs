...
impl MemoryColumnStore {
	...
	pub fn compress_columns(&self, columns: &Columns) -> Result<Vec<CompressedColumn>> {
	...
	/// Decompress columns
	pub fn decompress_columns(&self, compressed: &[CompressedColumn]) -> Result<Vec<ColumnData>> {
		// compressed
		// 	.iter()
		// 	.map(|col| {
