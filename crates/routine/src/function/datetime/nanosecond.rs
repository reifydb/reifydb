...
pub struct DateTimeNanosecond {
	info: FunctionInfo,
}

impl Default for DateTimeNanosecond {
	fn default() -> Self {
		Self::new()
	}
}

impl DateTimeNanosecond {
	pub fn new() -> Self {
		Self {
			info: FunctionInfo::new("datetime::nanosecond"),
		}
	}
}
			...
			ColumnBuffer::DateTime(container) => {
				...
				for i in 0..row_count {
					if let Some(dt) = container.get(i) {
						result.push(dt.nanosecond() as i32);
