...
fn flatten(ast: &Ast<'_>, depth: usize, rows: &mut Vec<(String, String)>) {
	...
	match ast {
		Ast::For(AstFor {
			...
			rows.push((
				"For".to_string(),
				format!("{}{} in {}", indent, variable.token.value(), render_let_value(iterable)),
			));
	...
	match ast {
		...
		Ast::For(AstFor {
			...
			let body_str = body
				...
				.collect::<Vec<_>>()
				.join("; ");
			format!("{} in {} {{ {} }}", variable.token.value(), render_let_value(iterable), body_str)
		}
...
fn render_let_value(value: &LetValue<'_>) -> String {
	match value {
		LetValue::Statement(stmt) => {
			stmt.nodes.iter().map(render_pipeline_node).collect::<Vec<_>>().join(" | ")
		}
		LetValue::Expression(expr) => render(expr),
	}
}
