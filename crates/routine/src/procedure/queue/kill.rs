...
static INFO: LazyLock<RoutineInfo> = LazyLock::new(|| RoutineInfo::new("queue::kill"));

const PROCEDURE: &str = "queue::kill";

pub struct QueueKill;

impl Default for QueueKill {
	fn default() -> Self {
		Self::new()
	}
}
...
impl<'a, 'tx> Routine<ProcedureContext<'a, 'tx>> for QueueKill {
	...
	#[instrument(name = "queue::kill", level = "debug", skip_all, fields(status = Empty))]
	fn execute(&self, ctx: &mut ProcedureContext<'a, 'tx>, _args: &Columns) -> Result<Columns, RoutineError> {
		...
		record_outcome(PROCEDURE, AttemptOutcome::Dead, ctx, &raw_token, response)
	}
}
