...
use crate::skipdb::watermark::{Closer, WaterMarkError};
...
impl WaterMark {
	...
	pub fn begin(&self, index: u64) -> Result<()> {
		self.check().map(|_| {
			...
			self.inner
				.mark_tx
				.send(Mark { index: MarkIndex::Single(index), waiter: None, done: false })
				.unwrap()
		})
	}
	...
	pub fn begin_many(&self, indices: MediumVec<u64>) -> Result<()> {
		...
		self.check().map(|_| {
			...
			self.inner
				.mark_tx
				.send(Mark { index: MarkIndex::Multiple(indices), waiter: None, done: false })
				.unwrap()
		})
	}
	...
	pub fn done(&self, index: u64) -> Result<()> {
		self.check().map(|_| {
			self.inner
				.mark_tx
				.send(Mark { index: MarkIndex::Single(index), waiter: None, done: true })
	...
	pub fn done_many(&self, indices: MediumVec<u64>) -> Result<()> {
		...
		self.check().map(|_| {
			self.inner
				.mark_tx
				.send(Mark { index: MarkIndex::Multiple(indices), waiter: None, done: true })
				.unwrap() // unwrap is safe because self also holds a receiver
		})
	}
	...
	pub fn wait_for_mark(&self, index: u64) -> Result<()> {
		self.check().map(|_| {
			...
			self.inner
				.mark_tx
				.send(Mark { index: MarkIndex::Single(index), waiter: Some(wait_tx), done: false })
				.unwrap(); // unwrap is safe because self also holds a receiver
...
mod tests {
	...
	fn test_wait_for_mark() {
		init_and_close(|watermark| {
			watermark.begin_many([1, 2, 3].into_iter().collect()).unwrap();
			watermark.done_many([2, 3].into_iter().collect()).unwrap();
