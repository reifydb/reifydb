...
fn test_intersecting_data() {
    ...
    // Setup
    let mut txn = engine.begin();
    txn.set(as_key!("a1"), as_value!(10u64)).unwrap();
    txn.set(as_key!("a2"), as_value!(20u64)).unwrap();
    txn.set(as_key!("b1"), as_value!(100u64)).unwrap();
    txn.set(as_key!("b2"), as_value!(200u64)).unwrap();
    txn.commit().unwrap();
    assert_eq!(1, engine.version());
    ...
    let val = txn1
        ...
        .filter_map(|tv| {
            ...
            let value = from_value!(u64, *tv.value());
            if key.starts_with('a') { Some(value) } else { None }
        })
        .sum::<u64>();

    txn1.set(as_key!("b3"), as_value!(30)).unwrap();
    assert_eq!(30, val);
    ...
    txn2.set(as_key!("a3"), as_value!(300u64)).unwrap();
    assert_eq!(300, val);
...
fn test_intersecting_data2() {
    ...
    let val = txn1
        ...
        .unwrap()
        .map(|tv| from_value!(u64, *tv.value()))
        .sum::<u64>();

    txn1.set(as_key!("b3"), as_value!(10)).unwrap();
    assert_eq!(10, val);
...
fn test_intersecting_data3() {
    ...
    let val = txn1
        ...
        .map(|tv| from_value!(u64, *tv.value()))
        .sum::<u64>();
    txn1.set(as_key!("b3"), as_value!(0u64)).unwrap();
    assert_eq!(0, val);
