...
//! store. Subscribes to CDC, batches writes per shape, drives the encoders in `reifydb-column`, writes transactional
//! `ColumnSnapshot` catalog rows, and populates the `ColumnBlockStore` so newly-written columns are visible to readers.
//!
