...
fn wire_value_to_value(wire: WireValue) -> Result<Value, String> {
	...
	match wire.type_name.as_str() {
		...
		"Float4" => v
			.parse::<f32>()
			.map(Value::float4)
			.map_err(|e| format!("invalid Float4 value '{}': {}", v, e)),
		"Float8" => v
			.parse::<f64>()
			.map(Value::float8)
			.map_err(|e| format!("invalid Float8 value '{}': {}", v, e)),
		"Int1" => v
			.parse::<i8>()
			.map(Value::Int1)
			.map_err(|e| format!("invalid Int1 value '{}': {}", v, e)),
		"Int2" => v
			.parse::<i16>()
			.map(Value::Int2)
			.map_err(|e| format!("invalid Int2 value '{}': {}", v, e)),
		"Int4" => v
			.parse::<i32>()
			.map(Value::Int4)
			.map_err(|e| format!("invalid Int4 value '{}': {}", v, e)),
		"Int8" => v
			.parse::<i64>()
			.map(Value::Int8)
			.map_err(|e| format!("invalid Int8 value '{}': {}", v, e)),
		"Int16" => v
			.parse::<i128>()
			.map(Value::Int16)
			.map_err(|e| format!("invalid Int16 value '{}': {}", v, e)),
		"Utf8" => Ok(Value::Utf8(v.to_string())),
		"Uint1" => v
			.parse::<u8>()
			.map(Value::Uint1)
			.map_err(|e| format!("invalid Uint1 value '{}': {}", v, e)),
		"Uint2" => v
			.parse::<u16>()
			.map(Value::Uint2)
			.map_err(|e| format!("invalid Uint2 value '{}': {}", v, e)),
		"Uint4" => v
			.parse::<u32>()
			.map(Value::Uint4)
			.map_err(|e| format!("invalid Uint4 value '{}': {}", v, e)),
		"Uint8" => v
			.parse::<u64>()
			.map(Value::Uint8)
			.map_err(|e| format!("invalid Uint8 value '{}': {}", v, e)),
		"Uint16" => v
			.parse::<u128>()
			.map(Value::Uint16)
			.map_err(|e| format!("invalid Uint16 value '{}': {}", v, e)),
