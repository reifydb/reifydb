...
fn run_volume(none_values: bool, cfg: ChaosConfig, seed: u64, policy: LatePolicy) -> ChaosOutcome {
	ChaosHarness::<FFIOperatorAdapter<TumblingDriver<VolumeTumbling>>>::builder()
		...
		.with_oracle(move |ctx, batches| {
			tumbling_accumulator_oracle(&VolumeTumbling, ctx, batches, &window_key(), policy)
		})
		.seed(seed)
		.build()
...
fn run_min(none_values: bool, cfg: ChaosConfig, seed: u64, policy: LatePolicy) -> ChaosOutcome {
	ChaosHarness::<FFIOperatorAdapter<TumblingDriver<MinTumbling>>>::builder()
		...
		.with_chaos(cfg)
		.with_oracle(move |ctx, batches| {
			tumbling_accumulator_oracle(&MinTumbling, ctx, batches, &window_key(), policy)
		})
		.seed(seed)
		.build()
...
fn run_ohlcv(none_values: bool, cfg: ChaosConfig, seed: u64, policy: LatePolicy) -> ChaosOutcome {
	...
	ChaosHarness::<FFIOperatorAdapter<TumblingDriver<OhlcvSealingTumbling>>>::builder()
		...
		.with_chaos(cfg)
		.with_oracle(move |ctx, batches| {
			tumbling_accumulator_oracle(&OhlcvSealingTumbling, ctx, batches, &window_key(), policy)
		})
		.seed(seed)
		.build()
...
fn volume_matches_across_configs_and_seeds() {
	for &seed in &common::SEEDS {
		for policy in common::POLICIES {
			run_volume(false, common::baseline(150, SupportedOps::insert_only()), seed, policy)
				.assert_matches();
			run_volume(false, common::baseline(150, SupportedOps::no_remove()), seed, policy)
				.assert_matches();
			run_volume(false, common::baseline(150, SupportedOps::no_update()), seed, policy)
				.assert_matches();
			run_volume(false, common::baseline(200, SupportedOps::all()), seed, policy).assert_matches();
			run_volume(false, common::full_chaos(200), seed, policy).assert_matches();
		}
	}
}
...
fn volume_handles_none_inputs() {
	for &seed in &common::SEEDS {
		for policy in common::POLICIES {
			let outcome = run_volume(true, common::full_chaos(200), seed, policy);
...
fn volume_empty_stream_is_empty() {
	let outcome = run_volume(false, common::baseline(0, SupportedOps::all()), 0, LatePolicy::Drop);
	outcome.assert_matches();
...
fn min_matches_across_configs_and_seeds() {
	for &seed in &common::SEEDS {
		for policy in common::POLICIES {
			run_min(false, common::baseline(150, SupportedOps::insert_only()), seed, policy)
				.assert_matches();
			run_min(false, common::baseline(150, SupportedOps::no_remove()), seed, policy).assert_matches();
			run_min(false, common::baseline(150, SupportedOps::no_update()), seed, policy).assert_matches();
			run_min(false, common::baseline(200, SupportedOps::all()), seed, policy).assert_matches();
			run_min(false, common::full_chaos(200), seed, policy).assert_matches();
		}
	}
}
...
fn min_handles_none_inputs() {
	for &seed in &common::SEEDS {
		for policy in common::POLICIES {
			run_min(true, common::full_chaos(200), seed, policy).assert_matches();
		}
	}
}
...
fn ohlcv_sealing_matches_across_configs_and_seeds() {
	for &seed in &common::SEEDS {
		for policy in common::POLICIES {
			run_ohlcv(false, common::baseline(150, SupportedOps::insert_only()), seed, policy)
				.assert_matches();
			run_ohlcv(false, common::baseline(150, SupportedOps::no_remove()), seed, policy)
				.assert_matches();
			run_ohlcv(false, common::baseline(200, SupportedOps::all()), seed, policy).assert_matches();
			run_ohlcv(false, common::full_chaos(250), seed, policy).assert_matches();
		}
	}
}
...
fn ohlcv_sealing_produces_nonempty_output() {
	...
	let outcome = run_ohlcv(false, common::baseline(200, SupportedOps::insert_only()), 42, LatePolicy::Drop);
	outcome.assert_matches();
	assert!(
