...
use reifydb_abi::{
	constants::{EXTERN_C_END_OF_ITERATION, EXTERN_C_NOT_FOUND, EXTERN_C_OK},
	context::iterators::ExternCStoreIterator,
...
pub(super) fn raw_store_get(ctx: &ExternCOperatorContext, key: &EncodedKey) -> Result<Option<EncodedBytes>> {
...
pub(super) fn raw_store_contains_key(ctx: &ExternCOperatorContext, key: &EncodedKey) -> Result<bool> {
	let key_bytes = key.as_bytes();
...
/// # Safety
///
/// `ctx.ctx` must point to a live `ExternCContext` whose store and memory callbacks
...
pub(super) unsafe fn collect_iterator_results(
	...
	loop {
		let mut key_buf = ExternCBuffer {
			ptr: null_mut(),
			len: 0,
			cap: 0,
		};
		let mut value_buf = ExternCBuffer {
			ptr: null_mut(),
			len: 0,
			cap: 0,
		};
		...
		if next_result == EXTERN_C_END_OF_ITERATION {
			break;
		} else if next_result != EXTERN_C_OK {
