...
type AccContribution<A> = <<A as MultiRollingOperatorV2>::WindowAcc as WindowAccumulator>::Contribution;

pub trait MultiRollingOperatorV2 {
...
pub trait MultiRollingRegistrationV2: MultiRollingOperatorV2 + Sized
where
	Self::Output: Row,
...
pub type MultiRollingBufferV2<A> =
	BTreeMap<<A as MultiRollingOperatorV2>::WindowCoord, <A as MultiRollingOperatorV2>::WindowAcc>;

pub type MultiRollingEmitV2<A> =
	BTreeMap<<A as MultiRollingOperatorV2>::SecondaryKey, <A as MultiRollingOperatorV2>::Output>;

#[derive(Serialize, Deserialize)]
#[serde(bound(
	serialize = "MultiRollingBufferV2<A>: Serialize, MultiRollingEmitV2<A>: Serialize",
	deserialize = "MultiRollingBufferV2<A>: serde::de::DeserializeOwned, MultiRollingEmitV2<A>: serde::de::DeserializeOwned"
))]
struct GroupState<A: MultiRollingOperatorV2> {
	buffer: MultiRollingBufferV2<A>,
	last_emit: MultiRollingEmitV2<A>,
}

impl<A: MultiRollingOperatorV2> Default for GroupState<A> {
	fn default() -> Self {
		Self {
			buffer: BTreeMap::new(),
...
impl<A: MultiRollingOperatorV2> Clone for GroupState<A> {
	fn clone(&self) -> Self {
		Self {
...
impl<A: MultiRollingOperatorV2> fmt::Debug for GroupState<A> {
...
pub struct MultiRollingDriverV2<A>
where
	A: MultiRollingRegistrationV2,
	A::Output: Row,
...
enum AccEvent<A: MultiRollingOperatorV2> {
	Add(AccContribution<A>),
	Remove(AccContribution<A>),
}

impl<A> OperatorMetadata for MultiRollingDriverV2<A>
where
	A: MultiRollingRegistrationV2 + 'static,
	A::Output: Row,
	for<'a> &'a A::GroupKey: IntoEncodedKey,
{
...
impl<A> OperatorLogic for MultiRollingDriverV2<A>
where
	A: MultiRollingRegistrationV2 + Send + Sync + 'static,
	...
	for<'a> &'a A::GroupKey: IntoEncodedKey,
{
	...
	fn apply(&mut self, ctx: &mut impl OperatorContext, change: impl ChangeView) -> Result<()> {
		...
		struct GroupSlot<A: MultiRollingOperatorV2> {
			state_row_number: RowNumber,
			buffer: MultiRollingBufferV2<A>,
			prior_emit: MultiRollingEmitV2<A>,
			buffer_changed: bool,
		}
...
mod tests {
	...
	impl MultiRollingOperatorV2 for TestTopVolume {
	...
	impl MultiRollingRegistrationV2 for TestTopVolume {
		const NAME: &'static str = "test_top_volume_v2";
	...
	fn same_window_volume_accumulates_per_trader() {
		let mut h = FFIOperatorHarnessBuilder::<FFIOperatorAdapter<MultiRollingDriverV2<TestTopVolume>>>::new()
			.build()
			.expect("harness");
