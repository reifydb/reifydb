...
type AccContribution<A> = <<A as RollingOperatorV2>::WindowAcc as WindowAccumulator>::Contribution;

pub trait RollingOperatorV2 {
...
pub trait RollingRegistrationV2: RollingOperatorV2 + Sized
where
	Self::Output: Row,
...
pub struct RollingDriverV2<A>
where
	A: RollingRegistrationV2,
	A::Output: Row,
...
impl<A> OperatorMetadata for RollingDriverV2<A>
where
	A: RollingRegistrationV2 + 'static,
	A::Output: Row,
	for<'a> &'a A::GroupKey: IntoEncodedKey,
{
...
impl<A> OperatorLogic for RollingDriverV2<A>
where
	A: RollingRegistrationV2 + Send + Sync + 'static,
	A::Output: Row,
	...
	for<'a> &'a A::GroupKey: IntoEncodedKey,
{
	...
	fn apply(&mut self, ctx: &mut impl OperatorContext, change: impl ChangeView) -> Result<()> {
		...
		struct GroupSlot<A: RollingOperatorV2> {
...
mod tests {
	...
	impl RollingOperatorV2 for TestRollingSum {
	...
	impl RollingRegistrationV2 for TestRollingSum {
		const NAME: &'static str = "test_rolling_sum_v2";
	...
	fn single_insert_emits_insert() {
		let mut h = FFIOperatorHarnessBuilder::<FFIOperatorAdapter<RollingDriverV2<TestRollingSum>>>::new()
			.build()
			.expect("harness");
		let out =
	...
	fn multiple_events_accumulate_within_one_window() {
		...
		// This is the V2 capability the old last-write-wins buffer lacked.
		let mut h = FFIOperatorHarnessBuilder::<FFIOperatorAdapter<RollingDriverV2<TestRollingSum>>>::new()
			.build()
			.expect("harness");
		let out = h
