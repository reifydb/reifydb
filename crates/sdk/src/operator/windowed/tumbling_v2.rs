...
type AccContribution<A> = <<A as TumblingOperatorV2>::Acc as WindowAccumulator>::Contribution;
type AccValue<A> = <<A as TumblingOperatorV2>::Acc as WindowAccumulator>::Output;
type Buckets<A> = BTreeMap<
	(<A as TumblingOperatorV2>::GroupKey, WindowSpan<<A as TumblingOperatorV2>::WindowCoord>),
	Vec<AccEvent<A>>,
>;

pub trait TumblingOperatorV2 {
...
pub struct TumblingDriverV2<A>
where
	A: TumblingRegistrationV2,
	A::Output: Row,
...
enum AccEvent<A: TumblingOperatorV2> {
	Add(AccContribution<A>),
	Remove(AccContribution<A>),
}

impl<A> TumblingDriverV2<A>
where
	A: TumblingRegistrationV2,
	A::Output: Row,
	for<'a> &'a A::GroupKey: IntoEncodedKey,
...
impl<A> OperatorMetadata for TumblingDriverV2<A>
where
	A: TumblingRegistrationV2 + 'static,
	A::Output: Row,
	for<'a> &'a A::GroupKey: IntoEncodedKey,
{
...
impl<A> OperatorLogic for TumblingDriverV2<A>
where
	A: TumblingRegistrationV2 + Send + Sync + 'static,
	A::Output: Row,
...
mod tests {
	...
	impl TumblingOperatorV2 for TestVolume {
	...
	impl TumblingRegistrationV2 for TestVolume {
		const NAME: &'static str = "test_volume_v2";
	...
	impl TumblingOperatorV2 for TestMin {
		type GroupKey = String;
		type WindowCoord = u64;
		type Acc = MinAcc;
		type Output = MinOut;
	...
	impl TumblingRegistrationV2 for TestMin {
		const NAME: &'static str = "test_min_v2";
		const VERSION: &'static str = "0.0.1";
		const DESCRIPTION: &'static str = "test fixture";
		const INPUT_COLUMNS: &'static [OperatorColumn] = &[];
		const OUTPUT_COLUMNS: &'static [OperatorColumn] = &[];
	...
	fn single_insert_emits_insert() {
		let mut h = FFIOperatorHarnessBuilder::<FFIOperatorAdapter<TumblingDriverV2<TestVolume>>>::new()
			.build()
			.expect("harness");
		let out =
	...
	fn min_update_replacing_minimum_raises_window_min() {
		...
		// leaves {6, 8, 10}.
		let mut h = FFIOperatorHarnessBuilder::<FFIOperatorAdapter<TumblingDriverV2<TestMin>>>::new()
			.build()
			.expect("harness");
		let _ = h
			.apply(TestChangeBuilder::new()
