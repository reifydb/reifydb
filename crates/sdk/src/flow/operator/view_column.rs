...
impl<'a> ColumnView<'a> {
	...
	pub fn u8(&self) -> Option<&'a [u8]> {
		if self.type_code() != ValueKind::Uint1 {
			return None;
		}
	...
	pub fn u16(&self) -> Option<&'a [u16]> {
		if self.type_code() != ValueKind::Uint2 {
			return None;
		}
	...
	pub fn u32(&self) -> Option<&'a [u32]> {
		if self.type_code() != ValueKind::Uint4 {
			return None;
		}
	...
	pub fn u64(&self) -> Option<&'a [u64]> {
		if self.type_code() != ValueKind::Uint8 {
			return None;
		}
	...
	pub fn u128(&self) -> Option<&'a [u128]> {
		if self.type_code() != ValueKind::Uint16 {
			return None;
		}
	...
	pub fn i8(&self) -> Option<&'a [i8]> {
		if self.type_code() != ValueKind::Int1 {
			return None;
		}
	...
	pub fn i16(&self) -> Option<&'a [i16]> {
		if self.type_code() != ValueKind::Int2 {
			return None;
		}
	...
	pub fn i32(&self) -> Option<&'a [i32]> {
		if self.type_code() != ValueKind::Int4 {
			return None;
		}
	...
	pub fn i64(&self) -> Option<&'a [i64]> {
		if self.type_code() != ValueKind::Int8 {
			return None;
		}
	...
	pub fn i128(&self) -> Option<&'a [i128]> {
		if self.type_code() != ValueKind::Int16 {
			return None;
		}
	...
	pub fn f32(&self) -> Option<&'a [f32]> {
		if self.type_code() != ValueKind::Float4 {
			return None;
		}
	...
	pub fn f64(&self) -> Option<&'a [f64]> {
		if self.type_code() != ValueKind::Float8 {
			return None;
		}
	...
	pub fn bool_iter(&self) -> Option<BoolIter<'a>> {
		if self.type_code() != ValueKind::Boolean {
			return None;
		}
		Some(BoolIter {
	...
	pub fn utf8_iter(&self) -> Option<impl Iterator<Item = &'a str> + 'a> {
		if self.type_code() != ValueKind::Utf8 {
			return None;
		}
	...
	pub fn blob_iter(&self) -> Option<impl Iterator<Item = &'a [u8]> + 'a> {
		if self.type_code() != ValueKind::Blob {
			return None;
		}
	...
	pub fn to_u64_vec(&self) -> Option<Vec<u64>> {
		match self.type_code() {
			ValueKind::Uint8 => self.u64().map(|s| s.to_vec()),
			ValueKind::Uint4 => self.u32().map(|s| s.iter().map(|v| u64::from(*v)).collect()),
			ValueKind::Uint2 => self.u16().map(|s| s.iter().map(|v| u64::from(*v)).collect()),
			ValueKind::Uint1 => self.u8().map(|s| s.iter().map(|v| u64::from(*v)).collect()),
			_ => None,
		}
	}
	...
	pub fn to_i64_vec(&self) -> Option<Vec<i64>> {
		match self.type_code() {
			ValueKind::Int8 => self.i64().map(|s| s.to_vec()),
			ValueKind::Int4 => self.i32().map(|s| s.iter().map(|v| i64::from(*v)).collect()),
			ValueKind::Int2 => self.i16().map(|s| s.iter().map(|v| i64::from(*v)).collect()),
			ValueKind::Int1 => self.i8().map(|s| s.iter().map(|v| i64::from(*v)).collect()),
			_ => None,
		}
	}
	...
	pub fn to_f64_vec(&self) -> Option<Vec<f64>> {
		match self.type_code() {
			ValueKind::Float8 => self.f64().map(|s| s.to_vec()),
			ValueKind::Float4 => self.f32().map(|s| s.iter().map(|v| f64::from(*v)).collect()),
			_ => None,
		}
	}
}
