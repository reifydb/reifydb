	...
	pub(crate) fn new(ctx: &'a mut ExternCContext) -> Self {
		Self {
			inner: ColumnsBuilder::new(
				...
				unsafe { (*ctx.ctx).callbacks.builder },
				unsafe { (*ctx.ctx).written_at_nanos },
			),
		}
	}
...
fn validate_row_or_panic(schema: &mut Vec<(String, ValueKind)>, fields: &[(String, Value)], context: &'static str) {
	if schema.is_empty() {
...
fn infer_schema_from_first_row(
	schema: &mut Vec<(String, ValueKind)>,
	fields: &[(String, Value)],
	context: &'static str,
) {
...
#[inline]
fn validate_row_against_schema(schema: &[(String, ValueKind)], fields: &[(String, Value)], context: &'static str) {
...
fn transpose(schema: &[(String, ValueKind)], rows: &[&Vec<(String, Value)>]) -> Result<Vec<Vec<Value>>, SdkError> {
...
fn type_to_column_code(ty: ValueType) -> Option<ValueKind> {
	let code = match ty {
