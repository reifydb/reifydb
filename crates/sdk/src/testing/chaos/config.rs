	...
	pub fn to_scenario(&self, coord_span_ms: u64, drain_at_ms: u64) -> Scenario {
		...
		let batch = match self.batch_size {
			...
			BatchSizeDist::Uniform {
				max,
				..
			} => BatchSize::Uniform(max.max(1) as u32),
			BatchSizeDist::Geometric(p) => BatchSize::Geometric {
				p,
				max: max_batch,
			},
		};
		Scenario {
			...
			..Scenario::windowed(self.num_ops as u32, max_batch, coord_span_ms, drain_at_ms)
				.with_mix(
					if self.supported_ops.remove {
						25
					} else {
						0
					},
					if self.supported_ops.update {
						30
					} else {
						0
					},
...
mod scenario_projection_tests {
	...
	fn a_guest_configuration_carries_its_primitives_into_the_shared_scenario() {
		...
		assert!(
			matches!(scenario.batch, BatchSize::Geometric { .. }),
			"a long-tailed batch must stay long-tailed"
		);
