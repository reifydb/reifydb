...
pub trait FFISingleStateful: FFIRawStatefulOperator {
	...
	fn load_state(&self, ctx: &mut FFIOperatorContext) -> Result<Option<Self::State>> {
		let key = self.key();
		ctx.state().get::<Self::State>(&key)
	}

	fn save_state(&self, ctx: &mut FFIOperatorContext, value: &Self::State) -> Result<()> {
		let key = self.key();
		ctx.state().set(&key, value)
	}

	fn clear_state(&self, ctx: &mut FFIOperatorContext) -> Result<()> {
		let key = self.key();
