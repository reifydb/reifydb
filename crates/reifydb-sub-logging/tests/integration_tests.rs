...
fn test_backend_failure_resilience() {
	...
	// These logs should not crash the system (they will be buffered
	// internally)
	for i in 0..5 {
		log(Record::new(
			LogLevel::Error,
	...
	// We should have the initial logs plus the recovery logs (not the
	// failed ones)
	wait_for(
	...
	// We should have at least 10 logs (5 before + 5 after, failed logs are
	// dropped)
	assert!(
		final_logs.len() >= 10,
