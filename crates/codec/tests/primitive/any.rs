...
use reifydb_codec::value::{decode_value as decode_value_codec, encode_value as encode_value_codec};
...
fn test_any_roundtrip_via_set_get_value() {
	...
	let cases: &[Value] =
		&[Value::Boolean(false), Value::Int4(42), Value::Utf8("test".to_string()), Value::Uint8(1234567890)];
...
fn test_any_multiple_fields() {
	let shape = RowShape::testing(RowFamily::Pod, &[ValueType::Any, ValueType::Int4, ValueType::Any]);
	let mut row = shape.allocate_pod();
...
fn test_update_any_with_other_dynamic_fields() {
	let shape = RowShape::testing(RowFamily::Pod, &[ValueType::Any, ValueType::Utf8, ValueType::Any]);
	let mut row = shape.allocate_pod();
...
fn test_any_set_get_none_duration() {
	...
	assert!(row.is_defined(0), "an Any field holding a None-of-Duration is a stored value, not an unset field");
