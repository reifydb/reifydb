...
impl EncodedRow {
	...
	pub fn set_time(&mut self, time: DateTime) {
		self.0.make_mut()[TIME_OFFSET..TIME_OFFSET + DateTime::ENCODED_SIZE].copy_from_slice(&time.to_le_bytes());
	}
}
...
mod tests {
	...
	use crate::encoded::{
		row::{CREATED_AT_OFFSET, FINGERPRINT_SIZE, TIME_OFFSET, UPDATED_AT_OFFSET},
		shape::{RowShape, RowShapeField, SHAPE_HEADER_SIZE},
	};
