...
pub fn type_constraint_to_extern_c(tc: &TypeConstraint) -> Result<ExternCTypeConstraint, EncodeError> {
	let base_type = TypeTag::of_type(&tc.get_type())?.byte();
	Ok(match tc.constraint() {
		None => ExternCTypeConstraint {
			base_type,
			constraint_type: 0,
			constraint_param1: 0,
			constraint_param2: 0,
		},
		Some(Constraint::MaxBytes(max)) => ExternCTypeConstraint {
			base_type,
			constraint_type: 1,
			constraint_param1: max.value(),
			constraint_param2: 0,
		},
		Some(Constraint::PrecisionScale(p, s)) => ExternCTypeConstraint {
			base_type,
			constraint_type: 2,
			constraint_param1: p.value() as u32,
			constraint_param2: s.value() as u32,
		},
		Some(Constraint::Dictionary(dict_id, id_type)) => ExternCTypeConstraint {
			base_type,
			constraint_type: 3,
			constraint_param1: dict_id.to_u64() as u32,
			constraint_param2: TypeTag::of_type(id_type)?.byte() as u32,
		},
		Some(Constraint::SumType(id)) => ExternCTypeConstraint {
			base_type,
			constraint_type: 4,
	...
	Ok(match extern_c.constraint_type {
		...
		2 => TypeConstraint::with_constraint(
			...
			Constraint::PrecisionScale(
				Precision::new(extern_c.constraint_param1 as u8),
				Scale::new(extern_c.constraint_param2 as u8),
			),
		),
		3 => TypeConstraint::with_constraint(
			ty,
			Constraint::Dictionary(
				DictionaryId::from(extern_c.constraint_param1 as u64),
				TypeTag::from_byte(extern_c.constraint_param2 as u8)?.to_type()?,
			),
		),
		4 => TypeConstraint::with_constraint(
			ty,
			Constraint::SumType(SumTypeId::from(extern_c.constraint_param1 as u64)),
		),
