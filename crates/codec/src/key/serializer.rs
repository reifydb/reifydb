...
pub mod tests {
	...
	use crate::key::{deserializer::KeyDeserializer, serializer::KeySerializer, sort::SortOrder};
	...
	fn test_extend_value_with_direction_none_policy() {
		...
		assert!(
			enc(&present, SortOrder::Asc) < enc(&none, SortOrder::Asc),
			"asc: present should sort before none"
		);
		assert!(
			enc(&none, SortOrder::Desc) < enc(&present, SortOrder::Desc),
