...
use reifydb_core::{
	...
	operator_with::WindowWith,
};
use reifydb_value::{fragment::Fragment, value::duration::Duration};

use crate::{
	Result,
	ast::ast::{AstOperatorWith, AstOperatorWithEntry, AstWindowKind},
	diagnostic::AstError,
...
impl<'bump> Compiler<'bump> {
	pub(crate) fn compile_window_with(
		kind: AstWindowKind,
		with: Option<&AstOperatorWith<'bump>>,
		window: Fragment,
	) -> Result<WindowWith> {
		let parsed = Self::parse_config(with, window)?;
		let kind = Self::build_window_kind(kind, &parsed)?;
	...
	fn parse_config(with: Option<&AstOperatorWith<'bump>>, window: Fragment) -> Result<ParsedConfig> {
	...
	pub(super) fn parse_config_item(entry: &AstOperatorWithEntry<'bump>, config: &mut ParsedConfig) -> Result<()> {
		...
		match key {
			"duration" => {
				config.duration = Some(declared_duration(
					literal(entry)?,
					"'duration'",
			...
			"count" => {
				config.count = Some(declared_count(literal(entry)?)?);
			}
			"slide" => {
				let token = literal(entry)?;
				if is_duration(token) {
					config.slide_duration =
						Some(declared_duration(token, "'slide'", DurationBound::Positive)?);
				} else if is_number(token) {
					config.slide_count = Some(declared_count(token)?);
				} else {
			...
			"gap" => {
				config.gap =
					Some(declared_duration(literal(entry)?, "'gap'", DurationBound::AllowZero)?);
			}
			"lag" => {
				config.lag =
					Some(declared_duration(literal(entry)?, "'lag'", DurationBound::AllowZero)?);
			}
			"lateness" => {
				config.lateness = Some(declared_duration(
					literal(entry)?,
					"'lateness'",
			...
			"immutable" => {
				...
				match literal_boolean(token) {
					...
					Some(false) => {}
					None => {
						config.immutable = Some(declared_duration(
							token,
							"'immutable'",
							DurationBound::AllowZero,
						)?);
			...
			_ => return Err(unknown_key(entry, WINDOW_CONFIG_KEYS)),
		}
		Ok(())
	}
	...
	fn reject_immutable_not_smaller_than_lateness(parsed: &ParsedConfig) -> Result<()> {
		reject_immutable_not_smaller_than_lateness(parsed.immutable.as_ref(), parsed.lateness.as_ref())
	}
...
mod tests {
	...
	fn parse_window_config(source: &str) -> Result<ParsedConfig> {
		...
		Compiler::parse_config(window.with.as_ref(), window.token.fragment.to_owned())
	}
