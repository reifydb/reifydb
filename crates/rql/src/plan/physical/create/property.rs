...
use crate::{
	Result,
	ast::identifier::MaybeQualifiedColumnObject,
...
impl<'bump> Compiler<'bump> {
	pub(crate) fn compile_create_column_property(
		...
		let (ns_segments, table_fragment) = match &create.column.object {
		...
		let Some(ns) = self.catalog.find_namespace_by_segments(rx, &ns_segments)? else {
			let ns_fragment = match &create.column.object {
				MaybeQualifiedColumnObject::Qualified {
					namespace,
					..
				} => {
		...
		let namespace_id = match &create.column.object {
			MaybeQualifiedColumnObject::Qualified {
				namespace,
				..
			} => {
