...
impl Compiler {
	pub(crate) fn compile_create_ring_buffer<'a>(
		...
		let namespace_name = create.ring_buffer.namespace.as_ref().map(|n| n.text()).unwrap_or("default");
		let Some(namespace_def) = CatalogStore::find_namespace_by_name(rx, namespace_name)? else {
			let ns_fragment = create.ring_buffer.namespace.clone().unwrap_or_else(|| {
		...
		let namespace_id = create.ring_buffer.namespace.clone().unwrap_or_else(|| {
			use reifydb_type::Fragment;
