...
use crate::{
	...
	plan::{
		logical,
		physical::{Compiler, CreateQueueNode, PhysicalPlan},
	},
};
...
impl<'bump> Compiler<'bump> {
	pub(crate) fn compile_create_queue(
		...
		create: logical::CreateQueueNode<'_>,
	) -> Result<PhysicalPlan<'bump>> {
		let ns_segments: Vec<&str> = create.queue.namespace.iter().map(|n| n.text()).collect();
		let Some(namespace) = self.catalog.find_namespace_by_segments(rx, &ns_segments)? else {
			let ns_fragment = if let Some(n) = create.queue.namespace.first() {
		...
		let namespace_id = if let Some(n) = create.queue.namespace.first() {
			let interned = self.interner.intern_fragment(n);
		...
		Ok(CreateQueue(CreateQueueNode {
