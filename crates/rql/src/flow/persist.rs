...
use reifydb_core::{
	interface::catalog::flow::{FlowEdge, FlowId, Operator, OperatorId},
	internal,
};
...
pub fn persist_flow(
	...
	let mut node_map: HashMap<LocalNodeId, OperatorId> = HashMap::new();

	let mut builder = FlowDag::builder(flow_id);

	for compiled_node in &plan.nodes {
		let real_node_id = catalog.next_operator_id(txn)?;
		...
		catalog.create_operator(txn, &node_def)?;

		builder.add_node(operator::FlowNode::new(real_node_id, compiled_node.node_type.clone()));
	}
	...
	for compiled_edge in &plan.edges {
		...
		builder.add_edge(operator::FlowEdge::new(real_edge_id, real_source, real_target))?;
	}
