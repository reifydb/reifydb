...
pub fn to_owned_expression(expr: Expression<'_>) -> Expression<'static> {
	match expr {
		...
		Expression::Alias(alias) => {
			Expression::Alias(AliasExpression {
				alias: IdentExpression(Fragment::Owned(
					alias.alias.0.into_owned(),
				)),
				expression: Box::new(to_owned_expression(
					*alias.expression,
				)),
				fragment: Fragment::Owned(
					alias.fragment.into_owned(),
				),
			})
		}
		...
		Expression::Call(call) => Expression::Call(CallExpression {
			func: IdentExpression(Fragment::Owned(
				call.func.0.into_owned(),
			)),
			args: call
				.args
				.into_iter()
				.map(to_owned_expression)
				.collect(),
		...
		Expression::Tuple(tuple) => {
			Expression::Tuple(TupleExpression {
				expressions: tuple
					...
					.collect(),
				fragment: Fragment::Owned(
					tuple.fragment.into_owned(),
				),
			})
		}
		Expression::Prefix(prefix) => {
			Expression::Prefix(PrefixExpression {
				operator: to_owned_prefix_operator(
					prefix.operator,
				),
				expression: Box::new(to_owned_expression(
					*prefix.expression,
		...
		Expression::GreaterThanEqual(gte) => {
			Expression::GreaterThanEqual(GreaterThanEqExpression {
				...
				right: Box::new(to_owned_expression(
					*gte.right,
				)),
				fragment: Fragment::Owned(
					gte.fragment.into_owned(),
				),
			})
		}
		...
		Expression::Between(between) => {
			Expression::Between(BetweenExpression {
				value: Box::new(to_owned_expression(
					*between.value,
				)),
				lower: Box::new(to_owned_expression(
					*between.lower,
				)),
				upper: Box::new(to_owned_expression(
					*between.upper,
				)),
				fragment: Fragment::Owned(
					between.fragment.into_owned(),
				),
			})
		}
		Expression::Type(type_expr) => {
			Expression::Type(to_owned_type_expression(type_expr))
		}
		Expression::Parameter(param) => Expression::Parameter(
			to_owned_parameter_expression(param),
		),
	}
}
...
fn to_owned_constant_expression(
	constant: ConstantExpression<'_>,
) -> ConstantExpression<'static> {
	match constant {
...
fn to_owned_type_expression(
	type_expr: TypeExpression<'_>,
) -> TypeExpression<'static> {
	TypeExpression {
		ty: type_expr.ty,
...
fn to_owned_parameter_expression(
...
fn to_owned_prefix_operator(op: PrefixOperator<'_>) -> PrefixOperator<'static> {
	match op {
		PrefixOperator::Minus(fragment) => PrefixOperator::Minus(
			Fragment::Owned(fragment.into_owned()),
		),
		PrefixOperator::Plus(fragment) => PrefixOperator::Plus(
			Fragment::Owned(fragment.into_owned()),
		),
		PrefixOperator::Not(fragment) => PrefixOperator::Not(
			Fragment::Owned(fragment.into_owned()),
		),
	}
}
...
pub fn to_owned_expressions(
	exprs: Vec<Expression<'_>>,
