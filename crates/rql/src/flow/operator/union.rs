...
use crate::{
	Result,
	plan::physical::{PhysicalPlan, UnionNode},
};

pub(crate) struct UnionCompiler {
	pub left: Box<PhysicalPlan<'static>>,
	pub right: Box<PhysicalPlan<'static>>,
}

impl<'a> From<UnionNode<'a>> for UnionCompiler {
	fn from(node: UnionNode<'a>) -> Self {
		Self {
...
impl<T: CommandTransaction> CompileOperator<T> for UnionCompiler {
	fn compile(self, compiler: &mut FlowCompiler<T>) -> Result<FlowNodeId> {
		let left_node = compiler.compile_plan(*self.left)?;
		let right_node = compiler.compile_plan(*self.right)?;
		let node = compiler.build_node(FlowNodeType::Union).with_inputs([left_node, right_node]).build()?;
		Ok(node)
	}
}
