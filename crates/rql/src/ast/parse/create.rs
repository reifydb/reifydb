...
mod tests {
	...
	#[test]
	fn test_create_table_with_overflow_policy() {
		let tokens = lex(r#"
            create table test.items(field: int2 policy (underflow error) )
        "#)
			.unwrap();
		let mut parser = Parser::new(tokens);
		let mut result = parser.parse().unwrap();
		...
		match create {
			AstCreate::Table { name, schema, definitions, .. } => {
...

				{
					...
					assert!(matches!(overflow.policy, AstPolicyKind::Underflow));
