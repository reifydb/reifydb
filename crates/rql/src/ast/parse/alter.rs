...
use crate::ast::lex::{Keyword, Operator, Token};
use crate::ast::parse::Parser;
use crate::ast::{AstAlter, AstAlterSequence};
...
impl Parser {
    ...
    fn parse_alter_sequence(&mut self, token: Token) -> crate::Result<AstAlter> {
        ...
        if self.current()?.is_operator(Operator::Dot) {
            ...
            unimplemented!("ALTER SEQUENCE requires table.column or schema.table.column");
        }
    }
}
...
mod tests {
    ...
    fn test_alter_sequence_with_schema() {
        ...
        match alter {
            AstAlter::Sequence(AstAlterSequence { schema, table, column, value, .. }) => {
                assert!(schema.is_some());
                assert_eq!(schema.as_ref().unwrap().value(), "test");
                assert_eq!(table.value(), "users");
                assert_eq!(column.value(), "id");
                match value {
                    crate::ast::AstLiteral::Number(num) => assert_eq!(num.value(), "1000"),
    ...
    fn test_alter_sequence_without_schema() {
        ...
        match alter {
            AstAlter::Sequence(AstAlterSequence { schema, table, column, value, .. }) => {
                ...
                assert_eq!(column.value(), "id");
                match value {
                    crate::ast::AstLiteral::Number(num) => assert_eq!(num.value(), "500"),
                    _ => panic!("Expected number literal"),
                }
            }
        }
    ...
    fn test_alter_sequence_case_insensitive() {
        let tokens = lex("alter sequence TEST.USERS.ID set value 2000").unwrap();
        let mut parser = Parser::new(tokens);
        let mut result = parser.parse().unwrap();
        assert_eq!(result.len(), 1);
        ...
        match alter {
            AstAlter::Sequence(AstAlterSequence { schema, table, column, value, .. }) => {
                ...
                assert_eq!(column.value(), "ID");
                match value {
                    crate::ast::AstLiteral::Number(num) => assert_eq!(num.value(), "2000"),
                    _ => panic!("Expected number literal"),
                }
            }
        }
