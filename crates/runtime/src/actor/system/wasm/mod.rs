...
impl ActorSystem {
	...
	/// A system for test fixtures. Actors run inline here, so unlike the native build there are no
	/// worker threads to share - the system only has to outlive the spawners it hands out, which
