...
//! # Native Only
//!
//! The runner is only used on native platforms where each actor runs on
//! its own OS thread with a blocking message receive loop.
//!
//! On WASM, actors process messages inline (synchronously) when sent,
//! so no separate runner is needed.
