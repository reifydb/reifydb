...
//! SQLite connection utilities for the CDC backend.
...
use super::config::{DbPath, OpenFlags};
...
pub(super) fn connect(path: &DbPath, flags: SqliteOpenFlags) -> Result<Connection> {
	fn connection_failed(path: String, error: String) -> String {
		format!("Failed to connect to CDC database at {}: {}", path, error)
	}
...
/// Resolve the CDC database path, creating directories as needed.
	...
	match db_path {
		...
		DbPath::File(config_path) => {
			...
			if is_uri {
				...
				DbPath::File(config_path.join("cdc.db"))
			} else {
