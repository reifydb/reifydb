...
use crate::{CatalogStore, store::ring_buffer::layout::ring_buffer};
			...
			if let Some(key) = Key::decode(&entry.key) {
				if let Key::RingBuffer(ring_buffer_key) = key {
					...
					let namespace_id = NamespaceId(
						ring_buffer::LAYOUT.get_u64(&entry.values, ring_buffer::NAMESPACE),
					);

					let name = ring_buffer::LAYOUT
						.get_utf8(&entry.values, ring_buffer::NAME)
						.to_string();

					let capacity =
						ring_buffer::LAYOUT.get_u64(&entry.values, ring_buffer::CAPACITY);
...
mod tests {
	...
	use crate::{
		CatalogStore, namespace::NamespaceToCreate, ring_buffer::create::RingBufferToCreate,
