...
//! RowSchema retrieval from storage.

use reifydb_core::{
	encoded::schema::{RowSchema, RowSchemaField, fingerprint::RowSchemaFingerprint},
	error::diagnostic::internal::internal,
	key::{
		EncodableKey,
		schema::{RowSchemaFieldKey, RowSchemaKey},
	},
};
...
pub(crate) fn find_row_schema_by_fingerprint(
	...
	for i in 0..field_count {
		...
		let field_entry = txn.get(&field_key)?.ok_or_else(|| {
			Error(internal(format!("RowSchema field {} missing for fingerprint {:?}", i, fingerprint)))
		})?;
	...
	Ok(Some(RowSchema::from_parts(fingerprint, fields)))
}
...
pub fn load_all_row_schemas(rx: &mut Transaction<'_>) -> Result<Vec<RowSchema>> {
	// First pass: collect all schema headers (fingerprint, field_count)
	let mut schema_headers: Vec<(RowSchemaFingerprint, usize)> = Vec::new();

	{
		let range = RowSchemaKey::full_scan();
		...
		while let Some(entry) = stream.next() {
			...
			let schema_key = RowSchemaKey::decode(&entry.key)
	...
	for (fingerprint, field_count) in schema_headers {
		...
		schemas.push(RowSchema::from_parts(fingerprint, fields));
	}
