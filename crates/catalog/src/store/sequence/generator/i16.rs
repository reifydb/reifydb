...
static LAYOUT: Lazy<EncodedValuesLayout> = Lazy::new(|| EncodedValuesLayout::new(&[Type::Int2]));
...
impl GeneratorI16 {
	...
	pub(crate) fn next_batched(
		...
		txn.with_single_command([key], |tx| match tx.get(key)? {
			Some(row) => {
				...
				let current_value = LAYOUT.get_i16(&row, 0);
			...
			None => {
				let result = default.unwrap_or(1i16);
				let mut new_row = LAYOUT.allocate();
				LAYOUT.set_i16(&mut new_row, 0, result);
				tx.set(key, new_row)?;
				Ok(result)
			}
		})
	...
	pub(crate) fn set(txn: &mut impl CommandTransaction, key: &EncodedKey, value: i16) -> crate::Result<()> {
...
mod tests {
	...
	use crate::store::sequence::generator::i16::{GeneratorI16, LAYOUT};

	#[test]
	fn test_ok() {
		let mut txn = create_test_command_transaction();
		for expected in 1..1000 {
			let got = GeneratorI16::next(&mut txn, &EncodedKey::new("sequence"), None).unwrap();
		...
		txn.with_single_query([&key], |tx| {
			...
			assert_eq!(LAYOUT.get_i16(&single.values, 0), 999);
			Ok(())
		})
		.unwrap();
	}
	...
	fn test_exhaustion() {
		...
		let err = GeneratorI16::next(&mut txn, &EncodedKey::new("sequence"), None).unwrap_err();
	...
	fn test_default() {
		...
		let got =
			GeneratorI16::next(&mut txn, &EncodedKey::new("sequence_with_default"), Some(100i16)).unwrap();
		assert_eq!(got, 100);

		let got =
			GeneratorI16::next(&mut txn, &EncodedKey::new("sequence_with_default"), Some(999i16)).unwrap();
		assert_eq!(got, 101);
	}
	...
	fn test_batched_ok() {
		...
		for i in 0..20 {
			let expected = 1 + (i * 50);
			let got = GeneratorI16::next_batched(&mut txn, &EncodedKey::new("sequence_by_50"), None, 50)
				.unwrap();
			assert_eq!(got, expected);
		}
		...
		txn.with_single_query([&key], |tx| {
			let single = tx.get(&key)?.unwrap();
			assert_eq!(LAYOUT.get_i16(&single.values, 0), 951);
			Ok(())
		})
		.unwrap();
		...
		for i in 0..10 {
			...
			let got = GeneratorI16::next_batched(&mut txn, &EncodedKey::new("sequence_by_100"), None, 100)
				.unwrap();
			assert_eq!(got, expected);
		}
	}
	...
	fn test_batched_exhaustion() {
		...
		let mut row = LAYOUT.allocate();
		LAYOUT.set_i16(&mut row, 0, i16::MAX - 20);

		let key = EncodedKey::new("sequence");
		txn.with_single_command([&key], |tx| tx.set(&key, row)).unwrap();

		// This should succeed (MAX - 20 + 50 saturates to MAX)
		let result = GeneratorI16::next_batched(&mut txn, &EncodedKey::new("sequence"), None, 50).unwrap();
		assert_eq!(result, i16::MAX);

		// This should fail (already at MAX)
		let err = GeneratorI16::next_batched(&mut txn, &EncodedKey::new("sequence"), None, 50).unwrap_err();
		assert_eq!(err.diagnostic(), sequence_exhausted(Type::Int2));
	}
	...
	fn test_batched_default() {
		...
		let got = GeneratorI16::next_batched(
			&mut txn,
