...
static LAYOUT: Lazy<EncodedValuesLayout> = Lazy::new(|| EncodedValuesLayout::new(&[Type::Int16]));
	...
	pub(crate) fn next(
		...
		txn.with_single_command(|tx| match tx.get(key)? {
			...
			None => {
				...
				LAYOUT.set_i128(&mut new_row, 0, result);
				tx.set(key, new_row)?;
				Ok(result)
			}
		})
...
mod tests {
	...
	use crate::store::sequence::generator::i128::{GeneratorI128, LAYOUT};
	...
	fn test_ok() {
		...
		txn.with_single_query(|tx| {
			...
			assert_eq!(LAYOUT.get_i128(&single.values, 0), 999);
