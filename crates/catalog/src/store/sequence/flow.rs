...
pub(crate) fn next_flow_id(txn: &mut StandardCommandTransaction) -> crate::Result<FlowId> {
	GeneratorU64::next(txn, &FLOW_KEY, None).map(FlowId)
}

pub(crate) fn next_flow_node_id(txn: &mut StandardCommandTransaction) -> crate::Result<FlowNodeId> {
	GeneratorU64::next(txn, &FLOW_NODE_KEY, None).map(FlowNodeId)
}

pub(crate) fn next_flow_edge_id(txn: &mut StandardCommandTransaction) -> crate::Result<FlowEdgeId> {
