...
use reifydb_core::interface::catalog::flow::{FlowEdgeId, FlowId, OperatorId};
...
pub(crate) fn next_operator_id(txn: &mut AdminTransaction) -> Result<OperatorId> {
	GeneratorU64::next(txn, &FLOW_NODE_KEY, None).map(OperatorId)
}
