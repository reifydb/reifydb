...
impl SystemSequence {
	pub(crate) async fn next_namespace_id(txn: &mut StandardCommandTransaction) -> crate::Result<NamespaceId> {
		GeneratorU64::next(txn, &NAMESPACE_KEY, Some(1025)).await.map(NamespaceId)
	}

	pub(crate) async fn next_table_id(txn: &mut StandardCommandTransaction) -> crate::Result<TableId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).await.map(TableId)
	}

	pub(crate) async fn next_column_id(txn: &mut StandardCommandTransaction) -> crate::Result<ColumnId> {
		GeneratorU64::next(txn, &COLUMN_KEY, Some(8193)).await.map(ColumnId)
	}

	pub(crate) async fn next_column_policy_id(
		txn: &mut StandardCommandTransaction,
	) -> crate::Result<ColumnPolicyId> {
		GeneratorU64::next(txn, &COLUMN_POLICY_KEY, Some(1025)).await.map(ColumnPolicyId)
	}

	pub(crate) async fn next_view_id(txn: &mut StandardCommandTransaction) -> crate::Result<ViewId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).await.map(ViewId)
	}

	pub(crate) async fn next_primary_key_id(txn: &mut StandardCommandTransaction) -> crate::Result<PrimaryKeyId> {
		GeneratorU64::next(txn, &PRIMARY_KEY_KEY, None).await.map(PrimaryKeyId)
	}

	pub(crate) async fn next_ringbuffer_id(txn: &mut StandardCommandTransaction) -> crate::Result<RingBufferId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).await.map(RingBufferId)
	}

	pub(crate) async fn next_dictionary_id(txn: &mut StandardCommandTransaction) -> crate::Result<DictionaryId> {
