...
impl SystemSequence {
	pub(crate) fn next_namespace_id(txn: &mut StandardCommandTransaction) -> crate::Result<NamespaceId> {
		GeneratorU64::next(txn, &NAMESPACE_KEY, Some(1025)).map(NamespaceId)
	}

	pub(crate) fn next_table_id(txn: &mut StandardCommandTransaction) -> crate::Result<TableId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).map(TableId)
	}

	pub(crate) fn next_column_id(txn: &mut StandardCommandTransaction) -> crate::Result<ColumnId> {
		GeneratorU64::next(txn, &COLUMN_KEY, Some(8193)).map(ColumnId)
	}

	pub(crate) fn next_column_policy_id(txn: &mut StandardCommandTransaction) -> crate::Result<ColumnPolicyId> {
		GeneratorU64::next(txn, &COLUMN_POLICY_KEY, Some(1025)).map(ColumnPolicyId)
	}

	pub(crate) fn next_view_id(txn: &mut StandardCommandTransaction) -> crate::Result<ViewId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).map(ViewId)
	}

	pub(crate) fn next_primary_key_id(txn: &mut StandardCommandTransaction) -> crate::Result<PrimaryKeyId> {
		GeneratorU64::next(txn, &PRIMARY_KEY_KEY, None).map(PrimaryKeyId)
	}

	pub(crate) fn next_ringbuffer_id(txn: &mut StandardCommandTransaction) -> crate::Result<RingBufferId> {
		GeneratorU64::next(txn, &SOURCE_KEY, Some(1025)).map(RingBufferId)
	}

	pub(crate) fn next_dictionary_id(txn: &mut StandardCommandTransaction) -> crate::Result<DictionaryId> {
