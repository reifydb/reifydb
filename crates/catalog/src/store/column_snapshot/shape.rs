...
use postcard::{from_bytes, to_allocvec, to_extend};
...
pub(crate) fn serialize_partition_values(values: &[Value]) -> Blob {
	Blob::from(to_allocvec(values).expect("postcard serialization of a Value is total"))
}
...
pub(crate) fn deserialize_partition_values(blob: &Blob) -> Vec<Value> {
	...
	from_bytes(bytes).expect("stored column snapshot partition values must decode")
}

pub(crate) fn serialize_stats(stats: &[ColumnStats]) -> Blob {
	let mut bytes = vec![STATS_ENCODING_V1];
	bytes = to_extend(stats, bytes).expect("postcard serialization of ColumnStats is total");
	Blob::from(bytes)
}
...
pub(crate) fn deserialize_stats(blob: &Blob) -> Vec<ColumnStats> {
	...
	from_bytes(rest).expect("stored column snapshot stats must decode")
}
...
pub mod tests {
	...
	fn test_stats_with_unknown_version_panics() {
		...
		deserialize_stats(&Blob::from(vec![2u8, 0u8]));
	}
}
