...
impl CatalogStore {
	pub fn find_flow(rx: &mut impl QueryTransaction, id: FlowId) -> crate::Result<Option<FlowDef>> {
		...
		let query = flow::LAYOUT.get_utf8(&row, flow::QUERY).to_string();
...
mod tests {
	...
	fn test_find_flow_by_name_ok() {
		...
		create_flow(&mut txn, "namespace_one", "flow_one", "MAP 1", &[]);
		create_flow(&mut txn, "namespace_two", "flow_two", "MAP 2", &[]);
	...
	fn test_find_flow_by_name_not_found() {
		...
		create_flow(&mut txn, "test_namespace", "flow_one", "MAP 1", &[]);
		create_flow(&mut txn, "test_namespace", "flow_two", "MAP 2", &[]);
	...
	fn test_find_flow_by_name_different_namespace() {
		...
		create_flow(&mut txn, "namespace_one", "my_flow", "MAP 1", &[]);
	...
	fn test_find_flow_by_name_case_sensitive() {
		...
		create_flow(&mut txn, "test_namespace", "MyFlow", "MAP 1", &[]);
	...
	fn test_find_flow_by_id() {
		...
		let flow = create_flow(&mut txn, "test_namespace", "test_flow", "SELECT 42", &[]);
		...
		assert_eq!(result.query, "SELECT 42");
	}
