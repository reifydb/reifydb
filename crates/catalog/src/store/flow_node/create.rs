...
impl crate::CatalogStore {
	pub async fn create_flow_node(txn: &mut impl CommandTransaction, node_def: &FlowNodeDef) -> crate::Result<()> {
		...
		txn.set(&FlowNodeKey::encoded(node_def.id), row).await?;
		...
		txn.set(&FlowNodeByFlowKey::encoded(node_def.flow, node_def.id), index_row).await?;
...
mod tests {
	...
	async fn test_create_flow_node() {
		...
		let node_id = next_flow_node_id(&mut txn).await.unwrap();
		...
		CatalogStore::create_flow_node(&mut txn, &node_def).await.unwrap();

		// Verify node was created
		let result = CatalogStore::get_flow_node(&mut txn, node_id).await.unwrap();
	...
	async fn test_create_multiple_nodes_same_flow() {
		...
		// Create first node
		let node1_id = next_flow_node_id(&mut txn).await.unwrap();
		...
		CatalogStore::create_flow_node(&mut txn, &node1).await.unwrap();

		// Create second node
		let node2_id = next_flow_node_id(&mut txn).await.unwrap();
		...
		CatalogStore::create_flow_node(&mut txn, &node2).await.unwrap();

		// Verify both nodes exist
		let result1 = CatalogStore::get_flow_node(&mut txn, node1_id).await.unwrap();
		let result2 = CatalogStore::get_flow_node(&mut txn, node2_id).await.unwrap();
	...
	async fn test_create_nodes_different_flows() {
		...
		// Create two flows
		let flow1 = crate::test_utils::create_flow(&mut txn, "test_namespace", "flow_one").await;
		let flow2 = crate::test_utils::create_flow(&mut txn, "test_namespace", "flow_two").await;
