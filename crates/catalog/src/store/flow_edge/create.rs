...
impl crate::CatalogStore {
	pub async fn create_flow_edge(txn: &mut impl CommandTransaction, edge_def: &FlowEdgeDef) -> crate::Result<()> {
		...
		txn.set(&FlowEdgeKey::encoded(edge_def.id), row).await?;
		...
		txn.set(&FlowEdgeByFlowKey::encoded(edge_def.flow, edge_def.id), index_row).await?;
...
mod tests {
	...
	async fn test_create_multiple_edges_same_flow() {
		...
		let result1 = CatalogStore::get_flow_edge(&mut txn, edge1.id).await.unwrap();
		let result2 = CatalogStore::get_flow_edge(&mut txn, edge2.id).await.unwrap();
	...
	async fn test_create_edges_different_flows() {
		...
		let node1a = create_flow_node(&mut txn, flow1.id, 1, &[0x01]).await;
		let node1b = create_flow_node(&mut txn, flow1.id, 4, &[0x02]).await;
		let node2a = create_flow_node(&mut txn, flow2.id, 1, &[0x03]).await;
		let node2b = create_flow_node(&mut txn, flow2.id, 4, &[0x04]).await;

		let edge1 = create_flow_edge(&mut txn, flow1.id, node1a.id, node1b.id).await;
		let edge2 = create_flow_edge(&mut txn, flow2.id, node2a.id, node2b.id).await;
