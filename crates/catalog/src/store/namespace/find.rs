...
use reifydb_codec::row::{bytes::EncodedBytes, catalog::EncodedCatalogRow};
use reifydb_core::{
...
impl CatalogStore {
	pub(crate) fn find_namespace_by_name(
		...
		let stream = rx.range(NamespaceKey::full_scan().encode(), RangeScope::All, 1024)?;

		for entry in stream {
			let multi = entry?;
			let bytes: &EncodedBytes = &multi.bytes;
			let namespace_name = namespace::get_name(EncodedCatalogRow::view(bytes));
