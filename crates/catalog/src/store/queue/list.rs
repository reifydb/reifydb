...
pub mod tests {
	...
	fn test_list_queues_empty() {
		...
		let queues = CatalogStore::list_queues(&mut Transaction::Admin(&mut txn)).unwrap();
	...
	fn test_list_queues_filters_by_namespace() {
		...
		let all = CatalogStore::list_queues(&mut Transaction::Admin(&mut txn)).unwrap();
		assert_eq!(all.len(), 2);

		let scoped =
			CatalogStore::list_queues_in_namespace(&mut Transaction::Admin(&mut txn), first.id()).unwrap();
