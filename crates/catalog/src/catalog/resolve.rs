...
impl Catalog {
	...
	pub async fn resolve_namespace<T: IntoStandardTransaction>(
		&self,
		txn: &mut T,
		namespace_id: NamespaceId,
	) -> crate::Result<ResolvedNamespace> {
		let def = self.get_namespace(txn, namespace_id).await?;
	...
	pub async fn resolve_table<T: IntoStandardTransaction>(
		&self,
		txn: &mut T,
		table_id: TableId,
	) -> crate::Result<ResolvedTable> {
		let table_def = self.get_table(txn, table_id).await?;
		let resolved_namespace = self.resolve_namespace(txn, table_def.namespace).await?;
	...
	pub async fn resolve_view<T: IntoStandardTransaction>(
		&self,
		txn: &mut T,
		view_id: ViewId,
	) -> crate::Result<ResolvedView> {
		let view_def = self.get_view(txn, view_id).await?;
		let resolved_namespace = self.resolve_namespace(txn, view_def.namespace).await?;
	...
	pub async fn resolve_flow<T: IntoStandardTransaction>(
		&self,
		txn: &mut T,
		flow_id: FlowId,
	) -> crate::Result<ResolvedFlow> {
		let flow_def = self.get_flow(txn, flow_id).await?;
		let resolved_namespace = self.resolve_namespace(txn, flow_def.namespace).await?;
	...
	pub async fn resolve_ringbuffer<T: IntoStandardTransaction>(
		&self,
		txn: &mut T,
		ringbuffer_id: RingBufferId,
	) -> crate::Result<ResolvedRingBuffer> {
		let ringbuffer_def = self.get_ringbuffer(txn, ringbuffer_id).await?;
		let resolved_namespace = self.resolve_namespace(txn, ringbuffer_def.namespace).await?;
