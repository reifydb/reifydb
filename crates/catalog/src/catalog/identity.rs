...
impl Catalog {
	...
	pub fn find_identity_by_name(&self, txn: &mut Transaction<'_>, name: &str) -> Result<Option<IdentityDef>> {
		match txn.reborrow() {
			...
			Transaction::Test(mut t) => {
				...
				if let Some(ident) =
					CatalogStore::find_identity_by_name(&mut Transaction::Test(t.reborrow()), name)?
				{
					return Ok(Some(ident));
				}
	...
	pub fn find_identity(&self, txn: &mut Transaction<'_>, identity: IdentityId) -> Result<Option<IdentityDef>> {
		match txn.reborrow() {
			...
			Transaction::Test(mut t) => {
				if let Some(ident) =
					CatalogStore::find_identity(&mut Transaction::Test(t.reborrow()), identity)?
				{
					return Ok(Some(ident));
				}
	...
	pub fn find_role_by_name(&self, txn: &mut Transaction<'_>, name: &str) -> Result<Option<RoleDef>> {
		match txn.reborrow() {
			...
			Transaction::Test(mut t) => {
				...
				if let Some(role) =
					CatalogStore::find_role_by_name(&mut Transaction::Test(t.reborrow()), name)?
				{
					return Ok(Some(role));
				}
