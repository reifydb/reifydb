...
impl Catalog {
	...
	pub fn find_column_by_name<T: AsTransaction>(
		&self,
		txn: &mut T,
	...
	pub fn get_column<T: AsTransaction>(&self, txn: &mut T, column_id: ColumnId) -> crate::Result<ColumnDef> {
		CatalogStore::get_column(txn, column_id)
	}

	#[instrument(name = "catalog::column::list_all", level = "debug", skip(self, txn))]
	pub fn list_columns_all<T: AsTransaction>(&self, txn: &mut T) -> crate::Result<Vec<ColumnInfo>> {
