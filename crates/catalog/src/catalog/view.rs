...
use reifydb_core::interface::{NamespaceId, QueryTransaction, TransactionalViewChanges, ViewDef, ViewId};
	...
	pub async fn find_view<T: IntoStandardTransaction>(
		...
		match txn.into_standard_transaction() {
			StandardTransaction::Command(cmd) => {
				...
				if let Some(view) = self.materialized.find_view(id, QueryTransaction::version(cmd)) {
					return Ok(Some(view));
				}
			...
			StandardTransaction::Query(qry) => {
				...
				if let Some(view) = self.materialized.find_view(id, QueryTransaction::version(qry)) {
					return Ok(Some(view));
				}
	...
	pub async fn find_view_by_name<T: IntoStandardTransaction>(
		...
		match txn.into_standard_transaction() {
			StandardTransaction::Command(cmd) => {
				...
				// 3. Check MaterializedCatalog
				if let Some(view) = self.materialized.find_view_by_name(
					namespace,
					name,
