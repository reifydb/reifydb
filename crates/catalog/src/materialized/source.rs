...
pub mod tests {
	...
	fn test_set_and_find_source() {
		...
		let source = create_test_source(source_id, namespace_id, "test_source");

		// Set source at version 1
		catalog.set_source(source_id, CommitVersion(1), Some(source.clone()));

		// Find source at version 1
		let found = catalog.find_source_at(source_id, CommitVersion(1));
		assert_eq!(found, Some(source.clone()));

		// Find source at later version (should return same source)
		let found = catalog.find_source_at(source_id, CommitVersion(5));
		assert_eq!(found, Some(source));

		// Source shouldn't exist at version 0
		let found = catalog.find_source_at(source_id, CommitVersion(0));
	...
	fn test_find_source_by_name() {
		...
		let source_id = SourceId(1);
		let namespace_id = NamespaceId::SYSTEM;
		let source = create_test_source(source_id, namespace_id, "named_source");
	...
	fn test_source_deletion() {
		...
		let source = create_test_source(source_id, namespace_id, "deletable_source");
		catalog.set_source(source_id, CommitVersion(1), Some(source.clone()));

		// Verify it exists
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(1)), Some(source.clone()));
		assert!(catalog.find_source_by_name_at(namespace_id, "deletable_source", CommitVersion(1)).is_some());

		// Delete the source
		catalog.set_source(source_id, CommitVersion(2), None);

		// Should not exist at version 2
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(2)), None);
		...
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(1)), Some(source));
	}
	...
	fn test_source_versioning() {
		...
		let source_v1 = create_test_source(source_id, namespace_id, "source_v1");
		...
		catalog.set_source(source_id, CommitVersion(10), Some(source_v1.clone()));
		catalog.set_source(source_id, CommitVersion(20), Some(source_v2.clone()));
		catalog.set_source(source_id, CommitVersion(30), Some(source_v3.clone()));

		// Query at different versions
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(5)), None);
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(10)), Some(source_v1.clone()));
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(15)), Some(source_v1));
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(20)), Some(source_v2.clone()));
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(25)), Some(source_v2));
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(30)), Some(source_v3.clone()));
		assert_eq!(catalog.find_source_at(source_id, CommitVersion(100)), Some(source_v3));
	}
}
