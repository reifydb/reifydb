...
mod tests {
	...
	fn test_set_and_find_table() {
		...
		let found = catalog.find_table(table_id, CommitVersion(1));
		assert_eq!(found, Some(table.clone()));

		// Find table at later version (should return same table)
		let found = catalog.find_table(table_id, CommitVersion(5));
		assert_eq!(found, Some(table));

		// Table shouldn't exist at version 0
		let found = catalog.find_table(table_id, CommitVersion(0));
	...
	fn test_find_table_by_name() {
		...
		// Find by name
		let found = catalog.find_table_by_name(namespace_id, "named_table", CommitVersion(1));
		assert_eq!(found, Some(table));

		// Shouldn't find with wrong name
		let found = catalog.find_table_by_name(namespace_id, "wrong_name", CommitVersion(1));
		assert_eq!(found, None);

		// Shouldn't find in wrong namespace
		let found = catalog.find_table_by_name(NamespaceId(2), "named_table", CommitVersion(1));
		assert_eq!(found, None);
	}
	...
	fn test_table_rename() {
		...
		assert!(catalog.find_table_by_name(namespace_id, "old_name", CommitVersion(1)).is_some());
		assert!(catalog.find_table_by_name(namespace_id, "new_name", CommitVersion(1)).is_none());
		...
		assert!(catalog.find_table_by_name(namespace_id, "old_name", CommitVersion(2)).is_none());

		// New name can be found
		assert_eq!(
			catalog.find_table_by_name(namespace_id, "new_name", CommitVersion(2)),
			Some(table_v2.clone())
		);

		// Historical query at version 1 should still show old name
		assert_eq!(catalog.find_table(table_id, CommitVersion(1)), Some(table_v1));
	...
	fn test_table_move_between_namespaces() {
		...
		assert!(catalog.find_table_by_name(namespace1, "movable_table", CommitVersion(1)).is_some());
		assert!(catalog.find_table_by_name(namespace2, "movable_table", CommitVersion(1)).is_none());
		...
		assert!(catalog.find_table_by_name(namespace1, "movable_table", CommitVersion(2)).is_none());
	...
	fn test_table_deletion() {
		...
		// Verify it exists
		assert_eq!(catalog.find_table(table_id, CommitVersion(1)), Some(table.clone()));
		assert!(catalog.find_table_by_name(namespace_id, "deletable_table", CommitVersion(1)).is_some());

		// Delete the table
		catalog.set_table(table_id, CommitVersion(2), None);

		// Should not exist at version 2
		assert_eq!(catalog.find_table(table_id, CommitVersion(2)), None);
		assert!(catalog.find_table_by_name(namespace_id, "deletable_table", CommitVersion(2)).is_none());
	...
	fn test_multiple_tables_in_namespace() {
		...
		assert_eq!(catalog.find_table_by_name(namespace_id, "table1", CommitVersion(1)), Some(table1));
		assert_eq!(catalog.find_table_by_name(namespace_id, "table2", CommitVersion(1)), Some(table2));
	...
	fn test_table_versioning() {
		...
		assert_eq!(catalog.find_table(table_id, CommitVersion(5)), None);
		assert_eq!(catalog.find_table(table_id, CommitVersion(10)), Some(table_v1.clone()));
		assert_eq!(catalog.find_table(table_id, CommitVersion(15)), Some(table_v1));
		assert_eq!(catalog.find_table(table_id, CommitVersion(20)), Some(table_v2.clone()));
		assert_eq!(catalog.find_table(table_id, CommitVersion(25)), Some(table_v2));
		assert_eq!(catalog.find_table(table_id, CommitVersion(30)), Some(table_v3.clone()));
		assert_eq!(catalog.find_table(table_id, CommitVersion(100)), Some(table_v3));
	}
}
