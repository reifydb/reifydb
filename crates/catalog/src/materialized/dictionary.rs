...
pub mod tests {
	...
	fn test_set_and_find_dictionary() {
		...
		let found = catalog.find_dictionary_at(dict_id, CommitVersion(1));
		assert_eq!(found, Some(dict.clone()));

		// Find dictionary at later version (should return same dictionary)
		let found = catalog.find_dictionary_at(dict_id, CommitVersion(5));
		assert_eq!(found, Some(dict));

		// Dictionary shouldn't exist at version 0
		let found = catalog.find_dictionary_at(dict_id, CommitVersion(0));
	...
	fn test_find_dictionary_by_name() {
		...
		// Find by name
		let found = catalog.find_dictionary_by_name_at(namespace_id, "named_dict", CommitVersion(1));
		assert_eq!(found, Some(dict));

		// Shouldn't find with wrong name
		let found = catalog.find_dictionary_by_name_at(namespace_id, "wrong_name", CommitVersion(1));
		assert_eq!(found, None);
	...
	fn test_dictionary_rename() {
		...
		assert!(catalog.find_dictionary_by_name_at(namespace_id, "old_name", CommitVersion(1)).is_some());
		assert!(catalog.find_dictionary_by_name_at(namespace_id, "new_name", CommitVersion(1)).is_none());
		...
		assert!(catalog.find_dictionary_by_name_at(namespace_id, "old_name", CommitVersion(2)).is_none());

		// New name can be found
		assert_eq!(
			catalog.find_dictionary_by_name_at(namespace_id, "new_name", CommitVersion(2)),
			Some(dict_v2.clone())
		);

		// Historical query at version 1 should still show old name
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(1)), Some(dict_v1));

		// Current version should show new name
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(2)), Some(dict_v2));
	}
	...
	fn test_dictionary_move_between_namespaces() {
		...
		assert!(catalog.find_dictionary_by_name_at(namespace1, "movable_dict", CommitVersion(1)).is_some());
		assert!(catalog.find_dictionary_by_name_at(namespace2, "movable_dict", CommitVersion(1)).is_none());
		...
		assert!(catalog.find_dictionary_by_name_at(namespace1, "movable_dict", CommitVersion(2)).is_none());

		// Should now be in namespace2
		assert!(catalog.find_dictionary_by_name_at(namespace2, "movable_dict", CommitVersion(2)).is_some());
	}
	...
	fn test_dictionary_deletion() {
		...
		// Verify it exists
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(1)), Some(dict.clone()));
		assert!(catalog.find_dictionary_by_name_at(namespace_id, "deletable_dict", CommitVersion(1)).is_some());

		// Delete the dictionary
		catalog.set_dictionary(dict_id, CommitVersion(2), None);

		// Should not exist at version 2
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(2)), None);
		assert!(catalog.find_dictionary_by_name_at(namespace_id, "deletable_dict", CommitVersion(2)).is_none());

		// Should still exist at version 1 (historical)
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(1)), Some(dict));
	}
	...
	fn test_multiple_dictionaries_in_namespace() {
		...
		assert_eq!(catalog.find_dictionary_by_name_at(namespace_id, "dict1", CommitVersion(1)), Some(dict1));
		assert_eq!(catalog.find_dictionary_by_name_at(namespace_id, "dict2", CommitVersion(1)), Some(dict2));
		assert_eq!(catalog.find_dictionary_by_name_at(namespace_id, "dict3", CommitVersion(1)), Some(dict3));
	}
	...
	fn test_dictionary_versioning() {
		...
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(5)), None);
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(10)), Some(dict_v1.clone()));
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(15)), Some(dict_v1));
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(20)), Some(dict_v2.clone()));
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(25)), Some(dict_v2));
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(30)), Some(dict_v3.clone()));
		assert_eq!(catalog.find_dictionary_at(dict_id, CommitVersion(100)), Some(dict_v3));
	}
}
