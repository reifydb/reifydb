...
use crate::materialized::{MaterializedCatalog, MultiVersionTestDef};

impl MaterializedCatalog {
	/// Find a test by ID at a specific version
	pub fn find_test_at(&self, test: TestId, version: CommitVersion) -> Option<TestDef> {
	...
	pub fn list_tests_in_namespace_at(&self, namespace: NamespaceId, version: CommitVersion) -> Vec<TestDef> {
		self.tests_by_name
			.iter()
	...
	pub fn list_all_tests_at(&self, version: CommitVersion) -> Vec<TestDef> {
		self.tests.iter().filter_map(|entry| entry.value().get(version)).collect()
	}

	pub fn set_test(&self, id: TestId, version: CommitVersion, test: Option<TestDef>) {
		...
		let multi = self.tests.get_or_insert_with(id, MultiVersionTestDef::new);
