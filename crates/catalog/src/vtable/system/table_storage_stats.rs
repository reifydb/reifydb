...
use reifydb_metric::{MetricId, storage::metric::MetricReader};
...
/// Virtual table that exposes storage statistics for tables
pub struct SystemTableStorageStats {
	pub(crate) vtable: Arc<VTable>,
	exhausted: bool,
	stats_reader: MetricReader<SingleStore>,
}

impl SystemTableStorageStats {
	pub fn new(stats_reader: MetricReader<SingleStore>) -> Self {
		Self {
			vtable: SystemCatalog::get_system_table_storage_stats_table().clone(),
			exhausted: false,
			stats_reader,
		}
	}
...
impl BaseVTable for SystemTableStorageStats {
	...
	fn next(&mut self, txn: &mut Transaction<'_>) -> Result<Option<Batch>> {
		...
		// Collect all table stats across all tiers
