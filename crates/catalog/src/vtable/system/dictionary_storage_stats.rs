...
pub struct SystemDictionaryStorageStats {
	pub(crate) vtable: Arc<VTable>,
	exhausted: bool,
	stats_reader: MetricReader<SingleStore>,
}

impl SystemDictionaryStorageStats {
	pub fn new(stats_reader: MetricReader<SingleStore>) -> Self {
		Self {
			vtable: SystemCatalog::get_system_dictionary_storage_stats_table().clone(),
			exhausted: false,
			stats_reader,
		}
	}
...
impl BaseVTable for SystemDictionaryStorageStats {
	...
	fn next(&mut self, txn: &mut Transaction<'_>) -> Result<Option<Batch>> {
		...
		for tier in [Tier::Hot, Tier::Warm, Tier::Cold] {
			...
			for (obj_id, stats) in tier_stats {
				...
				if let MetricId::Shape(ShapeId::Dictionary(dictionary_id)) = obj_id {
