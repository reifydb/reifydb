...
fn all_system_vtables() -> Vec<Box<dyn BaseVTable>> {
	...
	let result: Vec<Box<dyn BaseVTable>> = vec![
		...
		Box::new(SystemProceduresInProcess::new(catalog.clone())),
		Box::new(SystemProceduresExternC::new(catalog.clone())),
		Box::new(SystemProceduresExternWasm::new(catalog.clone())),
