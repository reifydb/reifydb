...
pub enum VTables {
	...
	SchemaRetentionPolicies(SystemSchemaRetentionPolicies),
...
impl VTables {
	...
	pub fn definition(&self) -> &VTable {
		match self {
			...
			Self::SchemaRetentionPolicies(t) => &t.definition,
	...
	pub fn initialize(&mut self, txn: &mut Transaction<'_>, ctx: VTableContext) -> Result<()> {
		match self {
			...
			Self::SchemaRetentionPolicies(t) => t.initialize(txn, ctx),
	...
	pub fn next(&mut self, txn: &mut Transaction<'_>) -> Result<Option<Batch>> {
		match self {
			...
			Self::SchemaRetentionPolicies(t) => t.next(txn),
