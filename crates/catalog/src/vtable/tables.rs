...
pub enum VTables {
	...
	Operators(SystemOperators),
	OperatorInputs(SystemOperatorInputs),
	OperatorOutputs(SystemOperatorOutputs),
	Dictionaries(SystemDictionaries),
...
impl VTables {
	pub fn vtable(&self) -> &VTable {
		match self {
			...
			Self::OperatorInputs(t) => &t.vtable,
			Self::OperatorOutputs(t) => &t.vtable,
	...
	pub fn initialize(&mut self, txn: &mut Transaction<'_>, ctx: VTableContext) -> Result<()> {
		match self {
			...
			Self::OperatorInputs(t) => t.initialize(txn, ctx),
			Self::OperatorOutputs(t) => t.initialize(txn, ctx),
	...
	pub fn next(&mut self, txn: &mut Transaction<'_>) -> Result<Option<Batch>> {
		match self {
			...
			Self::OperatorInputs(t) => t.next(txn),
			Self::OperatorOutputs(t) => t.next(txn),
