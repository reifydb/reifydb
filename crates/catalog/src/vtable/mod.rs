...
impl VTableRegistry {
	pub fn find_vtable(_rx: &mut Transaction<'_>, id: VTableId) -> Result<Option<Arc<VTable>>> {
		Ok(match id {
			...
			OPERATOR_INPUTS => Some(SystemCatalog::get_system_operator_inputs_table()),
			OPERATOR_OUTPUTS => Some(SystemCatalog::get_system_operator_outputs_table()),
	...
	pub fn list_vtables(_rx: &mut Transaction<'_>) -> Result<Vec<Arc<VTable>>> {
		Ok(vec![
			...
			SystemCatalog::get_system_operator_inputs_table(),
			SystemCatalog::get_system_operator_outputs_table(),
