...
use reifydb_core::interface::{
	CommandTransaction, EncodableKey, NamespaceDef, NamespaceKey,
};
use reifydb_type::{
	OwnedFragment, diagnostic::catalog::namespace_already_exists,
	return_error,
};

use crate::{
	CatalogStore, namespace::layout::namespace, sequence::SystemSequence,
};
...
impl CatalogStore {
	pub fn create_namespace(
		...
		if let Some(namespace) =
			Self::find_namespace_by_name(txn, &to_create.name)?
		{
		...
		namespace::LAYOUT.set_u64(
			&mut row,
			namespace::ID,
			namespace_id,
		);
		namespace::LAYOUT.set_utf8(
			&mut row,
			namespace::NAME,
			&to_create.name,
		);
...
mod tests {
	...
	use crate::{CatalogStore, namespace::create::NamespaceToCreate};
	...
	fn test_create_namespace() {
		...
		let err = CatalogStore::create_namespace(&mut txn, to_create)
			.unwrap_err();
