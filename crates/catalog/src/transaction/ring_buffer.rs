...
impl<QT: QueryTransaction + MaterializedCatalogTransaction> CatalogRingBufferQueryOperations for QT {
	...
	fn get_ring_buffer_by_name<'a>(
		...
		let ring_buffer = self.find_ring_buffer_by_name(namespace, name.clone())?;
		match ring_buffer {
			Some(rb) => Ok(rb),
			None => {
				let namespace = CatalogStore::get_namespace(self, namespace)?;
				return_error!(ring_buffer_not_found(name, &namespace.name, &name_text))
			}
		}
	}
}
...
pub trait CatalogTrackRingBufferChangeOperations {
	...
	fn track_ring_buffer_def_created(&mut self, ring_buffer: RingBufferDef) -> crate::Result<()>;

	fn track_ring_buffer_def_updated(&mut self, pre: RingBufferDef, post: RingBufferDef) -> crate::Result<()>;

	fn track_ring_buffer_def_deleted(&mut self, ring_buffer: RingBufferDef) -> crate::Result<()>;
}
