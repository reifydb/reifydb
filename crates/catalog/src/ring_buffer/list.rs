...
use reifydb_core::interface::{
	Key, NamespaceId, QueryTransaction, RingBufferDef, RingBufferKey,
};

use crate::{CatalogStore, ring_buffer::layout::ring_buffer};

impl CatalogStore {
	pub fn list_ring_buffers_all(
		rx: &mut impl QueryTransaction,
	) -> crate::Result<Vec<RingBufferDef>> {
		let mut result = Vec::new();

		let entries: Vec<_> = rx
			.range(RingBufferKey::full_scan())?
			.into_iter()
			.collect();
		...
		for entry in entries {
			if let Some(key) = Key::decode(&entry.key) {
				if let Key::RingBuffer(ring_buffer_key) = key {
					let ring_buffer_id =
						ring_buffer_key.ring_buffer;

					let namespace_id = NamespaceId(
						ring_buffer::LAYOUT.get_u64(
							&entry.row,
...
mod tests {
	...
	fn test_list_ring_buffers_empty() {
		...
		let buffers =
			CatalogStore::list_ring_buffers_all(&mut txn).unwrap();
	...
	fn test_list_ring_buffers_different_namespaces() {
		...
		let buffer1_entry = all_buffers
			.iter()
			.find(|b| b.name == "buffer1")
			.expect("buffer1 should exist");
		assert_eq!(buffer1_entry.namespace, namespace1.id);

		// Check that buffer2 is in namespace2
		let buffer2_entry = all_buffers
			.iter()
			.find(|b| b.name == "buffer2")
			.expect("buffer2 should exist");
