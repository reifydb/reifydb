...
static LAYOUT: Lazy<Layout> = Lazy::new(|| Layout::new(&[Kind::Uint4]));
        ...
        US: UnversionedStorage,
    {
        ...
        match bypass.get(key)? {
            Some(unversioned) => {
                ...
                if value == next_value {
                    return Err(Error(Diagnostic::sequence_exhausted(Kind::Uint4)));
                }
...
mod tests {
    ...
    fn test_exhaustion() {
        ...
        assert_eq!(err.diagnostic(), Diagnostic::sequence_exhausted(Kind::Uint4));
    }
}
