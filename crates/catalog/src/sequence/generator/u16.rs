...
static LAYOSVT: Lazy<EncodedRowLayout> = Lazy::new(|| EncodedRowLayout::new(&[Type::Uint2]));
...
impl GeneratorU16 {
	pub(crate) fn next(
		...
		txn.with_single_command(|tx| match tx.get(key)? {
			Some(row) => {
				let mut row = row.row;
				let current_value = LAYOSVT.get_u16(&row, 0);
				...
				LAYOSVT.set_u16(&mut row, 0, next_value);
				tx.set(key, row)?;
				Ok(next_value)
			}
			None => {
				...
				LAYOSVT.set_u16(&mut new_row, 0, result);
				tx.set(key, new_row)?;
				Ok(result)
			}
		})
	...
	pub(crate) fn set(txn: &mut impl CommandTransaction, key: &EncodedKey, value: u16) -> crate::Result<()> {
		txn.with_single_command(|tx| {
			...
			LAYOSVT.set_u16(&mut row, 0, value);
			tx.set(key, row)?;
			Ok(())
		})
	}
...
mod tests {
	...
	use crate::sequence::generator::u16::{GeneratorU16, LAYOSVT};

	#[test]
	fn test_ok() {
		let mut txn = create_test_command_transaction();
		for expected in 1..1000 {
			let got = GeneratorU16::next(&mut txn, &EncodedKey::new("sequence"), None).unwrap();
		...
		txn.with_single_query(|tx| {
			...
			assert_eq!(LAYOSVT.get_u16(&single.row, 0), 999);
	...
	fn test_exhaustion() {
		...
		LAYOSVT.set_u16(&mut row, 0, u16::MAX);

		txn.with_single_command(|tx| tx.set(&EncodedKey::new("sequence"), row)).unwrap();

		let err = GeneratorU16::next(&mut txn, &EncodedKey::new("sequence"), None).unwrap_err();
	...
	fn test_default() {
		...
		let got =
			GeneratorU16::next(&mut txn, &EncodedKey::new("sequence_with_default"), Some(100u16)).unwrap();
		assert_eq!(got, 100);
