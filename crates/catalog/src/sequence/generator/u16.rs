        ...
        UT: UnversionedTransaction,
    {
        atx.with_unversioned_write(|tx| match tx.get(key)? {
            Some(unversioned_row) => {
                ...
                let value = LAYOUT.get_u16(&row, 0);
            ...
            None => {
                ...
                LAYOUT.set_u16(&mut new_row, 0, 2u16);
                tx.set(key, new_row)?;
                Ok(1)
            }
        })
...
mod tests {
    ...
    fn test_ok() {
        ...
        for expected in 1..1000 {
            let got = GeneratorU16::next(&mut atx, &EncodedKey::new("sequence")).unwrap();
        ...
        atx.with_unversioned_read(|tx| {
            ...
            assert_eq!(LAYOUT.get_u16(&unversioned.row, 0), 1000);
    ...
    fn test_exhaustion() {
        ...
        let err = GeneratorU16::next(&mut atx, &EncodedKey::new("sequence")).unwrap_err();
