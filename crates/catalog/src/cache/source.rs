...
impl CatalogCache {
	...
	pub fn find_source_by_name_at(
		...
		self.sources_by_name.get(&(namespace, name.to_string())).and_then(|entry| {
			let object_id = *entry.value();
			self.find_source_at(object_id, version)
		})
	}
...
pub mod tests {
	...
	fn test_source_versioning() {
		...
		assert_eq!(catalog.find_source_at(object_id, CommitVersion(20)), Some(source_v2.clone()));
		assert_eq!(catalog.find_source_at(object_id, CommitVersion(25)), Some(source_v2));
		assert_eq!(catalog.find_source_at(object_id, CommitVersion(30)), Some(source_v3.clone()));
		assert_eq!(catalog.find_source_at(object_id, CommitVersion(100)), Some(source_v3));
	}
}
