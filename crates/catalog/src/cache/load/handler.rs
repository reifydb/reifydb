...
use reifydb_core::key::{any::AnyKey, catalog::HandlerKey};
...
pub(crate) fn load_handlers(rx: &mut Transaction<'_>, catalog: &CatalogCache) -> Result<()> {
	...
	{
		let stream = rx.range(HandlerKey::full_scan().encode(), RangeScope::All, 1024)?;
		for entry in stream {
			let entry = entry?;
			let version = entry.version;
			if let AnyKey::Handler(k) = &entry.key {
