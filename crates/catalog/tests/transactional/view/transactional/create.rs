...
fn uncommitted_create_is_visible_within_txn() {
	...
	let r = txn.rql(
		"CREATE TRANSACTIONAL VIEW vns_create_a::v { id: int4 } AS { FROM vns_create_a::src MAP { id: id } }",
		Params::None,
	);
...
fn rolled_back_create_is_not_visible() {
	...
	let mut txn = t.begin_admin(IdentityId::system()).unwrap();
	let r = txn.rql(
		"CREATE TRANSACTIONAL VIEW vns_create_b::v { id: int4 } AS { FROM vns_create_b::src MAP { id: id } }",
		Params::None,
	);
	assert!(r.error.is_none(), "create failed: {:?}", r.error);
	txn.rollback().unwrap();
...
fn committed_create_is_visible_in_new_txn() {
	...
	let mut txn = t.begin_admin(IdentityId::system()).unwrap();
	let r = txn.rql(
		"CREATE TRANSACTIONAL VIEW vns_create_c::v { id: int4 } AS { FROM vns_create_c::src MAP { id: id } }",
		Params::None,
	);
	assert!(r.error.is_none(), "create failed: {:?}", r.error);
	txn.commit().unwrap();
...
fn uncommitted_create_is_isolated_from_concurrent_txn() {
	...
	let r = txn1.rql(
		"CREATE TRANSACTIONAL VIEW vns_create_d::v { id: int4 } AS { FROM vns_create_d::src MAP { id: id } }",
		Params::None,
	);
	assert!(r.error.is_none(), "create failed: {:?}", r.error);
