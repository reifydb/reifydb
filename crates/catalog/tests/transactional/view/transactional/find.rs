...
fn create_and_drop_in_same_txn_reflects_both() {
	...
	t.admin("CREATE TRANSACTIONAL VIEW vwf_ns_a::keep { id: int4 } AS { FROM vwf_ns_a::src MAP { id: id } }");
	...
	txn.rql(
		"CREATE TRANSACTIONAL VIEW vwf_ns_a::new { id: int4 } AS { FROM vwf_ns_a::src MAP { id: id } }",
		Params::None,
	);
...
fn rolled_back_create_and_drop_leave_committed_state_intact() {
	...
	t.admin("CREATE TRANSACTIONAL VIEW vwf_ns_b::keep { id: int4 } AS { FROM vwf_ns_b::src MAP { id: id } }");
	...
	let mut txn = t.begin_admin(IdentityId::system()).unwrap();
	txn.rql(
		"CREATE TRANSACTIONAL VIEW vwf_ns_b::new { id: int4 } AS { FROM vwf_ns_b::src MAP { id: id } }",
		Params::None,
	);
...
fn committed_create_and_drop_are_reflected_in_new_txn() {
	...
	t.admin("CREATE TRANSACTIONAL VIEW vwf_ns_c::keep { id: int4 } AS { FROM vwf_ns_c::src MAP { id: id } }");
	...
	let mut txn = t.begin_admin(IdentityId::system()).unwrap();
	txn.rql(
		"CREATE TRANSACTIONAL VIEW vwf_ns_c::new { id: int4 } AS { FROM vwf_ns_c::src MAP { id: id } }",
		Params::None,
	);
...
fn concurrent_txn_sees_only_committed_state() {
	...
	t.admin("CREATE TRANSACTIONAL VIEW vwf_ns_d::keep { id: int4 } AS { FROM vwf_ns_d::src MAP { id: id } }");
	...
	txn1.rql(
		"CREATE TRANSACTIONAL VIEW vwf_ns_d::new { id: int4 } AS { FROM vwf_ns_d::src MAP { id: id } }",
		Params::None,
	);
