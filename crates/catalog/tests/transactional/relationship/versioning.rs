...
/// Drives MaterializedCatalog::set_relationship directly to confirm:
...
fn rename_does_not_leak_old_name_in_materialized_index() {
	...
	// Drive a rename directly through the materialized cache (no rename API yet).
	let v2 = CommitVersion(v_create.0 + 1_000_000);
	let renamed = Relationship {
		name: "manages".to_string(),
		..v1_rel.clone()
	};
	catalog.materialized.set_relationship(v1_rel.id, v2, Some(renamed.clone()));

	// Old name must no longer resolve at v2.
	assert!(catalog.materialized.find_relationship_by_name_at(f.namespace, f.source_table, "owns", v2).is_none());

	// New name resolves at v2.
	let by_new = catalog.materialized.find_relationship_by_name_at(f.namespace, f.source_table, "manages", v2);
...
fn historical_query_returns_old_version() {
	...
	catalog.materialized.set_relationship(v1_rel.id, v_late, Some(renamed));

	// At v_create we see the original name.
	let early_view = catalog.materialized.find_relationship_at(v1_rel.id, v_create);
	let late_view = catalog.materialized.find_relationship_at(v1_rel.id, v_late);
