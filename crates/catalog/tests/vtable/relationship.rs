...
#[test]
fn ddl_create_one_to_one_relationship_persists_and_appears_in_vtable() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_a");

	t.admin("CREATE RELATIONSHIP r_one_one
		    ON ddl_rel_a::parent(id)
		    REFERENCES ddl_rel_a::child(parent_id)
...
#[test]
fn ddl_create_many_to_many_with_through_records_junction() {
	let t = TestEngine::new();
	let f = Fixture::new(&t, "ddl_rel_b");

	t.admin("CREATE RELATIONSHIP r_n_m
		    ON ddl_rel_b::parent(id)
		    THROUGH ddl_rel_b::link(src_id, tgt_id)
		    REFERENCES ddl_rel_b::child(parent_id)
		    WITH { cardinality: 'N:M' }");
...
#[test]
fn ddl_create_n_m_without_through_is_rejected_at_parse() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_c");

	let err = t.admin_err(
		"CREATE RELATIONSHIP bad
		    ON ddl_rel_c::parent(id)
		    REFERENCES ddl_rel_c::child(parent_id)
		    WITH { cardinality: 'N:M' }",
	);
	assert!(err.contains("THROUGH") || err.to_lowercase().contains("junction"), "unexpected error: {err}");
}

#[test]
fn ddl_create_one_to_one_with_through_is_rejected_at_parse() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_d");

	let err = t.admin_err(
		"CREATE RELATIONSHIP bad
		    ON ddl_rel_d::parent(id)
		    THROUGH ddl_rel_d::link(src_id, tgt_id)
		    REFERENCES ddl_rel_d::child(parent_id)
		    WITH { cardinality: '1:1' }",
	);
	assert!(err.contains("THROUGH") || err.to_lowercase().contains("junction"), "unexpected error: {err}");
}
...
#[test]
fn ddl_drop_relationship_removes_it_from_vtable() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_e");

	t.admin("CREATE RELATIONSHIP gone_soon
		    ON ddl_rel_e::parent(id)
...
#[test]
fn ddl_drop_if_exists_succeeds_when_missing() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_f");

	t.admin("DROP RELATIONSHIP IF EXISTS never_existed ON ddl_rel_f::parent");
}

#[test]
fn ddl_drop_without_if_exists_errors_when_missing() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_g");

	let err = t.admin_err("DROP RELATIONSHIP nope ON ddl_rel_g::parent");
	assert!(err.to_lowercase().contains("relationship") || err.contains("not found"), "unexpected error: {err}");
}

#[test]
fn ddl_duplicate_relationship_is_rejected() {
	let t = TestEngine::new();
	let _f = Fixture::new(&t, "ddl_rel_h");

	t.admin("CREATE RELATIONSHIP dup
		    ON ddl_rel_h::parent(id)
		    REFERENCES ddl_rel_h::child(parent_id)
		    WITH { cardinality: '1:1' }");

	let err = t.admin_err(
		"CREATE RELATIONSHIP dup
		    ON ddl_rel_h::parent(id)
		    REFERENCES ddl_rel_h::child(parent_id)
		    WITH { cardinality: '1:1' }",
	);
	assert!(
		err.to_lowercase().contains("already") || err.to_lowercase().contains("exists"),
