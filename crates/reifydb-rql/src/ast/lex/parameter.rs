...
pub(crate) fn parse_parameter(input: LocatedSpan<&str>) -> IResult<LocatedSpan<&str>, Token> {
    alt((parse_positional_parameter, parse_named_parameter)).parse(input)
}

fn parse_positional_parameter(input: LocatedSpan<&str>) -> IResult<LocatedSpan<&str>, Token> {
...
fn parse_named_parameter(input: LocatedSpan<&str>) -> IResult<LocatedSpan<&str>, Token> {
    let (rest, span) = complete(recognize(preceded(
        tag("$"),
...
mod tests {
    ...
    fn test_positional_parameter() {
        let (_rest, result) = parse_parameter(LocatedSpan::new("$1")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Positional(1)));
        assert_eq!(&result.span.fragment, "$1");
        
        let (_rest, result) = parse_parameter(LocatedSpan::new("$42")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Positional(42)));
        assert_eq!(&result.span.fragment, "$42");
    }
    
    #[test]
    fn test_named_parameter() {
        let (_rest, result) = parse_parameter(LocatedSpan::new("$name")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Named));
        assert_eq!(&result.span.fragment, "$name");
        
        let (_rest, result) = parse_parameter(LocatedSpan::new("$user_id")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Named));
        assert_eq!(&result.span.fragment, "$user_id");
        
        let (_rest, result) = parse_parameter(LocatedSpan::new("$_private")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Named));
    ...
    fn test_invalid_parameters() {
        ...
        let (rest, result) = parse_parameter(LocatedSpan::new("$123name")).unwrap();
        assert_eq!(result.kind, TokenKind::Parameter(ParameterKind::Positional(123)));
