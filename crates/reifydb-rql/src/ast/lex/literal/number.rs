...
fn parse_decimal(
	...
	let integer_dot = complete(recognize(pair(
		take_while1(is_digit_or_underscore),
		char('.'),
	)));
	let dot_fraction = complete(recognize(pair(
		char('.'),
		take_while1(is_digit_or_underscore),
	)));
	let int_frac = complete(recognize(pair(
		take_while1(is_digit_or_underscore),
		fraction,
	)));
	let just_integer =
		complete(recognize(take_while1(is_digit_or_underscore)));
...
mod tests {
	...
	fn test_number_hex() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0x2A")).unwrap();
	...
	fn test_number_hex_with_underscores() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0x2A_AA")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_octal() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0o777")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_octal_with_underscores() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0o777_555")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_binary() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0b1010")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_binary_with_underscores() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("0b111_010")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_decimal() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("100")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_decimal_with_underscores() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("1_000_000")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_decimal_scientific() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("1e+500")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_float() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("42.5")).unwrap();
		assert_eq!(token.kind, Literal(Number));
	...
	fn test_number_float_with_underscores() {
		let (_rest, token) =
			parse_literal(LocatedSpan::new("1_142.5_234")).unwrap();
		assert_eq!(token.kind, Literal(Number));
