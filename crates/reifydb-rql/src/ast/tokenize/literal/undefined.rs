...
mod tests {
	...
	fn test_undefined() {
		...
		assert_eq!(tokens[0].fragment.fragment(), "undefined");
	}
	...
	fn test_undefined_with_trailing() {
		...
		assert_eq!(tokens[0].fragment.fragment(), "undefined123");

		let tokens = tokenize("undefined_value").unwrap();
		// Should parse as identifier, not undefined
		assert_eq!(tokens[0].kind, TokenKind::Identifier);
		assert_eq!(tokens[0].fragment.fragment(), "undefined_value");
	}
	...
	fn test_undefined_in_expression() {
		...
		assert_eq!(tokens[0].fragment.fragment(), "value");
		assert_eq!(tokens[2].kind, TokenKind::Literal(Undefined));
	}
}
