...
impl<T: Transaction> AdminServer<T> {
	...
	fn handle_request(
		...
		match route {
			...
			Route::UpdateConfig => {
				handlers::handle_update_config(config, request)
			}
			Route::Execute => {
				handlers::handle_execute(engine, request)
			}
			...
			Route::AuthStatus => {
				handlers::handle_auth_status(config, request)
			}
			Route::ServeIndex => handlers::serve_index(),
			Route::ServeStatic(path) => {
				handlers::serve_static(&path)
			}
			Route::WebSocket => {
				// WebSocket upgrade handled separately
				HttpResponse::bad_request().with_json(
					r#"{"error":"WebSocket not implemented yet"}"#,
				)
			}
