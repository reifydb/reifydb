...
pub fn handle_execute<T: Transaction>(_engine: &StandardEngine<T>, request: HttpRequest) -> HttpResponse {
	...
	match json_body {
		Ok(json) => {
			if let Some(_query) = json.get("query").and_then(|q| q.as_str()) {
				...
				HttpResponse::ok().with_json(&result.to_string())
			} else {
				HttpResponse::bad_request().with_json(r#"{"error":"Missing 'query' field"}"#)
			}
		}
