...
impl MaterializedCatalog {
	...
	pub fn set_primary_key(
		...
		let versioned = self
			.primary_keys
			.get_or_insert_with(id, VersionedPrimaryKeyDef::new);
...
mod tests {
	...
	fn test_primary_key_update() {
		...
		assert_eq!(
			catalog.find_primary_key(pk_id, 1)
				.unwrap()
				.columns
				.len(),
			1
		...
		assert_eq!(
			catalog.find_primary_key(pk_id, 2)
				.unwrap()
				.columns
				.len(),
			2
