...
use reifydb_core::interface::{
	ColumnDef, PrimaryKeyDef, PrimaryKeyId, PrimaryKeyKey, QueryTransaction,
};

use crate::{
	CatalogStore, MaterializedCatalog, primary_key::layout::primary_key,
};
...
pub fn load_primary_keys(
	...
	for versioned in primary_keys {
		...
		let pk_id = PrimaryKeyId(
			primary_key::LAYOUT.get_u64(&row, primary_key::ID),
		);

		// Deserialize column IDs
		let column_ids_blob = primary_key::LAYOUT
			.get_blob(&row, primary_key::COLUMN_IDS);
		let column_ids =
			primary_key::deserialize_column_ids(&column_ids_blob);

		// Fetch the full ColumnDef objects for each column ID
		let mut columns = Vec::new();
		for column_id in column_ids {
			let column_def =
				CatalogStore::get_column(qt, column_id)?;
