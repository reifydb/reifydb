    ...
    where
    {
        txn.with_unversioned_command(|tx| match tx.get(key)? {
            ...
            None => {
                ...
                LAYOUT.set_i64(&mut new_row, 0, 1i64);
                tx.set(key, new_row)?;
                Ok(1)
            }
        })
...
mod tests {
    ...
    fn test_ok() {
        ...
        for expected in 1..1000 {
            let got = GeneratorI64::next(&mut txn, &EncodedKey::new("sequence")).unwrap();
    ...
    fn test_exhaustion() {
        ...
        let err = GeneratorI64::next(&mut txn, &EncodedKey::new("sequence")).unwrap_err();
