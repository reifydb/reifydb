...
where {
        txn.with_unversioned_command(|tx| match tx.get(key)? {
            ...
            None => {
                ...
                LAYOUT.set_u32(&mut new_row, 0, 1u32);
                tx.set(key, new_row)?;
                Ok(1)
            }
        })
...
mod tests {
    ...
    fn test_ok() {
        ...
        for expected in 1..1000 {
            let got = GeneratorU32::next(&mut txn, &EncodedKey::new("sequence")).unwrap();
        ...
        txn.with_unversioned_query(|tx| {
            ...
            assert_eq!(LAYOUT.get_u32(&unversioned.row, 0), 999);
    ...
    fn test_exhaustion() {
        ...
        let err = GeneratorU32::next(&mut txn, &EncodedKey::new("sequence")).unwrap_err();
