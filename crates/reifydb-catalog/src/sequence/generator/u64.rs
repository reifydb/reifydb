    ...
    where
    {
        txn.with_unversioned_command(|tx| match tx.get(key)? {
            ...
            None => {
                ...
                LAYOUT.set_u64(&mut new_row, 0, 1u64);
                tx.set(key, new_row)?;
                Ok(1)
            }
        })
...
mod tests {
    ...
    fn test_ok() {
        ...
        for expected in 1..1000 {
            let got = GeneratorU64::next(&mut txn, &EncodedKey::new("sequence")).unwrap();
        ...
        txn.with_unversioned_query(|tx| {
            ...
            assert_eq!(LAYOUT.get_u64(&unversioned.row, 0), 999);
    ...
    fn test_exhaustion() {
        ...
        let err = GeneratorU64::next(&mut txn, &EncodedKey::new("sequence")).unwrap_err();
