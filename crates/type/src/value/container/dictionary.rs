...
impl DictionaryContainer {
	...
	pub fn data(&self) -> &CowVec<DictionaryEntryId> {
		&self.data
	}

	pub fn data_mut(&mut self) -> &mut CowVec<DictionaryEntryId> {
		&mut self.data
	}
	...
	pub fn extend_from_undefined(&mut self, count: usize) {
		self.data.extend(vec![DictionaryEntryId::default(); count]);
		for _ in 0..count {
			self.bitvec.push(false);
		}
	}
	...
	pub fn get_value(&self, index: usize) -> Value {
		self.get(index).map(Value::DictionaryId).unwrap_or(Value::Undefined)
	}
	...
	pub fn slice(&self, start: usize, end: usize) -> Self {
		let new_data: Vec<DictionaryEntryId> =
