...
pub fn parse_uuid4<'a>(fragment: impl IntoFragment<'a>) -> Result<Uuid4, Error> {
...
pub fn parse_uuid7<'a>(fragment: impl IntoFragment<'a>) -> Result<Uuid7, Error> {
	let fragment = fragment.into_fragment();
	let value = fragment.text().trim();
	if let Ok(uuid) = Uuid::parse_str(value) {
...
mod tests {
	...
	mod uuid4 {
		...
		fn test_invalid_uuid4_format() {
			let result = parse_uuid4(OwnedFragment::testing("not-a-uuid"));
			assert!(result.is_err());
		}
		...
		fn test_invalid_uuid4_malformed() {
			let result = parse_uuid4(OwnedFragment::testing("550e8400-e29b-41d4-a716"));
			assert!(result.is_err());
		}
	}
	...
	mod uuid7 {
		...
		fn test_invalid_uuid7_format() {
			let result = parse_uuid7(OwnedFragment::testing("invalid-uuid"));
			assert!(result.is_err());
		}
		...
		fn test_invalid_uuid7_malformed() {
			let result = parse_uuid7(OwnedFragment::testing("017f22e2-79b0-7cc3"));
			assert!(result.is_err());
		}
	}
}
