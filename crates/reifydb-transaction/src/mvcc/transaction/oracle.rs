    ...
    L: VersionProvider,
{
    ...
    pub fn new(query_name: Cow<'static, str>, command_name: Cow<'static, str>, clock: L) -> Self {
    ...
    pub(super) fn new_commit(
        ...
        let all_keys: Vec<_> = read_keys.iter().chain(conflict_keys.iter()).cloned().collect();

        // Only check conflicts in windows that contain relevant keys
        let relevant_windows: Vec<Version> = if all_keys.is_empty() {
            // If no specific keys, we need to check recent windows for range/all operations
            ...
            for key in &all_keys {
                if let Some(window_versions) = inner.key_to_windows.get(key) {
                    // Check all windows that have this key - version filtering happens later
                    for &window_version in window_versions.iter() {
                        windows.insert(window_version);
                    }
                }
            }
            // If no windows found via key index, check all windows as fallback
            // This handles range queries and other operations that can't be indexed by specific keys
            if windows.is_empty() {
        ...
        for window_version in relevant_windows {
            if let Some(window) = inner.time_windows.get(&window_version) {
                // Check conflicts with transactions in this window
                for committed_txn in &window.transactions {
                    // Skip transactions that committed before we started reading
    ...
    L: VersionProvider,
{
    ...
    fn add_committed_transaction(&mut self, version: Version, conflicts: ConflictManager) {
        ...
        let txn = CommittedTxn { version, conflict_manager: Some(conflicts) };
...
mod tests {
    ...
    impl MockVersionProvider {
        fn new(start: Version) -> Self {
            Self { current: Arc::new(AtomicU64::new(start)) }
        }
    }
    ...
    fn test_conflict_detection_between_transactions() {
        ...
        // txn2 also started reading at version 1, but txn1 committed at version 2
        // So txn2 should see the conflict
        let result2 = oracle.new_commit(&mut done_read2, 1, conflicts2).unwrap();

        // Should detect conflict because txn2 read shared_key which txn1 wrote to
    ...
    fn test_key_indexing_multiple_windows() {
        ...
        for i in 0..3 {
            ...
            let mut done_read = false;
            let version_start = (i as Version) * DEFAULT_WINDOW_SIZE + 1;
    ...
    fn test_write_write_conflict() {
        ...
        // Should detect conflict because both transactions write to the same key
        assert!(matches!(result2, CreateCommitResult::Conflict(_)));
    }
    ...
    fn test_read_write_conflict() {
        ...
        // Should detect conflict because txn2 read from key that txn1 wrote to
        assert!(matches!(result2, CreateCommitResult::Conflict(_)));
    }
