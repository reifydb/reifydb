...
/// Iterator for reverse range scan in an SVL WriteTransaction with owned values.
        ...
        loop {
            match (self.next_pending.as_ref(), self.next_committed.as_ref()) {
                ...
                (Some((pending_key, _delta)), Some(committed)) => {
                    // For reverse iteration, we want larger keys first
                    match pending_key.cmp(&committed.key) {
                        // Pending item has a larger key, yield it
                        ...
                        // Committed item has a larger key, yield it
                        cmp::Ordering::Less => {
