...
use reifydb_transaction::mvcc::transaction::MAX_COMMITTED_TXNS;
use reifydb_transaction::mvcc::transaction::optimistic::Optimistic;
use std::sync::Arc;
use std::thread;

/// Test that Oracle properly cleans up committed transactions when limit is exceeded
#[test]
fn test_oracle_committed_txns_cleanup() {
    ...
    for i in 0..NUM_TXNS {
        ...
        // Each transaction writes to a unique key to avoid actual conflicts
        ...
        if i > 0 && i % 1000 == 0 {
            // The Oracle should automatically clean up when exceeding limits
            // We can't directly check the internal state, but the fact that
            // we can continue creating transactions shows cleanup is working
