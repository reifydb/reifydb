...
mod tests {
    ...
    fn test_single_row_temporal_types() {
        ...
        assert_eq!(
            columns.column("datetime_col").unwrap().data().get_value(0),
            Value::DateTime(datetime)
        );
        assert_eq!(columns.column("time_col").unwrap().data().get_value(0), Value::Time(time));
        assert_eq!(
            columns.column("interval_col").unwrap().data().get_value(0),
    ...
    fn test_single_row_mixed_types() {
        ...
        assert_eq!(columns.column("bool_col").unwrap().data().get_value(0), Value::Bool(true));
