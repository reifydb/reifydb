...
use super::{scoped::ScopedPools, config::PoolConfig};
...
macro_rules! test_with_pools {
    ($name:ident, $body:block) => {
        #[test]
        fn $name() {
            let _pools = $crate::columnar::pool::testing::TestPools::new();
            $body
        }
    };
}
...
macro_rules! test_with_custom_pools {
    ($name:ident, $config:expr, $body:block) => {
        #[test]
        fn $name() {
            let _pools = $crate::columnar::pool::testing::TestPools::with_config($config);
            $body
        }
    };
}
...
mod tests {
    use super::*;
    use crate::columnar::pool::thread_local::{has_thread_pools, clear_thread_pools};
    ...
    fn test_test_pools() {
        ...
        
        {
            ...
            // (assuming factory methods will be updated to use thread-local pools)
    ...
    test_with_custom_pools!(test_custom_macro, PoolConfig {
        max_pool_size: 2,
        ..PoolConfig::test()
    }, {
        assert!(has_thread_pools());
