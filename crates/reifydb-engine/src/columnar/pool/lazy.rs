...
//! allowing ColumnData operations to work even when pools haven't been explicitly initialized.

use once_cell::sync::Lazy;
use std::sync::Mutex;
use super::{PoolConfig, Pools, thread_local::{get_thread_pools, set_thread_pools, has_thread_pools}};

/// Global default configuration for thread-local pools
static DEFAULT_CONFIG: Lazy<Mutex<PoolConfig>> = Lazy::new(|| {
    Mutex::new(PoolConfig::default())
});
...
/// This is like thread_pools() but will initialize with defaults instead of panicking
...
mod tests {
    use super::*;
    use crate::columnar::pool::thread_local::clear_thread_pools;
