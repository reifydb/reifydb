    ...
    pub fn push_value(&mut self, value: Value) {
        match value {
            ...
            Value::RowId(row_id) => match self {
                ColumnData::RowId(container) => container.push(row_id),
                ColumnData::Undefined(container) => {
                    let mut new_container = ColumnData::row_id(vec![]);
                    if let ColumnData::RowId(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
            ...
            Value::Blob(v) => match self {
                ColumnData::Blob(container) => container.push(v),
                ColumnData::Undefined(container) => {
                    let mut new_container = ColumnData::blob(vec![]);
                    if let ColumnData::Blob(new_container) = &mut new_container {
                        for _ in 0..container.len() {
                            new_container.push_undefined();
                        }
                        new_container.push(v);
