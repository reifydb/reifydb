...
impl TierStorage for SqlitePrimitiveStorage {
	...
	fn get(&self, key: &[u8]) -> Result<Option<CowVec<u8>>> {
		...
		match result {
			...
			Err(e) => Err(reifydb_core::internal_error!("Failed to get: {}", e)),
		}
	}
	...
	fn contains(&self, key: &[u8]) -> Result<bool> {
		...
		match result {
			...
			Err(e) => Err(reifydb_core::internal_error!("Failed to check contains: {}", e)),
		}
	}
	...
	fn set(&self, entries: Vec<(CowVec<u8>, Option<CowVec<u8>>)>) -> Result<()> {
		...
		let tx = conn
			...
			.map_err(|e| reifydb_core::internal_error!("Failed to start transaction: {}", e))?;
		...
		if let Err(e) = result {
			if e.to_string().contains("no such table") {
				...
				.map_err(|e| reifydb_core::internal_error!("Failed to create table: {}", e))?;
				insert_entries_in_tx(&tx, TABLE_NAME, &entries).map_err(|e| {
					reifydb_core::internal_error!("Failed to insert entries: {}", e)
				})?;
			} else {
				return Err(reifydb_core::internal_error!("Failed to insert entries: {}", e));
			}
		}

		tx.commit().map_err(|e| reifydb_core::internal_error!("Failed to commit transaction: {}", e))
	}
	...
	fn range_next(
		...
		let mut stmt = match conn.prepare(&query) {
			...
			Err(e) => return Err(reifydb_core::internal_error!("Failed to prepare query: {}", e)),
		};
		...
		let entries: Vec<RawEntry> = stmt
			...
			.map_err(|e| reifydb_core::internal_error!("Failed to query range: {}", e))?
			.filter_map(|r| r.ok())
			.collect();
	...
	fn ensure_table(&self) -> Result<()> {
		...
		.map(|_| ())
		.map_err(|e| reifydb_core::internal_error!("Failed to ensure table: {}", e))
	}
	...
	fn clear_table(&self) -> Result<()> {
		...
		match result {
			...
			Err(e) => Err(reifydb_core::internal_error!("Failed to clear table: {}", e)),
		}
	}
}
