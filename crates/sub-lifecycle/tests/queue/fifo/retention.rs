...
fn deduplication_count(t: &TestEngine, queue: QueueId) -> usize {
	let mut query_txn = t.inner().begin_query(TestEngine::identity()).unwrap();
	let mut txn = Transaction::Query(&mut query_txn);
	let stream = txn.range(QueueDeduplicationKey::full_scan(queue), RangeScope::All, 1024).unwrap();
	let mut count = 0;
	for item in stream {
		item.unwrap();
		count += 1;
...
fn test_a_terminal_item_with_no_attempt_record_is_left_alone() {
	...
	txn.remove(&QueueAttemptKey::new(queue, reifydb_value::value::row_number::RowNumber(1), 1)).unwrap();
	txn.commit().unwrap();
...
fn test_an_orphan_state_record_from_a_crashed_sweep_is_collected() {
	...
	let mut txn = t.inner().begin_command(TestEngine::identity()).unwrap();
	txn.remove(&reifydb_core::key::row::RowKey::new(queue, reifydb_value::value::row_number::RowNumber(1)))
		.unwrap();
	txn.remove(&QueueAttemptKey::new(queue, reifydb_value::value::row_number::RowNumber(1), 1)).unwrap();
	txn.commit().unwrap();
