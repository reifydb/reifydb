...
use reifydb_core::{Blob, CowVec, EncodedKey};

pub(crate) fn encode_cdc_event(event: &CdcEvent) -> reifydb_core::Result<EncodedRow> {
    ...
    CDC_EVENT_LAYOUT.set_u64(&mut row, CDC_TIMESTAMP_FIELD, event.timestamp);
    
    // Encode change type and variable-length fields
    match &event.change {
        Change::Insert { key, after } => {
            CDC_EVENT_LAYOUT.set_u8(&mut row, CDC_CHANGE_TYPE_FIELD, ChangeType::Insert as u8);
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_KEY_FIELD, &Blob::from_slice(key.as_slice()));
            CDC_EVENT_LAYOUT.set_undefined(&mut row, CDC_BEFORE_FIELD);
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_AFTER_FIELD, &Blob::from_slice(after.as_slice()));
        }
        Change::Update { key, before, after } => {
            CDC_EVENT_LAYOUT.set_u8(&mut row, CDC_CHANGE_TYPE_FIELD, ChangeType::Update as u8);
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_KEY_FIELD, &Blob::from_slice(key.as_slice()));
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_BEFORE_FIELD, &Blob::from_slice(before.as_slice()));
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_AFTER_FIELD, &Blob::from_slice(after.as_slice()));
        }
        Change::Delete { key, before } => {
            CDC_EVENT_LAYOUT.set_u8(&mut row, CDC_CHANGE_TYPE_FIELD, ChangeType::Delete as u8);
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_KEY_FIELD, &Blob::from_slice(key.as_slice()));
            CDC_EVENT_LAYOUT.set_blob(&mut row, CDC_BEFORE_FIELD, &Blob::from_slice(before.as_slice()));
            CDC_EVENT_LAYOUT.set_undefined(&mut row, CDC_AFTER_FIELD);
        }
    }
...
pub(crate) fn decode_cdc_event(row: &EncodedRow) -> reifydb_core::Result<CdcEvent> {
    ...
    let change_type = ChangeType::from(CDC_EVENT_LAYOUT.get_u8(row, CDC_CHANGE_TYPE_FIELD));
    ...
    let change = match change_type {
        ChangeType::Insert => {
            let after_blob = CDC_EVENT_LAYOUT.get_blob(row, CDC_AFTER_FIELD);
            let after = EncodedRow(CowVec::new(after_blob.as_bytes().to_vec()));
            Change::Insert { key, after }
        }
        ChangeType::Update => {
            let before_blob = CDC_EVENT_LAYOUT.get_blob(row, CDC_BEFORE_FIELD);
            let after_blob = CDC_EVENT_LAYOUT.get_blob(row, CDC_AFTER_FIELD);
...
mod tests {
    ...
    fn test_encode_decode_update() {
        ...
        match decoded.change {
            Change::Update { key: k, before: b, after: a } => {
                assert_eq!(k, key);
                assert_eq!(b, before);
                assert_eq!(a, after);
            }
