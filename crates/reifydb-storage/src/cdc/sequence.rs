...
impl SequenceTracker {
    ...
    /// Returns None if the sequence has reached u16::MAX
    pub fn next_sequence(&self, version: Version) -> Option<u16> {
        ...
        // Check if we've exhausted the sequence space
        if current == u16::MAX {
            return None;
        }
...
mod tests {
    ...
    fn test_sequence_tracking() {
        ...
        // First sequence for version 1 should be 1
        assert_eq!(tracker.next_sequence(1), Some(1));
        assert_eq!(tracker.next_sequence(1), Some(2));
        assert_eq!(tracker.next_sequence(1), Some(3));

        // Different version starts at 1
        assert_eq!(tracker.next_sequence(2), Some(1));
        assert_eq!(tracker.next_sequence(2), Some(2));

        // Version 1 continues from where it left off
        assert_eq!(tracker.next_sequence(1), Some(4));
    }
    ...
    fn test_lru_eviction() {
        ...
        assert_eq!(tracker.next_sequence(1), Some(1));
        assert_eq!(tracker.next_sequence(2), Some(1));
        assert_eq!(tracker.next_sequence(3), Some(1));

        // Increment counters
        assert_eq!(tracker.next_sequence(1), Some(2));
        assert_eq!(tracker.next_sequence(2), Some(2));
        assert_eq!(tracker.next_sequence(3), Some(2));

        // At this point, LRU order is [1, 2, 3]
        // Add a 4th version, should evict version 1 (least recently used)
        assert_eq!(tracker.next_sequence(4), Some(1));
    ...
    fn test_sequence_exhaustion() {
        ...
        // Manually set sequence to near max
        {
            let mut inner = tracker.inner.lock().unwrap();
            inner.sequences.insert(1, u16::MAX - 1);
        }
        
        // Should get the last two valid sequences
        assert_eq!(tracker.next_sequence(1), Some(u16::MAX));

        // Should return None when exhausted
        assert_eq!(tracker.next_sequence(1), None);
        assert_eq!(tracker.next_sequence(1), None); // Still None
    }
}
