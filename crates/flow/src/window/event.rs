...
mod tests {
	...
	fn an_update_decomposes_into_an_inverted_pair() {
		// Intent: an Update is a Remove of the pre image plus an Insert of the post, and
		...
		// Mutation: make inverted() the identity and an update double-counts its post.
	...
	fn an_event_carries_the_coordinate_of_its_own_domain() {
		// Intent: the domain decides the coordinate TYPE, so a count window cannot be fed
