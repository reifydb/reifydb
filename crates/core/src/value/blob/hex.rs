...
impl Blob {
    pub fn from_hex(span: impl Span) -> Result<Self, Error> {
        ...
        match hex::decode(clean_hex) {
            ...
            Err(_) => Err(Error(blob::invalid_hex_string(span.to_owned()))),
        }
    }
...
mod tests {
    ...
    #[test]
    fn test_from_hex() {
        let blob = Blob::from_hex(OwnedSpan::testing("48656c6c6f")).unwrap();
        assert_eq!(blob.as_bytes(), b"Hello");
    }

    #[test]
    fn test_from_hex_with_prefix() {
        let blob = Blob::from_hex(OwnedSpan::testing("0x48656c6c6f")).unwrap();
        assert_eq!(blob.as_bytes(), b"Hello");
    ...
    fn test_hex_roundtrip() {
        ...
        let decoded = Blob::from_hex(OwnedSpan::testing(&hex_str)).unwrap();
