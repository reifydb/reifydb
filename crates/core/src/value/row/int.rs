...
mod tests {
	...
	fn test_i64_inline() {
		...
		layout.set_int(&mut row, 0, &small);
		assert!(row.is_defined(0));

		let retrieved = layout.get_int(&row, 0);
	...
	fn test_i128_boundary() {
		...
		layout.set_int(&mut row, 0, &large);
		assert!(row.is_defined(0));
	...
	fn test_try_get() {
		...
		layout.set_int(&mut row, 0, &value);
		assert_eq!(layout.try_get_int(&row, 0), Some(value));
	}
	...
	fn test_multiple_fields() {
		...
		layout.set_int(&mut row, 1, &small);

		layout.set_utf8(&mut row, 2, "test");

		let large = Int::from(i128::MAX);
		layout.set_int(&mut row, 3, &large);

		assert_eq!(layout.get_i32(&row, 0), 42);
		assert_eq!(layout.get_int(&row, 1), small);
		assert_eq!(layout.get_utf8(&row, 2), "test");
		assert_eq!(layout.get_int(&row, 3), large);
	}
