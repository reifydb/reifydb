...
mod tests {
	...
	fn test_set_values_with_mixed_dynamic_content() {
		let layout = EncodedRowLayout::new(&[Type::Boolean, Type::Utf8, Type::Float4, Type::Utf8, Type::Int2]);
		let mut row = layout.allocate_row();
	...
	fn test_set_undefined_with_utf8_fields() {
		let layout = EncodedRowLayout::new(&[Type::Utf8, Type::Boolean, Type::Utf8]);
		let mut row = layout.allocate_row();
