...
impl EncodedValuesLayout {
	pub fn set_interval(&self, row: &mut EncodedValues, index: usize, value: Interval) {
	...
	pub fn get_interval(&self, row: &EncodedValues, index: usize) -> Interval {
		let field = &self.fields[index];
		debug_assert!(row.len() >= self.total_static_size());
		debug_assert_eq!(field.r#type, Type::Interval);
		unsafe {
	...
	pub fn try_get_interval(&self, row: &EncodedValues, index: usize) -> Option<Interval> {
