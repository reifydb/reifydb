...
mod tests {
    ...
    mod composite {
        ...
        fn test_mixed_directions() {
            ...
            // key1 (100, 1, 1) vs key2 (100, 2, 1): same first field, second field ascending
            assert!(key1.as_slice() < key2.as_slice());
            // key1 (100, 1, 1) vs key3 (50, 1, 1): first field is DESC, so 100 < 50 in byte order
            assert!(key1.as_slice() < key3.as_slice());
            // key3 (50, 1, 1) vs key4 (50, 1, 2): same first two fields, third field ascending
