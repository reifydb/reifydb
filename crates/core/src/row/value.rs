...
impl Layout {
    ...
    pub fn set_value(&self, row: &mut EncodedRow, index: usize, val: &Value) {
        ...
        match (field.value, val) {
            ...
            (Kind::String, Value::String(v)) => self.set_str(row, index, v),
            (Kind::String, Value::Undefined) => self.set_undefined(row, index),
    ...
    pub fn get_value(&self, row: &EncodedRow, index: usize) -> Value {
        ...
        unsafe {
            match field.value {
                ...
                Kind::String => Value::String(self.get_str(row, index).to_string()),
