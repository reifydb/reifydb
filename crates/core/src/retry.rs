...
use reifydb_value::{error::Error, value::duration::Duration};
...
impl RetryStrategy {
	...
	pub fn execute<F>(&self, rng: &Rng, rql: &str, f: F) -> ExecutionResult
	where
		F: FnMut() -> ExecutionResult,
	{
		self.run(rng, rql, f, |result| result.error.as_ref().is_some_and(is_conflict))
	}

	pub fn retry<T, F>(&self, rng: &Rng, operation: &str, f: F) -> Result<T, Error>
	where
		F: FnMut() -> Result<T, Error>,
	{
		self.run(rng, operation, f, |result| result.as_ref().is_err_and(is_conflict))
	}

	fn run<R>(&self, rng: &Rng, rql: &str, mut f: impl FnMut() -> R, conflicted: impl Fn(&R) -> bool) -> R {
		let mut last_result = None;
		for attempt in 0..self.max_attempts {
			let result = f();
			if !conflicted(&result) {
				return result;
			}
...
mod retry_tests {
	...
	use crate::{execution::ExecutionResult, metrics::execution::ExecutionMetrics};
