...
pub mod tests {
	...
	fn test_set_get_uuid4() {
		let layout = EncodedValuesLayout::new(&[Type::Uuid4]);
	...
	fn test_multiple_generations() {
		...
		for _ in 0..10 {
			...
			let uuid = Uuid4::generate();
			layout.set_uuid4(&mut row, 0, uuid.clone());
			let retrieved = layout.get_uuid4(&row, 0);
	...
	fn test_mixed_with_other_types() {
		let layout = EncodedValuesLayout::new(&[Type::Uuid4, Type::Boolean, Type::Uuid4, Type::Int4]);
		let mut row = layout.allocate_for_testing();

		let uuid1 = Uuid4::generate();
		let uuid2 = Uuid4::generate();

		layout.set_uuid4(&mut row, 0, uuid1.clone());
		layout.set_bool(&mut row, 1, true);
		layout.set_uuid4(&mut row, 2, uuid2.clone());
		layout.set_i32(&mut row, 3, 42);

		assert_eq!(layout.get_uuid4(&row, 0), uuid1);
		assert_eq!(layout.get_bool(&row, 1), true);
		assert_eq!(layout.get_uuid4(&row, 2), uuid2);
		assert_eq!(layout.get_i32(&row, 3), 42);
	}

	#[test]
	fn test_undefined_handling() {
		let layout = EncodedValuesLayout::new(&[Type::Uuid4, Type::Uuid4]);
		let mut row = layout.allocate_for_testing();
		...
		assert_eq!(layout.try_get_uuid4(&row, 0), Some(uuid));
		assert_eq!(layout.try_get_uuid4(&row, 1), None);
	...
	fn test_clone_consistency() {
		...
		layout.set_uuid4(&mut row, 0, original_uuid.clone());

		let retrieved_uuid = layout.get_uuid4(&row, 0);
	...
	fn test_multiple_fields() {
		let layout = EncodedValuesLayout::new(&[Type::Uuid4, Type::Uuid4, Type::Uuid4]);
		let mut row = layout.allocate_for_testing();
		...
		layout.set_uuid4(&mut row, 0, uuid1.clone());
		layout.set_uuid4(&mut row, 1, uuid2.clone());
		layout.set_uuid4(&mut row, 2, uuid3.clone());

		assert_eq!(layout.get_uuid4(&row, 0), uuid1);
		assert_eq!(layout.get_uuid4(&row, 1), uuid2);
		assert_eq!(layout.get_uuid4(&row, 2), uuid3);
