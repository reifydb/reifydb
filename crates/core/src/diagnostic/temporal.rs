...
pub fn invalid_date_format(span: impl IntoOwnedSpan) -> Diagnostic {
    ...
    let label = Some(format!("expected YYYY-MM-DD format, found '{}'", owned_span.fragment));
    Diagnostic {
...
pub fn invalid_time_format(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("expected HH:MM:SS format, found '{}'", owned_span.fragment));
    Diagnostic {
...
pub fn invalid_year(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("year '{}' cannot be parsed as a number", owned_span.fragment));
    Diagnostic {
        code: "TEMPORAL_005".to_string(),
        statement: None,
        message: format!("invalid year value '{}'", owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn invalid_month(span: impl IntoOwnedSpan) -> Diagnostic {
    ...
    Diagnostic {
        ...
        statement: None,
        message: format!("invalid month value '{}'", owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn invalid_day(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label =
        Some(format!("day '{}' cannot be parsed as a number (expected 1-31)", owned_span.fragment));
    Diagnostic {
...
pub fn invalid_minute(span: impl IntoOwnedSpan) -> Diagnostic {
    ...
    Diagnostic {
        ...
        statement: None,
        message: format!("invalid minute value '{}'", owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn invalid_second(span: impl IntoOwnedSpan) -> Diagnostic {
    ...
    Diagnostic {
        ...
        statement: None,
        message: format!("invalid second value '{}'", owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn invalid_fractional_seconds(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label =
        Some(format!("fractional seconds '{}' cannot be parsed as a number", owned_span.fragment));
    Diagnostic {
        code: "TEMPORAL_011".to_string(),
        statement: None,
        message: format!("invalid fractional seconds value '{}'", owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn invalid_date_values(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("date '{}' represents an invalid calendar date", owned_span.fragment));
    Diagnostic {
...
pub fn invalid_time_values(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("time '{}' contains out-of-range values", owned_span.fragment));
    Diagnostic {
...
pub fn incomplete_interval_specification(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("number '{}' is missing a unit specifier", owned_span.fragment));
    Diagnostic {
...
pub fn invalid_interval_component_value(span: impl IntoOwnedSpan, unit: char) -> Diagnostic {
    ...
    Diagnostic {
        ...
        statement: None,
        message: format!("invalid {} value '{}'", unit_name(unit), owned_span.fragment),
        span: Some(owned_span),
        label,
...
pub fn unrecognized_temporal_pattern(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("value '{}' does not match any temporal format", owned_span.fragment));
    Diagnostic {
...
pub fn empty_date_component(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("date component '{}' is empty", owned_span.fragment));
    Diagnostic {
...
pub fn empty_time_component(span: impl IntoOwnedSpan) -> Diagnostic {
    let owned_span = span.into_span();
    let label = Some(format!("time component '{}' is empty", owned_span.fragment));
    Diagnostic {
