...
impl EncodableKey for TableRowKey {
    ...
    fn decode(version: u8, payload: &[u8]) -> Option<Self> {
        ...
        Some(Self {
            table_id: u16::from_be_bytes([payload[0], payload[1]]),
            row_id: u64::from_be_bytes(payload[2..10].try_into().unwrap()),
        })
    }
}

impl TableRowKey {
    pub fn full_scan(table_id: u16) -> EncodedKeyRange {
    ...
    fn table_start(table_id: u16) -> EncodedKey {
    ...
    fn table_end(table_id: u16) -> EncodedKey {
        let mut out = Vec::with_capacity(4);
        out.push(VERSION);
        out.push(KeyKind::TableRow as u8);
