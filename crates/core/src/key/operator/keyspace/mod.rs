		...
		operator::{
			keyspace::{
				...
				window::{
					...
					GuestRetainedEntry, GuestRunning, GuestWindowPublish, RollingMeta, RowIndex, Running,
					Session, WindowMeta,
				},
			},
...
fn round_trips<K: Keyspace>() {
	...
	// probe able to fail
	let mut suffix = <K::Suffix as BoundedKey>::low();
	for step in 0..4 {
...
mod tests {
	...
	#[test]
	fn exactly_twenty_seven_of_the_forty_nine_keyspaces_are_group_scoped() {
