...
impl<T: Clone + PartialEq> PartialEq<AsyncCowVec<T>> for [T] {
    fn eq(&self, other: &AsyncCowVec<T>) -> bool {
        self == other.inner.as_slice()
    }
}

impl<T: Clone + PartialEq> Clone for AsyncCowVec<T> {
    fn clone(&self) -> Self {
        AsyncCowVec { inner: Arc::clone(&self.inner) }
    }
}

impl<T: Clone + PartialEq> AsyncCowVec<T> {
    pub fn new(vec: Vec<T>) -> Self {
        AsyncCowVec { inner: Arc::new(vec) }
    }

    pub fn from_rc(rc: Arc<Vec<T>>) -> Self {
        AsyncCowVec { inner: rc }
    }
...
impl<T: Clone + PartialEq> IntoIterator for AsyncCowVec<T> {
    type Item = T;
...
impl<T: Clone + PartialEq> Deref for AsyncCowVec<T> {
    type Target = [T];
...
impl<T> Serialize for AsyncCowVec<T>
where
...
impl<'de, T> Deserialize<'de> for AsyncCowVec<T>
where
    T: Clone + PartialEq + Deserialize<'de>,
{
        ...
        D: Deserializer<'de>,
    {
        ...
        Ok(AsyncCowVec { inner: Arc::new(vec) })
    }
}

#[cfg(test)]
mod tests {
    use crate::cowvec::r#async::AsyncCowVec;

    #[test]
    fn test_new() {
        let cow = AsyncCowVec::new(vec![1, 2, 3]);
    ...
    #[test]
    fn test_is_owned() {
        let owned = AsyncCowVec::new(vec![1, 2]);
    ...
    #[test]
    fn test_extend() {
        let mut owned = AsyncCowVec::new(vec![1, 2]);
    ...
    fn test_reorder_identity() {
        let mut cow = AsyncCowVec::new(vec![10, 20, 30]);
    ...
    fn ptr_of(v: &AsyncCowVec<i32>) -> *const i32 {
        v.as_slice().as_ptr()
    }
}
