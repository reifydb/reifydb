...
use crate::Value::{Uint1, Uint2, Uint16, Undefined};
use crate::value::IsNumber;
use crate::{BitVec, CowVec, Value};
use Value::{Int1, Int2, Int4, Int8, Int16, Uint4, Uint8};
        ...
        T: 'static,
    {
        if index < self.len() && self.is_defined(index) {
            ...
            if TypeId::of::<T>() == TypeId::of::<f32>() {
                ...
                crate::value::OrderedF32::try_from(f_val).map(Value::Float4).unwrap_or(Undefined)
            } else if TypeId::of::<T>() == TypeId::of::<f64>() {
                let f_val = unsafe { transmute_copy::<T, f64>(&value) };
                crate::value::OrderedF64::try_from(f_val).map(Value::Float8).unwrap_or(Undefined)
...
mod tests {
    ...
    fn test_push_with_convert() {
        ...
        container
            .push_with_convert(42u32, |x| if x <= i32::MAX as u32 { Some(x as i32) } else { None });

        // Failed conversion
        container.push_with_convert(u32::MAX, |x| {
            if x <= i32::MAX as u32 { Some(x as i32) } else { None }
        });
