...
use crate::frame::{
	ColumnQualified, ColumnValues, Frame, FrameColumn, FrameColumnLayout, TableQualified, FullyQualified, Unqualified,
};
...
impl Frame {
    pub fn apply_layout(&mut self, layout: &FrameLayout) {
        ...
        for (i, column_layout) in layout_with_qualification.columns.iter().enumerate() {
            if i < self.columns.len() {
                ...
                let values = std::mem::replace(column.values_mut(), ColumnValues::with_capacity(column_layout.ty, 0));
    ...
    fn qualify_duplicates_only(&self, layout: &FrameLayout) -> FrameLayout {
        ...
        let qualified_columns: Vec<_> = layout.columns.iter().map(|column_layout| {
            ...
            if has_duplicates && column_layout.schema.is_none() && column_layout.table.is_none() {
                ...
                if let Some(existing_column) = self.columns.iter().find(|c| c.name() == column_layout.name) {
