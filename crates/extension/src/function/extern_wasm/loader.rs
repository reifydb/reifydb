...
use super::ExternWasmScalarFunction;

pub fn register_extern_wasm_scalar_functions_from_dir(
	dir: &Path,
	mut builder: RoutinesConfigurator,
) -> Result<RoutinesConfigurator> {
	let entries = fs::read_dir(dir).map_err(|e| {
		SdkError::Other(format!("Failed to read WASM scalar function directory {}: {}", dir.display(), e))
	})?;
	...
	for entry in entries {
		...
		builder = builder.register_function(Arc::new(ExternWasmScalarFunction::new(name, wasm_bytes)));
	}
