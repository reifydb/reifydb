...
///
/// # Constraint Semantics (AND logic)
/// - `(None, None)`: Drop ALL versions
/// - `(Some(v), None)`: Drop versions where version < v
...
mod tests {
	...
	fn setup_versioned_entries(storage: &BackendStorage, table: TableId, key: &[u8], versions: &[u64]) {
		...
		let refs: Vec<_> =
			entries.iter().map(|(k, v)| (k.as_slice(), v.as_ref().map(|v| v.as_slice()))).collect();
