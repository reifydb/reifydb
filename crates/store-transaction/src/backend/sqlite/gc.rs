...
impl BackendGarbageCollect for SqliteBackend {
	fn compact_operator_states(&self) -> Result<GcStats> {
		...
		let rows = stmt
			...
			.map_err(|e| reifydb_type::Error(database_error(format!("Failed to fetch table names: {}", e))))?;

		for row in rows {
			match row {
				Ok(name) => operator_tables.push(name),
				Err(e) => {
					return Err(reifydb_type::Error(database_error(format!("Failed to read table name: {}", e))));
				}
			}
		}
		...
		for table_name in operator_tables {
			...
			let key_count: usize = conn
				.query_row(&count_query, [], |row| row.get(0))
				.map_err(|e| reifydb_type::Error(database_error(format!("Failed to count keys: {}", e))))?;
			...
			let query = format!(
				"SELECT key, version FROM {}
				 WHERE (key, version) NOT IN (
					 SELECT key, MAX(version) FROM {} GROUP BY key
				 )
				 LIMIT 1024",
			...
			.map_err(|e| reifydb_type::Error(database_error(format!("Failed to query old versions: {}", e))))?;
			...
			for row in rows {
				match row {
					...
					Err(e) => {
						return Err(reifydb_type::Error(database_error(format!("Failed to read row: {}", e))));
					}
				}
			}
			...
			println!("[GC-SQLite] Table '{}': Found {} old versions to delete (keys={})",
				table_name, deletions.len(), key_count);

			// Phase 2: Send to writer actor for deletion
			let (sender, receiver) = mpsc::channel();

			self.writer.send(WriteCommand::GarbageCollect {
				deletions,
				respond_to: sender,
			})
