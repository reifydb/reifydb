...
impl TierStorage for MemoryPrimitiveStorage {
	...
	#[instrument(name = "store::memory::set", level = "trace", skip(self, batches), fields(
		...
		publish_time_us = tracing::field::Empty
	))]
	fn set(&self, batches: HashMap<EntryKind, Vec<(CowVec<u8>, Option<CowVec<u8>>)>>) -> Result<()> {
		...
		for (table, entries) in sorted_batches {
			...
			// Measure lock acquisition time (writer-only contention)
			...
			// Measure insert time (appending to oplog)
		...
		let publish_span = debug_span!("store::memory::set::publish");
		...
		Span::current().record("publish_time_us", publish_us);
