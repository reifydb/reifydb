...
impl Hooks {
		...
		C: Callback<H>,
	{
		...
		self.callbacks
			...
			.entry(type_id)
			.or_insert_with(|| {
				Box::new(CallbackListImpl::<H>::new())
			})
			.as_any_mut()
		...
		H: Hook,
	{
		...
		while let Some(hook) = queue.pop() {
			...
			if let Some(handler_list) = callbacks.get(&type_id) {
				let new_hooks =
					handler_list.on_any(hook.as_any())?;
...
mod tests {
	...
	fn test_return_hooks_macro_single() {
		let result: crate::Result<BoxedHookIter> =
			return_hooks!(TestHook {});
		let mut iter = result.unwrap();
	...
	fn test_return_hooks_macro_multiple() {
		let result: crate::Result<BoxedHookIter> =
			return_hooks!(TestHook {}, AnotherHook {});
		let mut iter = result.unwrap();
		assert!(iter.next().is_some());
		assert!(iter.next().is_some());
		assert!(iter.next().is_none());
	...
	fn test_hook_pool_capacity_limit() {
		...
		for _ in 0..100 {
			large_queue
				.push(Box::new(TestHook {}) as Box<dyn Hook>);
		}
	...
	fn test_subscribe_to_many_hooks() {
		...
		test_instance.register::<TestHook, TestCallback>(
			test_callback.clone(),
		);
		test_instance.register::<AnotherHook, TestCallback>(
			test_callback.clone(),
		);
