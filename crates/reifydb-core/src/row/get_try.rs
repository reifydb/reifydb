...
mod tests {
	...
	fn test_try_get_interleaved_static_and_dynamic_setting() {
		...
		// Interleave static and dynamic field setting, with some
		// undefined
		layout.set_bool(&mut row, 0, true);
	...
	fn test_try_get_mixed_temporal_fields() {
		...
		assert_eq!(
			layout.try_get_date(&row, 0),
			Some(test_date.clone())
		);
		assert_eq!(layout.try_get_datetime(&row, 1), None);
		assert_eq!(
			layout.try_get_time(&row, 2),
			Some(test_time.clone())
		);
		assert_eq!(layout.try_get_interval(&row, 3), None);

		// Set remaining fields
		let test_datetime =
			DateTime::from_timestamp(1721030130).unwrap();
		...
		assert_eq!(
			layout.try_get_datetime(&row, 1),
			Some(test_datetime)
		);
		assert_eq!(
			layout.try_get_interval(&row, 3),
			Some(test_interval)
		);
	}
