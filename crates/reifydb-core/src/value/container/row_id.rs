...
mod tests {
	...
	fn test_from_vec() {
		let row_ids =
			vec![RowId::new(10), RowId::new(20), RowId::new(30)];
	...
	fn test_extend() {
		...
		let mut container1 =
			RowIdContainer::from_vec(vec![row_id1, row_id2]);
	...
	fn test_iter() {
		...
		assert_eq!(
			collected,
			vec![Some(row_ids[0]), None, Some(row_ids[2])]
		);
	}
