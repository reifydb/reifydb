...
use crate::{
	BorrowedSpan, Error, Span, Time, result::error::diagnostic::temporal,
	return_error,
};
...
pub fn parse_time(span: impl Span) -> Result<Time, Error> {
	...
	// Create a new span with the trimmed fragment but preserve original
	// position
	let time_span = BorrowedSpan {
		column: span.column(),
		line: span.line(),
		fragment: time_str,
	};
	...
	Time::new(hour, minute, second, nanosecond).ok_or_else(|| {
		Error(temporal::invalid_time_values(span.to_owned()))
	})
}
