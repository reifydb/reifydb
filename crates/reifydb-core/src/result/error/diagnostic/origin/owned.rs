...
impl OwnedSpan {
	...
	pub fn merge_all(
		spans: impl IntoIterator<Item = OwnedSpan>,
	) -> OwnedSpan {
		...
		let mut fragment = String::with_capacity(
			spans.iter().map(|s| s.fragment.len()).sum(),
		);
		for span in &spans {
	...
	/// part. Each resulting span will have the correct column offset and
	/// fragment.
	pub fn split(&self, delimiter: char) -> Vec<OwnedSpan> {
		...
		for part in parts {
			...
			// Move column forward by part length + 1 (for the
			// delimiter)
...
mod tests {
	mod merge_all {
		...
		fn test_merge_three_spans_out_of_order() {
			...
			let merged =
				OwnedSpan::merge_all([span1, span2, span3]);
