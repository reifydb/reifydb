...
impl DiagnosticOrigin {
	...
	pub fn fragment(&self) -> Option<&str> {
		match self {
			...
			DiagnosticOrigin::Statement { fragment, .. } |
			DiagnosticOrigin::Internal { fragment, .. } => Some(fragment),
		}
	}
	...
	pub fn location_info(&self) -> Option<(&str, &str, u32)> {
		match self {
			DiagnosticOrigin::None => None,
			DiagnosticOrigin::Statement { module, file, location_line, .. } => 
				Some((module.as_str(), file.as_str(), *location_line)),
			DiagnosticOrigin::Internal { module, file, line, .. } => 
...
pub trait Span: Clone {
	type SubSpan: Span + IntoOwnedSpan + IntoDiagnosticOrigin;
...
mod tests {
	...
	fn test_option_owned_span_to_diagnostic_origin() {
		...
		match origin {
			DiagnosticOrigin::Statement { line, column, fragment, .. } => {
