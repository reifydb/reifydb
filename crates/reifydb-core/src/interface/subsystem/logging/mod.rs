...
pub fn log(record: Record) {
	...
	if let Some(logger) = logger() {
		...
		if let Ok(mut buffer) = get_log_buffer().lock() {
			// Only buffer up to MAX_BUFFERED_RECORDS to prevent unbounded memory growth
			if buffer.len() < MAX_BUFFERED_RECORDS {
				buffer.push(record);
			}
			// If we've reached the limit, we silently drop new records
			// This prevents memory exhaustion if logging is misconfigured
		}
	}
}
