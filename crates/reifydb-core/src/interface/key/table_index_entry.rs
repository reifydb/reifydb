...
use crate::index::{EncodedIndexKey, EncodedIndexKeyRange};
...
impl TableIndexEntryKey {
    pub fn new(table: TableId, index: IndexId, key: EncodedIndexKey) -> Self {
...
impl EncodableKey for TableIndexEntryKey {
    ...
    fn decode(key: &EncodedKey) -> Option<Self> {
        ...
        let index: IndexId = keycode::deserialize(&payload[8..16]).ok()?;
        ...
        if payload.len() > 16 {
            ...
            Some(Self { table, index, key: index_key })
        } else {
            None
        }
    }
...
impl TableIndexEntryKey {
    ...
    pub fn index_range(table: TableId, index: IndexId) -> EncodedKeyRange {
        ...
        // For the end key, we append 0xFF to ensure we get all keys for this index
        // This works because any actual index key data will make the key longer
    ...
    pub fn key_prefix_range(table: TableId, index: IndexId, key_prefix: &[u8]) -> EncodedKeyRange {
    ...
    /// This method leverages the EncodedIndexKeyRange type for cleaner range handling.
    pub fn key_range(
        table: TableId,
        index: IndexId,
        ...
        let end = match index_range.end {
            ...
            Bound::Unbounded => {
                ...
                bytes.extend(&keycode::serialize(&KeyKind::TableIndexEntry));
...
mod tests {
    ...
    #[test]
    fn test_index_range() {
        let range = TableIndexEntryKey::index_range(TableId(10), IndexId(5));
        ...
        // Note: Due to keycode encoding, IndexId(6) will have a smaller encoded value than IndexId(5)
        // since keycode inverts bits (larger numbers become smaller byte sequences)
        let entry2 = TableIndexEntryKey {
            table: TableId(10),
            index: IndexId(6),
        ...
        let encoded2 = entry2.encode();
        // The entry with IndexId(6) should not be within the range for IndexId(5)
        if let (Bound::Included(start), Bound::Excluded(end)) = (&range.start, &range.end) {
    ...
    fn test_key_prefix_range() {
        ...
        let range = TableIndexEntryKey::key_prefix_range(TableId(1), IndexId(1), prefix);
