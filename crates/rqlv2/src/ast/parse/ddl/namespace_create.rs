...
use crate::{
	...
	token::{span::Span, token::TokenKind},
};
...
impl<'bump, 'src> Parser<'bump, 'src> {
	...
	pub(in crate::ast::parse) fn parse_create_namespace(
		...
		Ok(Statement::Create(CreateStmt::Namespace(CreateNamespace::new(name, if_not_exists, span))))
	}
}
...
pub mod tests {
	...
	fn test_create_namespace_if_not_exists_before() {
		...
		let program =
			crate::ast::parse::parse(&bump, &result.tokens, "CREATE NAMESPACE IF NOT EXISTS mydb").unwrap();
		let stmt = program.statements.first().copied().unwrap();
		match stmt {
			Statement::Create(crate::ast::stmt::ddl::CreateStmt::Namespace(ns)) => {
				assert_eq!(ns.name, "mydb");
	...
	fn test_create_namespace_if_not_exists_after() {
		...
		let program =
			crate::ast::parse::parse(&bump, &result.tokens, "CREATE NAMESPACE mydb IF NOT EXISTS").unwrap();
		let stmt = program.statements.first().copied().unwrap();
		match stmt {
			Statement::Create(crate::ast::stmt::ddl::CreateStmt::Namespace(ns)) => {
				assert_eq!(ns.name, "mydb");
