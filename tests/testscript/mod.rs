...
test_each_path! { in "tests/testscript/scripts" as scripts => test_testscript }
...
test_each_path! { for ["in", "out"] in "tests/testscript/generate" as generate => test_generate }
...
test_each_path! { for ["in", "error"] in "tests/testscript/errors" as errors => test_error }

fn test_error([in_path, out_path]: [&std::path::Path; 2]) {
    let input = std::fs::read_to_string(in_path).expect("failed to read file");
    let run =
        std::panic::AssertUnwindSafe(|| testscript::generate(&mut DebugRunner::new(), &input));
...
#[test]
fn btreemap() {
    testscript::run(&mut BTreeMapRunner::default(), "tests/testscript/btreemap")
        .expect("testscript failed")
}
