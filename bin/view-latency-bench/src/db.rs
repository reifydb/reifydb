...
pub fn build(workload: Workload, kind: Kind, n: usize, operators_dir: Option<PathBuf>, config: &Config) -> Database {
	...
	let db = match operators_dir {
		Some(dir) => embedded::memory()
			.with_config(ConfigKey::FlowTick, tick)
			.with_config(ConfigKey::ThreadsCommit, threads.clone())
			.with_config(ConfigKey::ThreadsSystem, threads)
			.with_flow(move |f| f.operators_dir(dir))
			.build(),
		None => embedded::memory()
			.with_config(ConfigKey::FlowTick, tick)
			.with_config(ConfigKey::ThreadsCommit, threads.clone())
			.with_config(ConfigKey::ThreadsSystem, threads)
			.with_flow(|f| f)
