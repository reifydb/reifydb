...
//     pub async fn create_sqlite_db(path: Option<&str>) -> Result<reifydb::Database> {
//         use reifydb::presets::Presets;
//
...
//             let db_path = temp_dir.join(format!("reifydb_example_{}.db", uuid::Uuid::new_v4()));
//             reifydb::Database::builder()
...
//                     print!(" | ");
//                 }
//                 let value = row.get(col.name()).unwrap_or(&reifydb::Value::Null);
...
//         pub async fn create_products_table(db: &reifydb::Database) -> Result<()> {
//             db.execute(
...
//         pub async fn create_orders_table(db: &reifydb::Database) -> Result<()> {
//             db.execute(
//                 "CREATE TABLE orders (
//                     id INTEGER PRIMARY KEY,
...
//
//         /// Insert sample users
//         pub async fn insert_sample_users(db: &reifydb::Database) -> Result<()> {
//             let users = vec![
...
//         pub async fn insert_sample_products(db: &reifydb::Database) -> Result<()> {
//             let products = vec![
