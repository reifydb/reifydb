...
fn main() {
    ...
    db.tx_as_root(r#"
create schema test;
create table test.one(field: int1, other: int1);
create table test.two(field: int1, name: text);
create table test.three(field: int1, type: text);
from [{field: 1, other: 2}, {field: 2, other: 2}, {field: 3, other: 2}, {field: 4, other: 2}, {field: 5, other: 2}] insert test.one;
from [{field: 2, name: 'Peter'}, {field: 5, name: 'Parker'}] insert test.two;
from [{field: 1, type: 'Engineer' },{field: 5, type: 'Barker'}] insert test.three;
    "#).unwrap();
    let l = db
        .tx_as_root(
            r#"
from test.one
    ...
    for (i, col) in frame.columns.iter().enumerate() {
        println!("  [{}] name: '{}', qualified_name: '{}', frame: {:?}", 
                 i, col.name(), col.qualified_name(), col.table());
    }
