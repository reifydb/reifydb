...
use reifydb_type::{
	...
	diagnostic::operator::div_cannot_be_applied_to_incompatible_types, return_error,
};

use crate::evaluate::{EvaluationContext, StandardEvaluator};

impl StandardEvaluator {
	pub(crate) fn div(&self, ctx: &EvaluationContext, div: &DivExpression) -> crate::Result<Column> {
		...
		match (&left.data(), &right.data()) {
			...
			(ColumnData::Float4(l), ColumnData::Float4(r)) => {
				div_numeric(ctx, l, r, target, || div.full_fragment_owned())
			}
			...
			) => div_numeric_clone(ctx, l, r, target, || div.full_fragment_owned()),
			(
				ColumnData::Int {
					container: l,
					..
			...
			(ColumnData::Undefined(l), _) => Ok(Column::ColumnQualified(ColumnQualified {
				name: div.full_fragment_owned().fragment().into(),
