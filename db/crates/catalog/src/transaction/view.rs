...
use reifydb_core::{
	interface::{
		...
		TransactionalChanges, TransactionalViewChanges, ViewDef,
		ViewId,
...
use crate::{
	CatalogNamespaceQueryOperations, CatalogStore,
	transaction::MaterializedCatalogTransaction, view::ViewToCreate,
};
...
> CatalogViewQueryOperations for QT
{
	fn find_view(
		...
		if let Some(view) =
			TransactionalViewChanges::find_view(self, id)
		{
	...
	fn find_view_by_name<'a>(
		...
		// 1. Check transactional changes first
		// nop for QueryTransaction
		if let Some(view) = TransactionalViewChanges::find_view_by_name(
			self,
			namespace,
			name.as_borrowed(),
		) {
		...
		// 2. Check if deleted
		// nop for QueryTransaction
		if TransactionalViewChanges::is_view_deleted_by_name(
			self,
			namespace,
			name.as_borrowed(),
		) {
		...
		self.find_view(id)?
			.ok_or_else(|| {
				error!(internal_error!(
							"View with ID {:?} not found in catalog. This indicates a critical catalog inconsistency.", id
						))
			})
	}
