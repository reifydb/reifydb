...
use reifydb_core::{
	interface::{
		CommandTransaction, NamespaceDef, NamespaceId,
		QueryTransaction, TransactionalChanges,
...
use crate::{
	CatalogStore, namespace::NamespaceToCreate,
...
> CatalogNamespaceQueryOperations for QT
{
	fn find_namespace(
		...
		if let Some(namespace) =
			TransactionalNamespaceChanges::find_namespace(self, id)
		{
		...
		if let Some(namespace) =
			self.catalog().find_namespace(id, self.version())
		{
	...
	fn find_namespace_by_name<'a>(
		...
		// nop for QueryTransaction
		if let Some(namespace) =
			TransactionalNamespaceChanges::find_namespace_by_name(
				self,
				name.as_borrowed(),
			) {
			return Ok(Some(namespace.clone()));
		...
		// 2. Check if deleted
		// nop for QueryTransaction
		if TransactionalNamespaceChanges::is_namespace_deleted_by_name(
			self,
			name.as_borrowed(),
		) {
			return Ok(None);
		...
		if let Some(namespace) = self
			.catalog()
			.find_namespace_by_name(name.text(), self.version())
		{
			return Ok(Some(namespace));
		}
		...
		// 4. Fall back to storage as defensive measure
		if let Some(namespace) =
			CatalogStore::find_namespace_by_name(self, name.text())?
		{
			log_warn!(
		...
		self.find_namespace(id)?
					.ok_or_else(|| {
						error!(internal_error!(
							"Namespace with ID {} not found in catalog. This indicates a critical catalog inconsistency.", id
						))
					})
	}
	...
	fn get_namespace_by_name<'a>(
		...
		let name = name.into_fragment();
		self.find_namespace_by_name(name.as_borrowed())?.ok_or_else(
			|| {
