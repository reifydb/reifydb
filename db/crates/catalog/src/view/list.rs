...
use reifydb_core::interface::{
	Key, NamespaceId, QueryTransaction, ViewDef, ViewKey, ViewKind,
};
...
impl CatalogStore {
	pub fn list_views_all(
		...
		let entries: Vec<_> =
			rx.range(ViewKey::full_scan())?.into_iter().collect();
		...
		for entry in entries {
			if let Some(key) = Key::decode(&entry.key) {
				if let Key::View(view_key) = key {
					...
					let kind_value = view::LAYOUT
						.get_u8(&entry.row, view::KIND);
