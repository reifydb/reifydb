...
impl MaterializedCatalog {
	...
	pub fn find_view_by_name(
		...
		self.views_by_name.get(&(namespace, name.to_string())).and_then(
			|entry| {
...
mod tests {
	...
	fn test_find_view_by_name() {
		...
		let namespace_id = NamespaceId(1);
		let view =
			create_test_view(view_id, namespace_id, "named_view");

		// Set view
		catalog.set_view(view_id, 1, Some(view.clone()));

		// Find by name
		let found = catalog.find_view_by_name(
			namespace_id,
			"named_view",
			1,
		);
	...
	fn test_view_rename() {
		...
		let view_v1 =
			create_test_view(view_id, namespace_id, "old_name");
		catalog.set_view(view_id, 1, Some(view_v1.clone()));

		// Verify initial state
		assert!(catalog
			.find_view_by_name(namespace_id, "old_name", 1)
			.is_some());
		assert!(catalog
			.find_view_by_name(namespace_id, "new_name", 1)
			.is_none());
		...
		assert!(catalog
			.find_view_by_name(namespace_id, "old_name", 2)
			.is_none());

		// New name can be found
		assert_eq!(
			catalog.find_view_by_name(namespace_id, "new_name", 2),
			Some(view_v2.clone())
		);
	...
	fn test_view_move_between_namespaces() {
		...
		let view_v1 =
			create_test_view(view_id, namespace1, "movable_view");
		catalog.set_view(view_id, 1, Some(view_v1.clone()));

		// Verify it's in namespace1
		assert!(catalog
			.find_view_by_name(namespace1, "movable_view", 1)
			.is_some());
		assert!(catalog
			.find_view_by_name(namespace2, "movable_view", 1)
			.is_none());
		...
		assert!(catalog
			.find_view_by_name(namespace1, "movable_view", 2)
			.is_none());

		// Should now be in namespace2
		assert!(catalog
			.find_view_by_name(namespace2, "movable_view", 2)
			.is_some());
	}
	...
	fn test_view_deletion() {
		...
		assert!(catalog
			.find_view_by_name(namespace_id, "deletable_view", 1)
			.is_some());
		...
		assert!(catalog
			.find_view_by_name(namespace_id, "deletable_view", 2)
			.is_none());
	...
	fn test_multiple_views_in_namespace() {
		...
		assert_eq!(
			catalog.find_view_by_name(namespace_id, "view1", 1),
			Some(view1)
		);
		assert_eq!(
			catalog.find_view_by_name(namespace_id, "view2", 1),
			Some(view2)
		);
		assert_eq!(
			catalog.find_view_by_name(namespace_id, "view3", 1),
			Some(view3)
		);
	}
	...
	fn test_view_versioning() {
		...
		let view_v1 =
			create_test_view(view_id, namespace_id, "view_v1");
		let mut view_v2 = view_v1.clone();
