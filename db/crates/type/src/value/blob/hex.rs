...
impl Blob {
	pub fn from_hex<'a>(fragment: impl IntoFragment<'a>) -> Result<Self, Error> {
...
mod tests {
	...
	#[test]
	fn test_from_hex() {
		let blob = Blob::from_hex(OwnedFragment::testing("48656c6c6f")).unwrap();
		assert_eq!(blob.as_bytes(), b"Hello");
	}

	#[test]
	fn test_from_hex_with_prefix() {
		let blob = Blob::from_hex(OwnedFragment::testing("0x48656c6c6f")).unwrap();
		assert_eq!(blob.as_bytes(), b"Hello");
	...
	fn test_from_hex_invalid() {
		...
		let result = Blob::from_hex(OwnedFragment::testing("48656c6c6")); // odd length
		assert!(result.is_err());
	}
	...
	fn test_hex_roundtrip() {
		...
		let decoded = Blob::from_hex(OwnedFragment::testing(&hex_str)).unwrap();
