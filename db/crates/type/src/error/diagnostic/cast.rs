...
pub fn unsupported_cast<'a>(fragment: impl IntoFragment<'a>, from_type: Type, to_type: Type) -> Diagnostic {
...
pub fn invalid_number<'a>(fragment: impl IntoFragment<'a>, target: Type, cause: Diagnostic) -> Diagnostic {
	let fragment = fragment.into_fragment().into_owned();
...
pub fn invalid_temporal<'a>(fragment: impl IntoFragment<'a>, target: Type, cause: Diagnostic) -> Diagnostic {
	let fragment = fragment.into_fragment().into_owned();
	let label = Some(format!("failed to cast to {}", target));
	Diagnostic {
...
pub fn invalid_boolean<'a>(fragment: impl IntoFragment<'a>, cause: Diagnostic) -> Diagnostic {
	let fragment = fragment.into_fragment().into_owned();
...
pub fn invalid_uuid<'a>(fragment: impl IntoFragment<'a>, target: Type, cause: Diagnostic) -> Diagnostic {
	let fragment = fragment.into_fragment().into_owned();
	let label = Some(format!("failed to cast to {}", target));
	Diagnostic {
...
pub fn invalid_blob_to_utf8<'a>(fragment: impl IntoFragment<'a>, cause: Diagnostic) -> Diagnostic {
	let fragment = fragment.into_fragment().into_owned();
