    ...
    I: Iterator,
{
    ...
    pub fn zip<J>(self, other: CursorIter<J>) -> CursorIter<impl Iterator<Item = (I::Item, J::Item)>>
    where
        J: Iterator,
    {
...
fn load_rows(offset: usize, limit: usize) -> Vec<CursorRow> {
    ...
    (offset + 1..end + 1).map(|i| CursorRow { id: i as u32, data: format!("data_{}", i) }).collect::<Vec<_>>()
}
