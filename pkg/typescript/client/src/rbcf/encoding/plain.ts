...
export function decodeAnyValue(data: Uint8Array, pos: number): { value: string; nextPos: number } {
    const tag = data[pos];
    pos += 1;
    switch (tag) {
        case TYPE_CODE.None: {
            const inner = decodeTypeInfo(data, pos);
            return { value: NONE_VALUE, nextPos: inner.nextPos };
        }
        ...
        case TYPE_CODE.DictionaryId: {
            ...
            switch (width) {
                ...
                case 2: v = readU16(data, pos); break;
                case 4: v = readU32(data, pos); break;
                case 8: v = readU64(data, pos); break;
                case 16: v = readU128(data, pos); break;
                default: throw new Error(`RBCF: invalid DictionaryId width ${width}`);
            }
            return { value: v.toString(), nextPos: pos + width };
        }
        case TYPE_CODE.Type: {
            const info = decodeTypeInfo(data, pos);
            return { value: info.name, nextPos: info.nextPos };
        }
        case TYPE_CODE.List:
        case TYPE_CODE.Tuple: {
            ...
            for (let i = 0; i < count; i++) {
                const item = decodeAnyValue(data, pos);
                items[i] = item.value;
                pos = item.nextPos;
            }
            const open = tag === TYPE_CODE.List ? "[" : "(";
            const close = tag === TYPE_CODE.List ? "]" : ")";
            return { value: `${open}${items.join(", ")}${close}`, nextPos: pos };
        }
        case TYPE_CODE.Record: {
            ...
            for (let i = 0; i < count; i++) {
                const keyLen = readU32(data, pos);
                pos += 4;
                const key = new TextDecoder("utf-8").decode(data.subarray(pos, pos + keyLen));
                pos += keyLen;
                const field = decodeAnyValue(data, pos);
                pos = field.nextPos;
                fields[i] = `${key}: ${field.value}`;
            }
            return { value: `{${fields.join(", ")}}`, nextPos: pos };
        }
        default:
