...
const ns = `digest_http_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
...
function connect(format: "frames" | "rbcf"): HttpClient {
    return Client.connectHttp(process.env.REIFYDB_HTTP_URL, {
        timeoutMs: 10000,
...
describe('Digest across encodings [http]', () => {
    it('gives byte-equal digests over the frames and rbcf encodings', async () => {
        ...
        const frames = connect("frames");
        const rbcf = connect("rbcf");
