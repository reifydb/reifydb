...
const ns = `digest_ws_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
...
async function connect(format: "frames" | "rbcf"): Promise<WsClient> {
...
describe.each([
    {format: "frames"},
    {format: "rbcf"},
] as const)('Digest [$format]', ({format}) => {
    let wsClient: WsClient;

    beforeEach(async () => {
        wsClient = await connect(format);
    }, 15000);
    ...
    it('decodes a digest per group with its inner type, accuracy and count', async () => {
        ...
        const [rows] = await wsClient.query(DIGESTS, null, []);
    ...
    it('matches the bytes the server builds again from reordered rows, a merge and an echoed parameter', async () => {
        ...
        for (const [i, row] of (rows as any[]).entries()) {
            ...
            const [[echoed]] = await wsClient.query('map { echoed: $1 }', [row.amounts], []);
    ...
    it('reads the same percentile from a positional digest parameter as from one query', async () => {
        ...
        const [rows] = await wsClient.query(DIGESTS, null, []);
        const [truth] = await wsClient.query(PERCENTILES, null, []);
        ...
        for (const [i, row] of (rows as any[]).entries()) {
            for (const param of [row.amounts, Option.some(row.amounts)]) {
                const [[read]] = await wsClient.query(
    ...
    it('reads a percentile of a duration digest parameter back as a duration', async () => {
        ...
        for (const [i, row] of (rows as any[]).entries()) {
            const [[positional]] = await wsClient.query('map { p99: stats::approx_percentile($1, 0.99) }', [row.latencies], []);
            const [[named]] = await wsClient.query('map { p99: stats::approx_percentile($d, 0.99) }', {d: row.latencies}, []);
    ...
    it('decodes an all-none group as none and sends it back as a none parameter', async () => {
        ...
        for (const {rql, params} of sent) {
            const [[read]] = await wsClient.query(rql, params, []);
    ...
    it('rejects a malformed digest parameter instead of reading it as none', async () => {
        ...
        const [[control]] = await wsClient.query(rql, [rows[0].amounts], []);
        ...
        for (const {param, text} of cases) {
            await expect(wsClient.query(rql, [param], [])).rejects.toMatchObject({
                name: "ReifyError",
                code: "INVALID_PARAMS",
...
describe('Digest across encodings [ws]', () => {
    it('gives byte-equal digests over the frames and rbcf encodings', async () => {
        // The frames hex cell and the rbcf binary column must carry the same canonical bytes for every digest.
        const frames = await connect("frames");
        const rbcf = await connect("rbcf");
        try {
            const [jsonRows] = await frames.query(DIGESTS, null, []);
