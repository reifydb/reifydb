...
const ns = `digest_json_ws_${Date.now()}_${Math.random().toString(36).slice(2, 8)}`;
...
async function connect(): Promise<JsonWsClient> {
    return Client.connectJsonWs(process.env.REIFYDB_WS_URL, {
        timeoutMs: 10000,
...
describe('Digest across transports [json-ws]', () => {
    it('gives byte-equal digests over json and rbcf', async () => {
        ...
        const rbcf: WsClient = await Client.connectWs(process.env.REIFYDB_WS_URL, {
            timeoutMs: 10000,
            token: process.env.REIFYDB_TOKEN,
            format: "rbcf",
        });
