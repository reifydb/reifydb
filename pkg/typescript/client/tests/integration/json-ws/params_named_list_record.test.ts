...
describe('Named Parameters (List of Records)', () => {
    ...
    it('admin round-trips a List(Record) param', async () => {
        const frames = await wsClient.admin('MAP {result: $regions}', {regions: regions()});
        expectSingleValueResult(frames, regions());
    }, 1000);

    it('command round-trips a List(Record) param', async () => {
        const frames = await wsClient.command('MAP {result: $regions}', {regions: regions()});
        expectSingleValueResult(frames, regions());
    }, 1000);

    it('query round-trips a List(Record) param', async () => {
        const frames = await wsClient.query('MAP {result: $regions}', {regions: regions()});
        expectSingleValueResult(frames, regions());
    }, 1000);
});
