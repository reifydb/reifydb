...
describe('JsonWs call RPC', () => {
    ...
    beforeAll(async () => {
        ...
        client = await Client.connectJsonWs(WS_URL, {timeoutMs: 10000, token: AUTH_TOKEN});
        ...
        await client.admin(`CREATE WS BINDING ${ns}::greet_ws FOR ${ns}::greet WITH { name: "${greetBinding}", format: "json" }`);
        await client.admin(`CREATE WS BINDING ${ns}::echo_ws FOR ${ns}::echo WITH { name: "${echoBinding}", format: "json" }`);
    }, 30000);
    ...
    it('rejects a missing required param with INVALID_PARAMS', async () => {
        await expect(client.call(echoBinding, {})).rejects.toMatchObject({
            name: 'ReifyError',
            code: 'INVALID_PARAMS',
        });
    }, 10000);
