...
describe('WebSocket Client Reconnection', () => {
    ...
    describe('Automatic Reconnection', () => {
        ...
        it('should reconnect after connection is lost', async () => {
            wsClient = await Client.connectJsonWs(WS_URL, {
                timeoutMs: 10000,
                token: AUTH_TOKEN,
            ...
            const firstResult = await wsClient.query('MAP {result: 42}');

            expect(firstResult[0][0].result.value).toBe(42);
            ...
            const secondResult = await wsClient.query('MAP {result: 84}');

            expect(secondResult[0][0].result.value).toBe(84);
        }, 15000);
        ...
        it('should stop reconnecting after max attempts', async () => {
            ...
            wsClient = await Client.connectJsonWs('ws://127.0.0.1:9999', {
                timeoutMs: 1000,
                token: AUTH_TOKEN,
        ...
        it('should reject pending requests when connection is lost', async () => {
            ...
            const queryPromise = wsClient.query('MAP {result: 42}');
        ...
        it('should successfully execute queries after reconnection', async () => {
            ...
            const results = [];
            for (let i = 0; i < 3; i++) {
                const result = await wsClient.query(`MAP {result: ${i}}`);
                results.push(result[0][0].result.value);
            }
        ...
        it('should reset reconnection attempts counter after successful reconnection', async () => {
            ...
            await wsClient.query('MAP {result: 1}');
            ...
            await wsClient.query('MAP {result: 2}');
