...
export function useSubscriptionExecutor<T = any>(
    ...
    const rqlRef = useRef<string | undefined>(undefined);
    const paramsRef = useRef<any>(undefined);
    const shapeRef = useRef<ShapeNode | undefined>(undefined);
    const configRef = useRef<SubscriptionConfig | undefined>(undefined);
    ...
    const subscribe = useCallback(async (
        ...
        try {
            const subId = await currentClient.subscribe(rql, params, shape, {
