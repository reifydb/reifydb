...
import {Float4Value} from '../../src/value/float4';

describe('Float4Value', () => {
    describe('constructor', () => {
        it('should create instance with valid value', () => {
            const float4 = new Float4Value(3.14159);
        ...
        it('should accept zero', () => {
            const float4 = new Float4Value(0);
        ...
        it('should convert to Float32 precision', () => {
            ...
            const float4 = new Float4Value(3.141592653589793);
        ...
        it('should throw error for values above max', () => {
            expect(() => new Float4Value(3.5e38)).toThrow('Float4 overflow: value 3.5e+38 exceeds maximum 3.4028235e+38');
        });

        it('should throw error for values below min', () => {
            expect(() => new Float4Value(-3.5e38)).toThrow('Float4 underflow: value -3.5e+38 exceeds minimum -3.4028235e+38');
        });

        it('should underflow to zero for very small positive values', () => {
            const float4 = new Float4Value(1e-39);
            expect(float4.value).toBe(0);
        });
        ...
        it('should preserve values near Float32 min positive', () => {
            const float4 = new Float4Value(1.175494e-38);
            expect(float4.value).toBeGreaterThan(0);
            expect(float4.value).toBeLessThan(1.2e-38);
        });

        it('should throw error for non-number value', () => {
            expect(() => new Float4Value("123" as any)).toThrow('Float4 value must be a number, got string');
        });
    });

    describe('parse', () => {
        it('should parse valid float string', () => {
            const float4 = Float4Value.parse('3.14159');
            expect(float4.value).toBeCloseTo(3.14159, 5);
        });
        ...
        it('should trim whitespace', () => {
            const float4 = Float4Value.parse('  3.14  ');
        ...
        it('should throw error for non-numeric string', () => {
            expect(() => Float4Value.parse('abc')).toThrow('Cannot parse "abc" as Float4');
        });

        it('should throw error for mixed alphanumeric', () => {
            expect(() => Float4Value.parse('3.14abc')).toThrow('Cannot parse "3.14abc" as Float4');
        });
    });
