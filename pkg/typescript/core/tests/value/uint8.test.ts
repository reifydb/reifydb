...
describe('Uint8', () => {
    describe('constructor', () => {
        ...
        it('should accept minimum value 0', () => {
            const uint8 = new Uint8(BigInt(0));
            expect(uint8.value).toBe(BigInt(0));
        });

        it('should accept maximum value 18446744073709551615', () => {
            const uint8 = new Uint8(BigInt("18446744073709551615"));
            expect(uint8.value).toBe(BigInt("18446744073709551615"));
        });

        it('should accept large positive number', () => {
            const uint8 = new Uint8(BigInt("10000000000000000"));
            expect(uint8.value).toBe(BigInt("10000000000000000"));
        });

        it('should throw error for negative value', () => {
            expect(() => new Uint8(BigInt(-1))).toThrow('Uint8 value must be between 0 and 18446744073709551615, got -1');
        });

        it('should throw error for value above maximum', () => {
            expect(() => new Uint8(BigInt("18446744073709551616"))).toThrow('Uint8 value must be between 0 and 18446744073709551615, got 18446744073709551616');
        });
    });

    describe('parse', () => {
        it('should parse valid integer string', () => {
            const uint8 = Uint8.parse('1000000');
            expect(uint8.value).toBe(BigInt(1000000));
        });

        it('should parse minimum value string', () => {
            const uint8 = Uint8.parse('0');
            expect(uint8.value).toBe(BigInt(0));
        });

        it('should parse maximum value string', () => {
            const uint8 = Uint8.parse('18446744073709551615');
            expect(uint8.value).toBe(BigInt("18446744073709551615"));
        });

        it('should parse large positive number string', () => {
            const uint8 = Uint8.parse('10000000000000000');
            expect(uint8.value).toBe(BigInt("10000000000000000"));
        });
        ...
        it('should throw error for non-numeric string', () => {
            expect(() => Uint8.parse('abc')).toThrow('Cannot parse "abc" as Uint8');
        });

        it('should throw error for negative value string', () => {
            expect(() => Uint8.parse('-1')).toThrow('Uint8 value must be between 0 and 18446744073709551615, got -1');
        });

        it('should throw error for value outside range', () => {
            expect(() => Uint8.parse('18446744073709551616')).toThrow('Uint8 value must be between 0 and 18446744073709551615, got 18446744073709551616');
        });

        it('should throw error for decimal string', () => {
            expect(() => Uint8.parse('42.5')).toThrow('Cannot parse "42.5" as Uint8');
        });
    });
    ...
    describe('toNumber', () => {
        ...
        it('should convert large value to number (with potential precision loss)', () => {
            const uint8 = new Uint8(BigInt("9007199254740993")); // Number.MAX_SAFE_INTEGER + 2
