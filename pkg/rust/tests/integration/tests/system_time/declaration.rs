...
fn a_populator_naming_no_column_is_rejected_at_definition_time() {
	let db = db();
	let err = admin_err(&db, "CREATE TABLE st::t { id: int4 } with { time: event, ts: nope }");
...
fn a_populator_that_is_not_a_datetime_is_rejected_at_definition_time() {
	let db = db();
	let err = admin_err(&db, "CREATE TABLE st::t { id: int4 } with { time: event, ts: id }");
...
fn a_datetime_populator_is_accepted() {
	let db = db();
	db.admin("CREATE TABLE st::t { id: int4, at: datetime } with { time: event, ts: at }");
...
fn every_source_object_kind_validates_its_populator() {
	let db = db();
	for rql in [
		"CREATE TABLE st::a { id: int4 } with { time: event, ts: nope }",
		"CREATE SERIES st::b { k: int8, id: int4 } with { key: k, time: event, ts: nope }",
		"CREATE RINGBUFFER st::c { id: int4 } with { capacity: 8, time: event, ts: nope }",
		"CREATE QUEUE st::d { id: int4 } with { fifo: {}, time: event, ts: nope }",
	] {
