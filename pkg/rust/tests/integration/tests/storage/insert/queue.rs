...
fn plain() -> TestDb {
	...
	db.admin("create queue test::q { id: int4, status: utf8 } with { fifo: {} }");
	db
}
...
fn colliding() -> TestDb {
	let db = TestDb::memory();
	db.admin("create namespace test");
	db.admin("create queue test::q { id: int4, status: utf8, pre_status: utf8 } with { fifo: {} }");
	db
}
...
fn test_insert_returning_post_image() {
	...
	let frames = db.command(r#"insert test::q [{ id: 1, status: "new" }] returning { id, status }"#);
	let f = &frames[0];
...
fn test_insert_returning_pre_image_is_none() {
	...
	let frames = db.command(r#"insert test::q [{ id: 1, status: "new" }] returning { id, pre_status }"#);
	let f = &frames[0];
...
fn test_insert_returning_pre_prefixed_column_is_the_stored_column() {
	...
	let frames = db
		.command(r#"insert test::q [{ id: 1, status: "new", pre_status: "b" }] returning { id, pre_status }"#);
	let f = &frames[0];
