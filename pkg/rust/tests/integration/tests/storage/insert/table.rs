...
fn test_insert_returning_post_image() {
	...
	let frames = db.command(r#"insert test::t [{ id: 1, status: "new" }] returning { id, status }"#);
	let f = &frames[0];
...
fn test_insert_returning_pre_image_is_none() {
	...
	let frames = db.command(r#"insert test::t [{ id: 1, status: "new" }] returning { id, pre_status }"#);
	let f = &frames[0];
...
fn test_insert_returning_pre_prefixed_column_is_the_stored_column() {
	...
	let frames = db
		.command(r#"insert test::t [{ id: 1, status: "new", pre_status: "b" }] returning { id, pre_status }"#);
	let f = &frames[0];
