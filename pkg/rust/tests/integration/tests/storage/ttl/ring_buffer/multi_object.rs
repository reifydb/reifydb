...
fn ddl(name: &str, ttl: Option<&str>) -> String {
	match ttl {
		Some(ttl) => format!(
			"create ringbuffer test::{name} {{ n: int4 }} with {{ time: processing, capacity: 100, row: {{ ttl: {ttl} }} }}"
		),
		None => format!("create ringbuffer test::{name} {{ n: int4 }} with {{ capacity: 100 }}"),
	}
}
...
#[test]
fn every_buffer_drains_when_one_tick_reaches_only_one_of_them() {
	// A tick that always restarts at the first object never reaches the last, and it expires forever.
	let path = TempDbPath::new("ttl_multi_object_ringbuffer");
...
#[test]
fn a_small_buffer_is_not_starved_by_a_large_one() {
	// A budget spent entirely on the backlog ahead of it leaves the small object unvisited forever.
	let path = TempDbPath::new("ttl_multi_object_ringbuffer_starve");
...
#[test]
fn a_buffer_without_a_ttl_is_never_touched() {
	// Only objects that declare a ttl may be scanned, or an untimed buffer quietly loses rows.
	let path = TempDbPath::new("ttl_multi_object_ringbuffer_untimed");
	...
	assert_eq!(db.row_count("from test::untimed"), 3, "a buffer with no ttl must keep every row");
	db.stop();
}
