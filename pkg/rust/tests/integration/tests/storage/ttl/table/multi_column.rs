...
const TWO_COL: &str = "create table test::t { a: utf8, b: utf8, n: int4 } with { time: processing, row: { ttl: %TTL%s }, partition: { by: { a, b } } }";

const FOUR_COL: &str = "create table test::t { a: utf8, b: utf8, c: utf8, d: utf8, n: int4 } with { time: processing, row: { ttl: %TTL%s }, partition: { by: { a, b, c, d } } }";
...
const TWO_COL_ROWS: &str =
	"insert test::t [{ a: \"x\", b: \"1\", n: 1 }, { a: \"x\", b: \"2\", n: 2 }, { a: \"y\", b: \"1\", n: 3 }]";

const FOUR_COL_ROWS: &str = "insert test::t [{ a: \"x\", b: \"1\", c: \"p\", d: \"q\", n: 1 }, { a: \"x\", b: \"2\", c: \"p\", d: \"q\", n: 2 }, { a: \"y\", b: \"1\", c: \"r\", d: \"s\", n: 3 }]";

#[test]
fn two_column_partitions_evict_nothing_while_every_row_is_inside_the_ttl() {
	let path = TempDbPath::new("ttl_table_2col_none");
	...
	assert_eq!(db.row_count("from test::t"), 3, "a live evictor must leave every in-ttl row alone");
	db.stop();
}

#[test]
fn two_column_partitions_drain_completely_once_every_row_expires() {
	let path = TempDbPath::new("ttl_table_2col_full");
...
fn two_column_partitions_evict_only_the_aged_partition() {
	...
	let path = TempDbPath::new("ttl_table_2col_partial");

	let mut db = ttl_db(&path, []);
	db.admin("create namespace test");
	db.admin(&ddl(TWO_COL, "10"));
	db.command("insert test::t [{ a: \"old\", b: \"1\", n: 1 }, { a: \"old\", b: \"2\", n: 2 }]");

	age_past(STRADDLE_TTL_SECS);
	db.command("insert test::t [{ a: \"new\", b: \"1\", n: 3 }]");

	await_survivor(&db, "from test::t", 1);
	assert_eq!(db.row_count("from test::t filter a == \"new\""), 1, "the younger partition must survive");
	assert_eq!(db.row_count("from test::t filter a == \"old\""), 0, "both aged partitions must be emptied");
	db.stop();
}

#[test]
fn four_column_partitions_evict_nothing_while_every_row_is_inside_the_ttl() {
	let path = TempDbPath::new("ttl_table_4col_none");
...
fn four_column_partitions_drain_completely_once_every_row_expires() {
	let path = TempDbPath::new("ttl_table_4col_full");
...
fn four_column_partitions_evict_only_the_aged_partition() {
	...
	let path = TempDbPath::new("ttl_table_4col_partial");

	let mut db = ttl_db(&path, []);
	db.admin("create namespace test");
	db.admin(&ddl(FOUR_COL, "10"));
	db.command(
		"insert test::t [{ a: \"old\", b: \"1\", c: \"p\", d: \"q\", n: 1 }, { a: \"old\", b: \"2\", c: \"p\", d: \"q\", n: 2 }]",
	);

	age_past(STRADDLE_TTL_SECS);
	db.command("insert test::t [{ a: \"new\", b: \"1\", c: \"r\", d: \"s\", n: 3 }]");
