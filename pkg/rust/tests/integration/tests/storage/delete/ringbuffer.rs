...
fn test_delete_returning_post_image() {
	...
	let db = plain();
	db.command(r#"insert test::rb [{ id: 1, status: "old" }]"#);

	let frames = db.command(r#"delete test::rb filter { id == 1 } returning { id, status }"#);
	let f = &frames[0];

	assert_eq!(f.row_count(), 1);
	assert_eq!(f.get::<String>("status", 0).unwrap().unwrap(), "old");

	let frames = db.query("from test::rb");
...
fn test_delete_returning_pre_image_equals_post_image() {
	...
	let frames = db.command(r#"delete test::rb filter { id == 1 } returning { pre_status, status }"#);
	let f = &frames[0];
...
fn test_delete_returning_pre_prefixed_column_is_the_stored_column() {
	...
	let db = colliding();
	db.command(r#"insert test::rb [{ id: 1, status: "old", pre_status: "a" }]"#);

	let frames = db.command(r#"delete test::rb filter { id == 1 } returning { pre_status }"#);
	let f = &frames[0];
