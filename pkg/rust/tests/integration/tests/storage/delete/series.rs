...
fn test_delete_returning_post_image() {
	...
	let frames = db.command(r#"delete test::s filter { ts == 1000 } returning { ts, status }"#);
	let f = &frames[0];

	assert_eq!(f.row_count(), 1);
	assert_eq!(f.get::<String>("status", 0).unwrap().unwrap(), "old");

	let frames = db.query("from test::s");
...
fn test_delete_returning_pre_image_equals_post_image() {
	...
	let frames = db.command(r#"delete test::s filter { ts == 1000 } returning { pre_status, status }"#);
	let f = &frames[0];
...
fn test_delete_returning_pre_prefixed_column_is_the_stored_column() {
	...
	let frames = db.command(r#"delete test::s filter { ts == 1000 } returning { pre_status }"#);
	let f = &frames[0];
