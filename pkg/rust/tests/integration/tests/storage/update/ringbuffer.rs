...
fn test_update_returning_post_image() {
	...
	let frames = db.command(r#"update test::rb { status: "new" } filter { id == 1 } returning { id, status }"#);
	let f = &frames[0];
...
fn test_update_returning_pre_image() {
	...
	let frames =
		db.command(r#"update test::rb { status: "new" } filter { id == 1 } returning { pre_status, status }"#);
	let f = &frames[0];
...
fn test_update_returning_pre_prefixed_column_is_the_stored_column() {
	...
	let frames = db.command(
		r#"update test::rb { status: "new", pre_status: "b" } filter { id == 1 } returning { pre_status }"#,
	);
	let f = &frames[0];
