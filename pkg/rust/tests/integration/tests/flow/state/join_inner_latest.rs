...
fn latest_inner_join(db: &TestDb) {
...
#[test]
fn a_left_row_with_no_slot_publishes_nothing_until_one_arrives() {
	// An inner join has no unmatched form, so the left row must wait in state and publish only once matched.
	let db = setup();
	latest_inner_join(&db);
	insert_left(&db);
	db.await_all_flows(TIMEOUT);
	...
	assert_eq!(db.row_count("FROM app::j"), 1, "and the first slot must publish it exactly once");
}

#[test]
fn emptying_the_slot_withdraws_the_joined_row() {
	// An inner join's row exists only while both sides do, so losing the slot must retract it rather than strand
	// it.
	let db = setup();
	latest_inner_join(&db);
	insert_left(&db);
