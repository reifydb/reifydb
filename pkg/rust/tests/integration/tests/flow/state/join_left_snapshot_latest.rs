...
fn snapshot_latest_left_join(db: &TestDb, tail: &str) {
	...
	db.admin(&format!(r#"CREATE DEFERRED VIEW app::j {{ k: int4, lv: int4, rv: Option(int4) }} AS {{
			FROM app::lhs
				| left join {{ from app::rhs }} as r using (k, r.k)
					with {{ seal: {{ left: {{ duration: '1s' }} }}, snapshot: true, latest: true }}
...
fn a_left_row_joins_only_the_newest_right_row_that_was_already_there() {
	...
	assert_eq!(await_keys_in(&db, PIN, 1), 1, "pinning the single version it was published against");
}

#[test]
fn a_right_row_arriving_after_the_left_row_never_reaches_it() {
	// A snapshot join freezes the left row's view of the slot, so a later arrival must not revise it.
	let db = setup();
	snapshot_latest_left_join(&db, "");
	insert_left(&db);
	db.await_row_count("FROM app::j", 1, TIMEOUT);
	assert!(
		...
		"precondition: a left row that saw an empty slot publishes none, found {:?}",
		view_rv(&db)
	);
	...
	assert_eq!(db.row_count("FROM app::j"), 1, "filling the slot later must not add a row to a frozen left row");
	assert!(
		matches!(view_rv(&db).as_slice(), [Value::None { .. }]),
...
fn withdrawing_a_left_row_retracts_the_slot_version_it_published_against() {
	...
	let db = setup();
	snapshot_latest_left_join(&db, PUBLISHED_VERSION_ONLY);
	insert_right_a(&db);
	db.await_all_flows(TIMEOUT);
	insert_left(&db);
	db.await_row_count("FROM app::j FILTER { rv == 7 }", 1, TIMEOUT);
...
fn a_sealed_left_row_takes_its_ledger_entry_and_spares_the_unsealed_slot() {
	...
	assert_eq!(keys_in(&db, LEFT), 0, "with still no left row anywhere");
	assert_eq!(
		await_keys_in(&db, RIGHT, 1),
		1,
