...
pub struct EmbeddedBlockingBuilder<VS, US, T>
where
    VS: VersionedStorage,
...
impl<VS, US, T> EmbeddedBlockingBuilder<VS, US, T>
where
    VS: VersionedStorage,
    US: UnversionedStorage,
    T: Transaction<VS, US>,
{
    ...
    pub fn build(self) -> EmbeddedBlocking<VS, US, T> {
        self.engine.get_hooks().trigger(OnInitHook {}).unwrap();
        EmbeddedBlocking { engine: self.engine }
    }
}

impl<VS, US, T> WithHooks<VS, US, T> for EmbeddedBlockingBuilder<VS, US, T>
where
    VS: VersionedStorage,
    US: UnversionedStorage,
    T: Transaction<VS, US>,
