...
use std::{cmp, collections::{BinaryHeap, HashMap}, sync::{
...
impl TaskScheduler {
	...
	pub fn cancel(&mut self, handle: TaskHandle) {
		...
		// Note: The task reference remains in the queue but will be ignored
		// when popped since it's no longer in the tasks map
	}
