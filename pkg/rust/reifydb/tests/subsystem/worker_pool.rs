...
fn test_worker_pool_periodic_tasks() {
	...
	let handle = pool
		.schedule_periodic(
			task,
			Duration::from_millis(30), // Increased interval for more predictable behavior
			Priority::Normal,
		)
		.unwrap();
...
fn test_priority_ordering_with_blocking_tasks() {
	...
	let blocking_task = Box::new(ClosureTask::new(
		...
		move |_ctx| {
			...
			// Wait until all tasks are queued before releasing this worker
...
fn test_priority_with_all_levels() {
	...
	let high_positions: Vec<_> = high_tasks
		.iter()
		.filter_map(|&id| final_results.iter().position(|&x| x == id))
		.collect();
	let normal_positions: Vec<_> = normal_tasks
		.iter()
		.filter_map(|&id| final_results.iter().position(|&x| x == id))
		.collect();
	let low_positions: Vec<_> = low_tasks
		.iter()
		.filter_map(|&id| final_results.iter().position(|&x| x == id))
		.collect();
	...
	assert!(
		max_high < min_normal,
		"All High tasks should execute before Normal tasks"
	);
...
fn test_priority_starvation_prevention() {
	...
	assert_eq!(
		high_count, 10,
		"All high priority tasks should be executed"
	);
	assert_eq!(
		low_count, 5,
...
fn test_priority_with_periodic_tasks() {
	...
	assert!(
		order.contains(&"high"),
		"High priority periodic task should execute"
	);
	assert!(
		order.contains(&"low"),
