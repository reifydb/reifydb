...
fn main() -> Result<(), Box<dyn std::error::Error>> {
	...
	let session = HttpCallbackSession::new("127.0.0.1", 8090, Some("mysecrettoken".to_string()))?;

	// Execute a command to create a table
	session.command("CREATE NAMESPACE test; CREATE TABLE test.users { id: INT4, name: UTF8 }", None, |result| {
		match result {
