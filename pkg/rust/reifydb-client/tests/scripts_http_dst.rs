...
impl testscript::runner::Runner for DstHttpRunner {
	fn run(&mut self, command: &Command) -> Result<String, Box<dyn Error>> {
		...
		match command.name.as_str() {
			"admin" => {
				...
				println!("admin: {rql}");
				let response = ctx.http_client.admin(identity, vec![rql], Params::None);
			...
			"command" => {
				let rql = parse_rql(command);
				println!("command: {rql}");
				let response = ctx.http_client.command(identity, vec![rql], Params::None);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"command_positional" => {
				...
				let response = ctx.http_client.command(identity, vec![rql], params);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"query" => {
				let rql = parse_rql(command);
				println!("query: {rql}");
				let response = ctx.http_client.query(identity, vec![rql], Params::None);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"query_positional" => {
				...
				let response = ctx.http_client.query(identity, vec![rql], params);
				write_frames(dst_response_to_result(response)?)
			}
...
test_each_path! { in "pkg/rust/reifydb-client/tests/scripts" as scripts_dst_http => test_dst_http }

fn test_dst_http(path: &Path) {
	testscript::runner::run_path(&mut DstHttpRunner::new(), path).unwrap()
}
