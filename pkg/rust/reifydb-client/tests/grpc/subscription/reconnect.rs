...
use reifydb_client::{
	BatchStreamEvent, BatchSubscribeItem, GrpcClient, GrpcClientOptions, ReconnectOptions, SubscriptionConfig,
	WireFormat,
};
...
fn reconnect_transparently_resubscribes_batch() {
	with_grpc_server(|port| async move {
		...
		let mut batch = client
			.batch_subscribe(&[BatchSubscribeItem::new(&query, SubscriptionConfig::default())])
			.await
			.unwrap();
