...
use crate::ws::subscription::{create_test_table, find_column, recv_with_timeout, unique_table_name};
...
fn test_filtered_subscription() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("from [{{ id: 15, value: 150 }}] insert test.{}", table), None).await.unwrap();
...
fn test_no_callback_for_non_matching() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("from [{{ id: 5, value: 50 }}] insert test.{}", table), None).await.unwrap();
