...
fn test_multiple_subscriptions_different_tables() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("from [{{ id: 1, name: 'alice' }}] insert test.{}", table1), None)
			.await
			.unwrap();
...
fn test_multiple_subscriptions_same_table() {
	...
	runtime.block_on(async {
		...
		// Insert data
		client.command(&format!("from [{{ id: 1, name: 'test' }}] insert test.{}", table), None).await.unwrap();
...
fn test_changes_routed_to_correct_subscription() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("from [{{ id: 100 }}] insert test.{}", table1), None).await.unwrap();
