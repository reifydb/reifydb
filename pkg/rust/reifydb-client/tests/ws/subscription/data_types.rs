...
fn test_subscription_int_types() {
	...
	runtime.block_on(async {
		...
		client.command(
			&format!(
				"INSERT test.{} FROM [{{ i1: 127, i2: 32767, i4: 2147483647, i8: 9223372036854775807 }}]",
				table
			),
			None,
		)
...
fn test_subscription_uint_types() {
	...
	runtime.block_on(async {
		...
		client.command(
			&format!(
				"INSERT test.{} FROM [{{ u1: 255, u2: 65535, u4: 4294967295, u8: 18446744073709551615 }}]",
				table
			),
			None,
		)
...
fn test_subscription_float_types() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("INSERT test.{} FROM [{{ f4: 3.14, f8: 2.718281828459045 }}]", table), None)
			.await
			.unwrap();
...
fn test_subscription_string_types() {
	...
	runtime.block_on(async {
		...
		client.command(&format!("INSERT test.{} FROM [{{ s: 'hello world', s2: 'test data' }}]", table), None)
			.await
			.unwrap();
...
fn test_subscription_temporal() {
	...
	runtime.block_on(async {
		...
		client.command(
			&format!(
				"INSERT test.{} FROM [{{ d: '2025-01-15', t: '14:30:00', dt: '2025-01-15T14:30:00Z' }}]",
				table
			),
			None,
		)
...
fn test_subscription_uuid() {
	...
	runtime.block_on(async {
		...
		client
			.command(
				&format!(
					"INSERT test.{} FROM [{{ u4: 'a0eebc99-9c0b-4ef8-bb6d-6bb9bd380a11', u7: '019478f0-d3af-7e22-9d7a-f8d7c7a3b3c4' }}]",
					table
				),
				None,
			)
