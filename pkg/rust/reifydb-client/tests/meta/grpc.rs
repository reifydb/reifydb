	...
	Fut: std::future::Future<Output = ()>,
{
	...
	runtime.block_on(async {
		let mut client = GrpcClient::connect(&format!("http://[::1]:{}", port), format).await.unwrap();
...
#[test]
fn admin_with_meta_proto() {
	run_with_format(WireFormat::Proto, |client| async move {
