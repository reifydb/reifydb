...
impl testscript::runner::Runner for DstGrpcRunner {
	fn run(&mut self, command: &Command) -> Result<String, Box<dyn Error>> {
		...
		match command.name.as_str() {
			"admin" => {
				...
				println!("admin: {rql}");
				let response = ctx.grpc_client.admin(identity, vec![rql], Params::None);
			...
			"command" => {
				let rql = parse_rql(command);
				println!("command: {rql}");
				let response = ctx.grpc_client.command(identity, vec![rql], Params::None);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"command_positional" => {
				...
				let response = ctx.grpc_client.command(identity, vec![rql], params);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"query" => {
				let rql = parse_rql(command);
				println!("query: {rql}");
				let response = ctx.grpc_client.query(identity, vec![rql], Params::None);
				write_frames(dst_response_to_result(response)?)
			}
			...
			"query_positional" => {
				...
				let response = ctx.grpc_client.query(identity, vec![rql], params);
				write_frames(dst_response_to_result(response)?)
			}
...
test_each_path! { in "pkg/rust/reifydb-client/tests/scripts" as scripts_dst_grpc => test_dst_grpc }

fn test_dst_grpc(path: &Path) {
	testscript::runner::run_path(&mut DstGrpcRunner::new(), path).unwrap()
}
