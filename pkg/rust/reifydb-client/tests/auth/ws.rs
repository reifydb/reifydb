...
fn test_password_login_success() {
	...
	runtime.block_on(async {
		let mut client = WsClient::connect(&format!("ws://[::1]:{}", ws_port), Encoding::Json).await.unwrap();
...
fn test_logout_success() {
	...
	runtime.block_on(async {
		...
		let mut client2 = WsClient::connect(&format!("ws://[::1]:{}", ws_port), Encoding::Json).await.unwrap();
...
fn test_logout_independent_sessions() {
	...
	runtime.block_on(async {
		let mut client_a = WsClient::connect(&format!("ws://[::1]:{}", ws_port), Encoding::Json).await.unwrap();
		let mut client_b = WsClient::connect(&format!("ws://[::1]:{}", ws_port), Encoding::Json).await.unwrap();
