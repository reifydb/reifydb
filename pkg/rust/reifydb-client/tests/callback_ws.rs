...
use common::{
	cleanup_server, cleanup_ws_client, connect_ws, parse_named_params,
...
impl<VT, UT, C> CallbackRunner<VT, UT, C>
where
	VT: VersionedTransaction,
	UT: UnversionedTransaction,
	C: CdcTransaction,
...
impl<VT, UT, C> testscript::Runner for CallbackRunner<VT, UT, C>
where
	VT: VersionedTransaction,
	UT: UnversionedTransaction,
	C: CdcTransaction,
	...
	fn run(&mut self, command: &Command) -> Result<String, Box<dyn Error>> {
		...
		match command.name.as_str() {
			"command" => {
				...
				let result_holder =
					self.last_command_result.clone();

				// Clear previous result
				*result_holder.lock().unwrap() = None;

				let _request_id = session.command(
					&rql,
					None,
					move |result| {
						let mut holder = result_holder
							.lock()
							.unwrap();
						*holder = Some(result.map_err(
							|e| e.to_string(),
						));
					},
				)?;
			...
			"query" => {
				...
				let result_holder =
					self.last_query_result.clone();

				// Clear previous result
				*result_holder.lock().unwrap() = None;

				let _request_id = session.query(
					&rql,
					None,
					move |result| {
						let mut holder = result_holder
...
fn test_callback(path: &Path) {
	retry(3, || {
		testscript::run_path(
			&mut CallbackRunner::new(optimistic(memory())),
			path,
		)
	})
	.expect("test failed")
