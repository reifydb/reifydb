...
fn value_to_wire(value: Value) -> WireValue {
	...
	let json = value_to_json(&value, &r#type);
	WireValue {
		r#type: WireValueType(r#type),
		value: json,
	}
}
...
mod tests {
	use reifydb_codec::json::NONE_MARKER;
	...
	fn a_none_of_int4_is_an_option_of_int4_with_the_bare_marker() {
		...
		assert_eq!(wire.value, json!(NONE_MARKER));
	}

	#[test]
	fn a_none_of_option_int4_is_a_two_layer_option_with_the_bare_marker() {
		// a none at the outermost layer carries no depth suffix, even under two Option layers
	...
	#[test]
	fn a_list_of_records_parameter_serializes_as_real_json_structure() {
		let regions = Value::List(vec![
			Value::Record(vec![("id".to_string(), Value::Utf8("us".to_string()))]),
			Value::Record(vec![("id".to_string(), Value::Utf8("eu".to_string()))]),
		]);
		let wire = value_to_wire(regions);
		assert_eq!(
			to_value(&wire).unwrap(),
			json!({
				"type": {"id": "List", "underlying": {"id": "Record", "underlying": [{"name": "id", "type": {"id": "Utf8"}}]}},
				"value": [{"id": "us"}, {"id": "eu"}]
			})
		);
	}
