...
import { ReifyError, rql, type Store } from '@reifydb/react'
...
const DELETE_MONITOR = rql.write([])<{ id: string }>`CALL uptime::delete_monitor($id)`
...
describe('monitor detail writes', () => {
  ...
  it('deletes the monitor with its regions, results and status page entries, and nothing of another monitor', async () => {
    ...
    const alpha = await createMonitor(db, identity, 'alpha', [usEast, euWest])
    const beta = await createMonitor(db, identity, 'beta', [usEast])
    ...
    const page = await createStatusPage(db, identity, 'public', 'Public', [alpha, beta])
  ...
  it('shows why a refused pause failed next to the buttons and writes nothing', async () => {
    ...
    const id = await createMonitor(db, identity, 'alpha', [usEast])
    await renderDetail(id)
    await commandAs(db, owner, DELETE_MONITOR, { id })
