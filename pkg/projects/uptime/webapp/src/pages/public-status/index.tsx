...
function OverallBanner({ statuses }: { statuses: string[] }) {
  ...
  const label = all_up
    ...
    : down > 0
      ...
      : degraded > 0
        ? `${degraded} of ${statuses.length} systems degraded`
        : 'Status partially unknown'
  const tone = all_up ? 'bg-status-success/10' : down > 0 ? 'bg-status-error/10' : 'bg-status-warning/10'
  return (
    <div
        ...
        <div className="space-y-6">
          ...
          <div className="glass-card divide-y divide-border-light">
            {data.monitors.map((m, index) => {
              ...
              const upCount = m.regions.filter((r) => r.status === 'up').length
              return (
                <div key={m.name} className="space-y-3 px-6 py-5">
                  ...
                  {isOpen && m.regions.length > 0 && (
                    <div className="space-y-4 border-t border-border-light pl-6 pt-4">
                      {m.regions.map((r) => (
                          ...
                          <div className="flex items-center justify-between gap-4">
                            ...
                            <div className="flex items-center gap-3">
                              <span className="text-xs text-text-muted">
                                Last <RelativeTime iso={r.last_checked_at} />
                              </span>
                              <StatusBadge status={r.status} className="text-xs" />
                            </div>
                          </div>
                          <DailyUptimeBar daily={r.daily} height="h-4" />
                        </div>
                      ))}
                    </div>
                  )}
