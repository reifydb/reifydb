...
FlowTransaction does not currently track the **read set** - the set of keys that have been read during the transaction. In a general-purpose MVCC (Multi-Version Concurrency Control) implementation, tracking reads is essential for conflict detection.

**Why read tracking matters in MVCC:**
- When a transaction commits, it needs to verify that none of the keys it read have been modified by concurrent transactions
